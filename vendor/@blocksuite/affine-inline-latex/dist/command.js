...
function openInlineLatexEditor(inlineEditor, index) {
    inlineEditor
        .waitForUpdate()
        .then(async () => {
...
function getSingleBlockInlineRange(textSelection) {
    if (textSelection.to) {
        return null;
    }
    return {
...
export const insertInlineLatex = (ctx, next) => {
    ...
    const latex = textSelection.isCollapsed()
        ? ''
        : inlineEditor.yTextString.slice(inlineRange.index, inlineRange.index + inlineRange.length);
