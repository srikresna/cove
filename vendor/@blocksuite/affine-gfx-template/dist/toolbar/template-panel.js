...
function toSvgPreviewDataUrl(svg) {
...
let EdgelessTemplatePanel = (() => {
  ...
  `; }
        ...
        async _insertTemplate(template, bound) {
            ...
            const templateJob = createTemplateJob(this.edgeless.std, template.type, center);
            try {
                ...
                if (insertedBound && template.type === 'template') {
                    ...
                    this.gfx.viewport.setViewportByBound(insertedBound, [padding, padding, padding, padding], true);
                }
            }
            finally {
