...
import { ConnectorElementModel, ConnectorMode, GroupElementModel, } from '@blocksuite/affine-model';
import { ThemeProvider } from '@blocksuite/affine-shared/services';
import { BlockSuiteError } from '@blocksuite/global/exceptions';
import { almostEqual, Bound, getBezierCurveBoundingBox, getBezierParameters, getBoundFromPoints, getBoundWithRotation, getPointFromBoundsWithRotation, isOverlap, isVecZero, lineIntersects, PI2, PointLocation, sign, toRadian, Vec, } from '@blocksuite/global/gfx';
...
function computePoints(startPoint, endPoint, nextStartPoint, lastEndPoint, startBound, endBound, expandStartBound, expandEndBound) {
    ...
    const result = getConnectablePoints(startPointVec3, endPointVec3, nextStartPointVec3, lastEndPointVec3, startBound, endBound, expandStartBound, expandEndBound);
...
function getConnectablePoints(startPoint, endPoint, nextStartPoint, lastEndPoint, startBound, endBound, expandStartBound, expandEndBound) {
    ...
    if (!startEnds[0] || !startEnds[1]) {
        throw new BlockSuiteError(BlockSuiteError.ErrorCode.ValueNotExists, 'Failed to get start and end points when getting connectable points');
    }
...
function mergePath(points) {
    ...
    for (let i = 0; i < result.length - 1; i++) {
        ...
        const isAlmostEqual = almostEqual(cur[0], next[0], 0.02) || almostEqual(cur[1], next[1], 0.02);
        if (!isAlmostEqual) {
    ...
    if (almostEqual(bound.x, result[0]))
        ...
        if (slope === 0) {
            if (xDirection > 0) {
                ...
                if (!intersects) {
                    throw new BlockSuiteError(BlockSuiteError.ErrorCode.ValueNotExists, 'Failed to get line intersections for getNextPoint');
                }
...
export class ConnectionOverlay extends Overlay {
    static { this.overlayName = 'connection'; }
    constructor(gfx) {
        super(gfx);
        this._themeDisposer = null;
    ...
    _getEmphasisColor() {
        return getComputedStyle(this.gfx.std.host).getPropertyValue('--affine-text-emphasis-color');
    }
    _setupThemeListener() {
    ...
    renderConnector(point, excludedIds = []) {
        ...
        for (let i = 0; i < connectables.length; i++) {
            ...
            for (; j < len; j++) {
                ...
                const anchorViewCoord = context.viewport.toViewCoord(anchor.point[0], anchor.point[1]);
            ...
            if (connectable.includesPoint(point[0], point[1], {
...
export class PathGenerator {
    ...
    generateOrthogonalConnectorPath(input) {
        ...
        const [, startPointV3, endPointV3, nextStartPointV3, lastEndPointV3] = points;
        adjustStartEndPoint(startPointV3, endPointV3, startBound, endBound);
        this._aStarRunner = new AStarRunner(finalPoints, nextStartPointV3, lastEndPointV3, startPointV3, endPointV3, blocks, expandBlocks);
...
export class ConnectorPathGenerator extends PathGenerator {
    ...
    static updatePath(connector, path, elementGetter) {
        ...
        if (source.id || target.id) {
            ...
            if (source.id) {
                ...
                startPoint.out = isVecZero(startTangentVertical)
                    ...
                    : Vec.mul(startTangentVertical, Math.max(100, Math.abs(Vec.pry(Vec.sub(endPoint, startPoint), startTangentVertical)) / 3));
            }
            if (target.id) {
                ...
                endPoint.in = isVecZero(endTangentVertical)
                    ...
                    : Vec.mul(endTangentVertical, Math.max(100, Math.abs(Vec.pry(Vec.sub(startPoint, endPoint), endTangentVertical)) / 3));
            }
            return [startPoint, endPoint];
        }
        else {
