...
export class ConnectorFilter extends InteractivityExtension {
    static { this.key = 'connector-filter'; }
    mounted() {
        this.action.onDragInitialize((context) => {
            ...
            const elements = context.elements.filter(elem => {
                if (elem instanceof ConnectorElementModel) {
                    const sourceElemNotFound = elem.source.id && !elementSet.has(elem.source.id);
                    const targetElemNotFound = elem.target.id && !elementSet.has(elem.target.id);
