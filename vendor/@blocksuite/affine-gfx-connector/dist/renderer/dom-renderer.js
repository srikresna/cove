...
const retainedConnectorDom = new WeakMap();
...
function createConnectorPath(points, mode) {
    if (points.length < 2)
        return '';
    ...
    if (mode === ConnectorMode.Curve) {
        ...
        for (let i = 1; i < points.length; i++) {
            ...
            const curr = points[i];
            pathBuilder.curveTo(prev.absOut[0], prev.absOut[1], curr.absIn[0], curr.absIn[1], curr[0], curr[1]);
        }
    }
    else {
        // Use straight lines
...
function createArrowMarker(id, style, color, strokeWidth, isStart = false) {
    ...
    switch (style) {
        case 'Arrow': {
            ...
            path.setAttribute('d', isStart ? 'M 20 5 L 10 10 L 20 15 Z' : 'M 0 5 L 10 10 L 0 15 Z');
        ...
        case 'Triangle': {
            const path = document.createElementNS(SVG_NS, 'path');
            path.setAttribute('d', isStart ? 'M 20 7 L 12 10 L 20 13 Z' : 'M 0 7 L 8 10 L 0 13 Z');
            path.setAttribute('fill', color);
            path.setAttribute('stroke', color);
            marker.append(path);
            break;
...
function clearRetainedConnectorDom(element) {
    retainedConnectorDom.delete(element);
    element.replaceChildren();
}
function getRetainedConnectorDom(element) {
...
function getOrCreateLabelElement(retained) {
    if (retained.label) {
...
function renderConnectorLabel(model, retained, renderer, zoom) {
    ...
    const { labelStyle: { color, fontSize, fontWeight, fontStyle, fontFamily, textAlign, }, } = model;
    ...
    labelElement.style.justifyContent =
        textAlign === 'center'
            ...
            : textAlign === 'right'
                ...
                : 'flex-start';
    // Style the text
    labelElement.style.color = renderer.getColorValue(color, DefaultTheme.black, true);
 ...
 */
export const connectorBaseDomRenderer = (model, element, renderer) => {
    const { zoom } = renderer.viewport;
    const { mode, path: points, strokeStyle, frontEndpointStyle, rearEndpointStyle, strokeWidth, stroke, } = model;
    ...
    const strokeColor = renderer.getColorValue(stroke, DefaultTheme.connectorColor, true);
    const markers = [];
    let startMarkerId = '';
    let endMarkerId = '';
    if (frontEndpointStyle !== 'None') {
        startMarkerId = `start-marker-${model.id}`;
        markers.push(createArrowMarker(startMarkerId, frontEndpointStyle, strokeColor, strokeWidth, true));
    }
    if (rearEndpointStyle !== 'None') {
        endMarkerId = `end-marker-${model.id}`;
        markers.push(createArrowMarker(endMarkerId, rearEndpointStyle, strokeColor, strokeWidth, false));
    }
...
export const connectorDomRenderer = (model, element, renderer) => {
