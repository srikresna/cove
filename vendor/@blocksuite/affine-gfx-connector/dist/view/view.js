...
import { generateKeyBetween, GfxElementModelView, GfxViewInteractionExtension, } from '@blocksuite/std/gfx';
import { mountConnectorLabelEditor } from '../text/edgeless-connector-label-editor';
export class ConnectorElementView extends GfxElementModelView {
    constructor() {
        super(...arguments);
        ...
        this.onDragEnd = (context) => {
            super.onDragEnd(context);
            this.model.stash('labelXYWH');
        };
        this.onDragMove = (context) => {
            const { dx, dy, currentBound } = context;
            this.model.moveTo(currentBound.moveDelta(dx, dy));
        };
    }
    static { this.type = 'connector'; }
    onCreated() {
        super.onCreated();
        this._initLabelMoving();
    }
    _initLabelMoving() {
        let curLabelElement = null;
        if (this.model.isLocked()) {
            return;
        }
        const enterLabelEditor = (evt) => {
            const edgeless = this.std.view.getBlock(this.std.store.root.id);
            if (edgeless && !this.model.isLocked()) {
                mountConnectorLabelEditor(this.model, edgeless, this.gfx.viewport.toModelCoord(evt.x, evt.y));
            }
        };
        ...
        const watchEvent = (labelModel) => {
            const view = this.gfx.view.get(labelModel);
            ...
            view.on('dragmove', evt => {
                ...
                const center = connectorModel.getNearestPoint(Vec.add(newBound.center, delta));
        ...
        const updateLabelElement = () => {
            ...
            const labelElement = curLabelElement || new LocalShapeElementModel(this.surface);
        ...
        this.disposable.add(this.model.propsUpdated.subscribe(payload => {
            if (payload.key === 'labelXYWH' ||
                payload.key === 'text' ||
                payload.key === 'index') {
                updateLabelElement();
            }
...
export const ConnectorInteraction = GfxViewInteractionExtension(ConnectorElementView.type, {
