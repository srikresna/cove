...
import { createTextActions, normalizeTextBound, } from '@blocksuite/affine-gfx-text';
import { ConnectorElementModel, ConnectorMode, DEFAULT_FRONT_ENDPOINT_STYLE, DEFAULT_REAR_ENDPOINT_STYLE, DefaultTheme, LineWidth, PointStyle, resolveColor, StrokeStyle, } from '@blocksuite/affine-model';
import { ToolbarModuleExtension, } from '@blocksuite/affine-shared/services';
import { getMostCommonResolvedValue, getMostCommonValue, } from '@blocksuite/affine-shared/utils';
import { getRootBlock, LINE_STYLE_LIST, renderMenu, } from '@blocksuite/affine-widget-edgeless-toolbar';
import { Bound } from '@blocksuite/global/gfx';
import { AddTextIcon, ConnectorCIcon, ConnectorEIcon, ConnectorLIcon, EndPointArrowIcon, EndPointCircleIcon, EndPointDiamondIcon, EndPointTriangleIcon, FlipDirectionIcon, StartPointArrowIcon, StartPointCircleIcon, StartPointDiamondIcon, StartPointIcon, StartPointTriangleIcon, } from '@blocksuite/icons/lit';
    ...
    actions: [
        {
            ...
            content(ctx) {
                ...
                const strokeWidth = getMostCommonValue(models, 'strokeWidth') ?? LineWidth.Four;
                const strokeStyle = getMostCommonValue(models, 'strokeStyle') ?? StrokeStyle.Solid;
                const stroke = getMostCommonResolvedValue(models, field, stroke => resolveColor(stroke, theme)) ?? resolveColor(DefaultTheme.connectorColor, theme);
                const onPickColor = (e) => {
                    switch (e.type) {
                        case 'pick':
                            {
            ...
            actions: [
                {
                    ...
                    content(ctx) {
                        ...
                        const pointStyle = getMostCommonValue(models, field) ?? DEFAULT_FRONT_ENDPOINT_STYLE;
                        const onPick = (value) => {
                            updateModelsWith(ctx, models, field, value);
                        };
                        return renderMenu({
                ...
                },
                {
                    ...
                    run(ctx) {
                        ...
                        if (!models.length)
                            return;
                        const frontEndpointStyle = getMostCommonValue(models, 'frontEndpointStyle') ??
                            DEFAULT_FRONT_ENDPOINT_STYLE;
                        const rearEndpointStyle = getMostCommonValue(models, 'rearEndpointStyle') ??
                ...
                },
                {
                    ...
                    content(ctx) {
                        ...
                        const pointStyle = getMostCommonValue(models, field) ?? DEFAULT_REAR_ENDPOINT_STYLE;
                        const onPick = (value) => {
                            updateModelsWith(ctx, models, field, value);
                        };
                        return renderMenu({
                ...
                },
                {
                    ...
                    content(ctx) {
                        ...
                        if (!models.length)
                            return null;
                        const field = 'mode';
                        const mode = getMostCommonValue(models, field) ?? ConnectorMode.Orthogonal;
                        const onPick = (value) => {
                            updateModelsWith(ctx, models, field, value);
                        };
                        return renderMenu({
        ...
        // id: `g.text`
        ...createTextActions(ConnectorElementModel, 'connector', (ctx, model, props) => {
            ...
            const { text, labelXYWH, labelConstraints: { hasMaxWidth, maxWidth }, } = model;
        ...
        }, model => model.labelStyle, (model, type, _) => model[type]('labelStyle')).map(action => ({
            ...action,
            id: `g.text-${action.id}`,
            when(ctx) {
                const models = ctx.getSurfaceModelsByType(ConnectorElementModel);
...
function updateModelsWith(ctx, models, field, value) {
    ctx.store.captureSync();
    for (const model of models) {
        ctx.std
            .get(EdgelessCRUDIdentifier)
