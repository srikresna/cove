...
import { isInsidePageEditor, matchModels, } from '@blocksuite/affine-shared/utils';
import { BlockSuiteError, ErrorCode } from '@blocksuite/global/exceptions';
import { deserializeXYWH } from '@blocksuite/global/gfx';
import { StdIdentifier, } from '@blocksuite/std';
import { GfxBlockElementModel, GfxControllerIdentifier, GfxPrimitiveElementModel, isGfxGroupCompatibleModel, } from '@blocksuite/std/gfx';
...
export class ExportManager {
    ...
    async edgelessToCanvas(surfaceRenderer, bound, gfx, blocks, elements, edgelessBackground) {
        if (!(surfaceRenderer instanceof CanvasRenderer)) {
            console.warn('ExportManager.edgelessToCanvas was called with an invalid renderer type. Expected CanvasRenderer.');
            return undefined;
        }
        const rootModel = this.doc.root;
        if (!rootModel)
    ...
    async exportPdf() {
        ...
        FileExporter.exportFile(rootModel.props.title.toString() + '.pdf', pdfBase64);
    }
    async exportPng() {
        const rootModel = this.doc.root;
        if (!rootModel)
        ...
        FileExporter.exportPng(this.doc.root.props.title.toString(), canvasImage.toDataURL('image/png'));
    }
}
...
function getRootByEditorHost(editorHost) {
