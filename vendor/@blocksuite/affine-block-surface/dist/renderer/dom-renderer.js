...
export class DomRenderer {
    ...
    _renderOrUpdatePlaceholder(elementModel, viewportBounds, zoom, addedElements) {
        ...
        domElement.style.backgroundColor = resolveSurfacePlaceholderColor(this.getColorScheme());
    ...
    _watchSurface(surfaceModel) {
        ...
        this._disposables.add(surfaceModel.localElementAdded.subscribe(payload => {
            this._markElementDirty(payload.id, UpdateType.ELEMENT_ADDED, payload);
            this._markViewportDirty();
            this.refresh();
        }));
        ...
        this._disposables.add(surfaceModel.localElementUpdated.subscribe(payload => {
            this._markElementDirty(payload.model.id, UpdateType.ELEMENT_UPDATED, payload.model);
    ...
    _markElementDirty(elementId, updateType, elementModel) {
        ...
        if (updateType === UpdateType.ELEMENT_REMOVED) {
            ...
            const model = elementModel ?? this._surfaceModel.getElementById(elementId);
            if (model) {
                this._pendingElements.set(elementId, model);
            }
        }
    ...
    _elementInViewport(elementModel, viewportBounds) {
        const display = (elementModel.display ?? true) && !elementModel.hidden;
        return (display && intersects(getBoundWithRotation(elementModel), viewportBounds));
    }
    _getPendingElementsInViewport(viewportBounds) {
        const elements = [];
    ...
    _getElementsInViewport(viewportBounds) {
        ...
        for (const elementModel of this._getPendingElementsInViewport(viewportBounds)) {
            elementsById.set(elementModel.id, elementModel);
        }
