import { HastUtils, NotionHtmlASTToDeltaExtension, } from '@blocksuite/affine-shared/adapters';
...
export const notionHtmlSpanElementToDeltaMatcher = NotionHtmlASTToDeltaExtension({
    name: 'span-element',
    ...
    toDelta: (ast, context) => {
        ...
        if (Array.isArray(ast.properties?.className) &&
            ...
            const latex = HastUtils.getTextContent(HastUtils.querySelector(ast, 'annotation'));
            return [{ insert: ' ', attributes: { latex } }];
        }
        // Add underline style detection
        if (typeof ast.properties?.style === 'string' &&
            ast.properties?.style?.includes(NotionUnderlineStyleToken)) {
            return ast.children.flatMap(child => context.toDelta(child, options).map(delta => {
...
export const notionHtmlStrongElementToDeltaMatcher = NotionHtmlASTToDeltaExtension({
    name: 'strong-element',
    ...
    toDelta: (ast, context) => {
        ...
        const { toDelta, options } = context;
        return ast.children.flatMap(child => toDelta(child, options).map(delta => {
            delta.attributes = { ...delta.attributes, bold: true };
            return delta;
        }));
    },
});
export const notionHtmlItalicElementToDeltaMatcher = NotionHtmlASTToDeltaExtension({
    name: 'italic-element',
...
export const notionHtmlCodeElementToDeltaMatcher = NotionHtmlASTToDeltaExtension({
    name: 'code-element',
...
export const notionHtmlUnderlineElementToDeltaMatcher = NotionHtmlASTToDeltaExtension({
...
export const notionHtmlMarkElementToDeltaMatcher = NotionHtmlASTToDeltaExtension({
    name: 'mark-element',
...
export const notionHtmlStyleElementToDeltaMatcher = NotionHtmlASTToDeltaExtension({
    name: 'style-element',
