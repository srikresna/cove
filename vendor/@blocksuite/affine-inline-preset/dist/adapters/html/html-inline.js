import { HtmlASTToDeltaExtension, } from '@blocksuite/affine-shared/adapters';
import { collapseWhiteSpace } from 'collapse-white-space';
import { extractColorFromStyle, resolveNearestSupportedColor, } from './color-utils.js';
/**
 ...
 * ```
 */
const isLineThroughElement = (ast) => {
    if (!isElement(ast)) {
        return false;
    }
    const style = typeof ast.properties.style === 'string' ? ast.properties.style : '';
 ...
 * @returns
 */
const isTextLikeElement = (ast) => {
    if (!isElement(ast)) {
        return false;
    }
    return (textLikeElementTags.has(ast.tagName) &&
        !isStrongElement(ast) &&
        !isItalicElement(ast) &&
...
export const htmlColorStyleElementToDeltaMatcher = HtmlASTToDeltaExtension({
    ...
    toDelta: (ast, context) => {
        ...
        const colorValue = extractColorFromStyle(typeof ast.properties?.style === 'string' ? ast.properties.style : '');
        const mappedColor = colorValue
            ? resolveNearestSupportedColor(colorValue)
            : null;
        const deltas = ast.children.flatMap(child => context.toDelta(child, baseOptions).map(delta => {
            if (mappedColor) {
...
export const htmlMarkElementToDeltaMatcher = HtmlASTToDeltaExtension({
    ...
    toDelta: (ast, context) => {
        ...
        const dataColor = typeof ast.properties?.dataColor === 'string'
            ? ast.properties.dataColor
            : '';
        const colorName = dataColor &&
