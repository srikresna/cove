...
export function getCombinedTextAttributes(chain) {
    return handleCurrentSelection(chain, (type, inlineEditors) => {
        if (type === 'text') {
            return {
                textAttributes: getCombinedFormatFromInlineEditors(inlineEditors.map(e => [e, e.getInlineRange()])),
            };
        }
        if (type === 'block') {
            return {
                textAttributes: getCombinedFormatFromInlineEditors(inlineEditors.map(e => [e, { index: 0, length: e.yTextLength }])),
            };
        }
        if (type === 'native') {
            return {
