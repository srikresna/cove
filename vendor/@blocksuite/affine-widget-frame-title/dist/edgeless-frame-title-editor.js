...
import { AFFINE_FRAME_TITLE_WIDGET, } from './affine-frame-title-widget';
...
let EdgelessFrameTitleEditor = (() => {
  ...
  `; }
        ...
        render() {
            ...
            const isInner = this.gfx.grid.has(this.frameModel.elementBound, true, true, model => model !== this.frameModel && model instanceof FrameBlockModel);
            const frameTitleWidget = this.edgeless.std.view.getWidget(AFFINE_FRAME_TITLE_WIDGET, this.frameModel.id);
            if (!frameTitleWidget)
                return nothing;
            const frameTitle = frameTitleWidget.querySelector('affine-frame-title');
