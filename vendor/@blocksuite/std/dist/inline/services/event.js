...
export class EventService {
    ...
    constructor(editor) {
        ...
        this._getClosestInlineRoot = (node) => {
            const el = node instanceof Element ? node : node.parentElement;
            return el?.closest(`[${INLINE_ROOT_ATTR}]`) ?? null;
        };
        this._isRangeCompletelyInRoot = (range) => {
            if (range.commonAncestorContainer.ownerDocument !== document)
                return false;
            ...
            const containsStart = range.startContainer === rootElement ||
                rootElement.contains(range.startContainer);
            const containsEnd = range.endContainer === rootElement ||
                rootElement.contains(range.endContainer);
            return containsStart && containsEnd;
        };
        this._onBeforeInput = async (event) => {
            const range = this.editor.rangeService.getNativeRange();
            if (this.editor.isReadonly || !range)
                return;
            const rootElement = this.editor.rootElement;
            if (!rootElement)
                return;
            const startInRoot = range.startContainer === rootElement ||
                rootElement.contains(range.startContainer);
            const endInRoot = range.endContainer === rootElement ||
                rootElement.contains(range.endContainer);
            ...
            if (startInRoot !== endInRoot) {
                ...
                if (otherRoot && otherRoot !== rootElement)
                    return;
            }
            ...
            transformInput(ctx.raw.inputType, ctx.data, ctx.attributes, ctx.inlineRange, this.editor);
