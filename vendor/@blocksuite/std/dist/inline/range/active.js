...
export function isRangeSyncExcludedTarget(target) {
    return !!getClosestElement(target)?.closest(`[${RANGE_SYNC_EXCLUDE_ATTR}="true"]`);
}
export function shouldDeactivateEditorOnFocusOut(editorHost, relatedTarget) {
 ...
 */
export function isActiveInEditor(editorHost) {
    const currentActiveElement = document.activeElement;
    if (!currentActiveElement)
        return false;
    // The input or textarea in the widget should be ignored.
    if (isRangeSyncExcludedTarget(currentActiveElement))
        return false;
