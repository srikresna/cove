    ...
    constructor(manager) {
        ...
        this._onCompositionStart = () => {
            ...
            this._compositionStartCallback = async (event) => {
                ...
                delete this.host.renderRoot['_$litPart$'];
        ...
        this._onNativeSelectionChanged = async () => {
            ...
            // ex. placeholder
            const isRangeOutNotEditable = range.startContainer instanceof HTMLElement &&
        ...
        this._onStdSelectionChanged = (selections) => {
            ...
            if (closestHost && closestHost !== this.host)
                return;
            const active = this.host.event.active;
            if (!active)
                return;
            const text = selections.find((selection) => selection.is(TextSelection)) ?? null;
            if (!text && selections.length > 0) {
                const hasRecoverable = selections.find(selection => {
                    const selectionConstructor = selection.constructor;
        ...
        this._prevTextSelection = null;
        this.isComposing = false;
        this.host.disposables.add(this.selectionManager.slots.changed.subscribe(this._onStdSelectionChanged));
