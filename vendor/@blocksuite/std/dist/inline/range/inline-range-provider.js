...
export const getInlineRangeProvider = element => {
    ...
    const calculateInlineRange = (range, textSelection) => {
        ...
        const elementRange = rangeManager.textSelectionToRange(selectionManager.create(TextSelection, {
            from: {
                index: 0,
    ...
    element.disposables.add(selectionManager.slots.changed.subscribe(selections => {
        if (!isActiveInEditor(editorHost))
            return;
