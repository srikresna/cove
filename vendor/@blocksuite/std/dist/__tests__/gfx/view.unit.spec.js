...
import { GfxViewportElement, shouldUseLowZoomBlockSurvivalMode, } from '../../gfx/viewport-element.js';
import { TestEditorContainer } from '../test-editor.js';
import { TestLocalElement } from '../test-gfx-element.js';
import { RootBlockSchemaExtension, SurfaceBlockSchemaExtension, TestGfxBlockSchemaExtension, } from '../test-schema.js';
...
const commonSetup = async () => {
    ...
    return {
        ...
        surfaceId,
        rootId,
        surfaceModel: surfaceBlock.model,
    };
};
...
const getTestGfxBlockModel = (gfx, id) => {
...
const getTestGfxBlockView = (gfx, id) => {
...
const getViewportChildBlockIds = (viewportElement) => [...viewportElement.children].map(child => child.dataset.blockId);
const setBlockXYWH = (gfx, id, xywh) => {
...
describe('gfx element view basic', () => {
    ...
    test('detects low-zoom DOM survival mode only during active gestures for gesture-safe viewport configs', () => {
        expect(shouldUseLowZoomBlockSurvivalMode({
            zoom: 0.4,
    ...
    test('keeps selected block active while degrading unselected low-zoom viewport blocks', async () => {
        ...
        const waitViewConnected = waitGfxViewConnected(gfx);
        const selectedId = gfx.std.store.addBlock('test:gfx-block', undefined, surfaceId);
        const inViewportId = gfx.std.store.addBlock('test:gfx-block', undefined, surfaceId);
        const outOfViewportId = gfx.std.store.addBlock('test:gfx-block', undefined, surfaceId);
        await Promise.all([
        ...
        viewportElement.getModelsInViewport = () => new Set([selectedModel, inViewportModel]);
        viewportElement._lastVisibleModels = new Set([
            selectedModel,
            inViewportModel,
            outOfViewportModel,
        ]);
        viewportElement._hideOutsideAndNoSelectedBlock();
    ...
    test('parks non-active low-zoom gesture blocks outside viewport DOM while gesture is running', async () => {
        ...
        const waitViewConnected = waitGfxViewConnected(gfx);
        const selectedId = gfx.std.store.addBlock('test:gfx-block', undefined, surfaceId);
        const nearbyId = gfx.std.store.addBlock('test:gfx-block', undefined, surfaceId);
        const farVisibleId = gfx.std.store.addBlock('test:gfx-block', undefined, surfaceId);
        const outOfViewportId = gfx.std.store.addBlock('test:gfx-block', undefined, surfaceId);
        await Promise.all([
            waitViewConnected(selectedId),
        ...
        gfx.viewport._shell = shell;
        gfx.viewport._cachedBoundingClientRect = new DOMRect(0, 0, 844, 390);
        gfx.viewport._cachedOffsetWidth = 844;
        gfx.viewport.setZoom(0.4, { x: 0, y: 0 }, false, true, true);
        gfx.viewport.panning$.next(true);
        const viewportElement = new GfxViewportElement();
        viewportElement.host = editorContainer.std.host;
        viewportElement.viewport = gfx.viewport;
        viewportElement.getModelsInViewport = () => new Set([selectedModel, nearbyModel, farVisibleModel]);
        document.body.append(viewportElement);
        viewportElement.append(selectedView, nearbyView, farVisibleView, outOfViewportView);
        viewportElement._hideOutsideAndNoSelectedBlock();
    ...
    test('restores parked low-zoom blocks after gesture recovery completes', async () => {
        ...
        try {
            const { editorContainer, gfx, surfaceId } = await commonSetup();
            const waitViewConnected = waitGfxViewConnected(gfx);
            const firstId = gfx.std.store.addBlock('test:gfx-block', undefined, surfaceId);
            const secondId = gfx.std.store.addBlock('test:gfx-block', undefined, surfaceId);
            const thirdId = gfx.std.store.addBlock('test:gfx-block', undefined, surfaceId);
            await Promise.all([
            ...
            viewportElement.host = editorContainer.std.host;
            viewportElement.viewport = gfx.viewport;
            viewportElement.getModelsInViewport = () => new Set([firstModel, secondModel, thirdModel]);
            document.body.append(viewportElement);
            ...
            expect(new Set(getViewportChildBlockIds(viewportElement))).toEqual(new Set([firstId, secondId, thirdId]));
    ...
    test('programmatic low-zoom viewport changes still emit viewport updates', async () => {
        ...
        viewport.SKIP_REFRESH_DURING_GESTURE = true;
        const updates = [];
        const subscription = viewport.viewportUpdated.subscribe(({ zoom, center }) => {
    ...
    test('chunked low-zoom refresh idles out-of-viewport blocks on the first pass', async () => {
        ...
        await new Promise(resolve => {
            viewportElement._chunkedHideOutsideAndNoSelectedBlock(resolve);
        });
        expect(selectedView.transformState$.value).toBe('active');
        expect(inViewportView.transformState$.value).toBe('survival');
        expect(outOfViewportView.transformState$.value).toBe('idle');
    ...
    test('resize completion clears low-zoom gesture recovery before sizeUpdated subscribers run', async () => {
        ...
        gfx.viewport._cachedBoundingClientRect = new DOMRect(0, 0, 844, 390);
        gfx.viewport._cachedOffsetWidth = 844;
        let panningAtSizeUpdated = null;
        let zoomingAtSizeUpdated = null;
        let blockSurvivalAtSizeUpdated = null;
        let canvasRecoveryDelayAtSizeUpdated = null;
        const subscription = gfx.viewport.sizeUpdated.subscribe(() => {
            const gestureActive = gfx.viewport.panning$.value || gfx.viewport.zooming$.value;
