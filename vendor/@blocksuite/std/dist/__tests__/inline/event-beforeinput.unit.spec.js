...
effects();
async function setupInlineEditor(text) {
...
async function teardownInlineEditor(ctx) {
...
test('beforeinput prevents native edits for selection partially outside inline root', async () => {
    ...
    try {
        ...
        await ctx.editor.eventService._onBeforeInput(event);
...
test('beforeinput does not intercept when selection spans another inline root', async () => {
    ...
    try {
        ...
        await ctx1.editor.eventService._onBeforeInput(event);
