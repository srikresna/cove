import { Bound, clamp, Vec, } from '@blocksuite/global/gfx';
...
export function getPostGestureRecoveryDelay({ isPanning, isZooming, fallbackDelayMs, }) {
 ...
 */
export function getEffectiveDpr(zoom, rawDpr = window.devicePixelRatio) {
...
export function clientToModelCoord(viewport, clientCoord) {
...
export class Viewport {
    ...
    constructor() {
        ...
        this._width = 0;
        this._zoom = 1.0;
        this.elementReady = new Subject();
        this.sizeUpdated = new Subject();
        this.resizeStarted = new Subject();
        this.viewportMoved = new Subject();
        this.viewportUpdated = new Subject();
        this.zoomUpdated = new Subject();
        this.zooming$ = new BehaviorSubject(false);
        this.panning$ = new BehaviorSubject(false);
        /**
         ...
         */
        this.VIEWPORT_REFRESH_PIXEL_THRESHOLD = viewportRuntimeConfig.VIEWPORT_REFRESH_PIXEL_THRESHOLD;
        /**
         * Maximum interval (ms) between viewport refreshes during continuous interaction.
         * Higher values reduce refresh frequency, lowering memory pressure on mobile.
         * Default: 120 (desktop-optimized).
         */
        this.VIEWPORT_REFRESH_MAX_INTERVAL = viewportRuntimeConfig.VIEWPORT_REFRESH_MAX_INTERVAL;
        /**
         ...
         */
        this.SKIP_REFRESH_DURING_GESTURE = viewportRuntimeConfig.SKIP_REFRESH_DURING_GESTURE;
        this.LOW_ZOOM_GESTURE_ACTIVE_BLOCK_LIMIT = viewportRuntimeConfig.LOW_ZOOM_GESTURE_ACTIVE_BLOCK_LIMIT;
        this.LOW_ZOOM_GESTURE_ACTIVE_DISTANCE_RATIO = viewportRuntimeConfig.LOW_ZOOM_GESTURE_ACTIVE_DISTANCE_RATIO;
        this._resetZooming = debounce(() => {
            this.zooming$.next(false);
        }, 200);
        this._resetPanning = debounce(() => {
    ...
    _setupResizeObserver() {
        this._resizeSubject
            ...
            if (!this._shell || !this._initialTopLeft)
                return;
            this._completeResize(width, height, left, top);
        });
    }
    _completeResize(width, height, left, top) {
        if (!this._initialTopLeft)
            return;
    ...
    _enlargeBounds(ratio) {
        ...
        return new Bound(bounds.x - marginX, bounds.y - marginY, bounds.w + marginX * 2, bounds.h + marginY * 2);
    }
    get viewportMaxXY() {
    ...
    getFitToScreenData(bounds, padding = [0, 0, 0, 0], maxZoom = this.ZOOM_MAX, fitToScreenPadding = 100) {
        ...
        zoom = Math.min((width - fitToScreenPadding - (pr + pl)) / w, (height - fitToScreenPadding - (pt + pb)) / h);
        zoom = clamp(zoom, this.ZOOM_MIN, clamp(maxZoom, this.ZOOM_MIN, this.ZOOM_MAX));
     ...
     */
    setCenter(centerX, centerY, forceUpdate = true, signalPanning = true) {
     ...
     * @param forceUpdate Whether to force complete any pending resize operations before setting the viewport.
     */
    setViewport(newZoom, newCenter = Vec.toVec(this.center), smooth = false, forceUpdate = true, signalGesture = false) {
     ...
     * @param forceUpdate whether to force complete any pending resize operations before setting the viewport
     */
    setViewportByBound(bound, padding = [0, 0, 0, 0], smooth = false, forceUpdate = true, signalGesture = false) {
     ...
     * @param forceUpdate Whether to force complete any pending resize operations before setting the viewport.
     */
    setZoom(zoom, focusPoint, _wheel = false, forceUpdate = true, signalGesture = false) {
        if (forceUpdate && this._isResizing) {
            this._forceCompleteResize();
        }
    ...
    smoothTranslate(x, y, numSteps = 10, signalGesture = false) {
        const { center } = this;
    ...
    smoothZoom(zoom, focusPoint, numSteps = 10, signalGesture = false) {
    ...
    toModelCoord(viewX, viewY, zoom = this.zoom, center) {
