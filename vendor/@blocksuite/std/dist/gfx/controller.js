...
export class GfxController extends LifeCycleWatcher {
    static { this.key = gfxControllerKey; }
    get grid() {
    ...
    constructor(std) {
        ...
        this._surface$ = new Signal(null);
        this.cursor$ = new Signal();
    ...
    fitToScreen(options = {
        ...
        const elemBounds = options.bounds ??
            this.gfxElements.map(element => Bound.deserialize(element.xywh));
        const commonBound = getCommonBound(elemBounds);
        const { zoom, centerX, centerY } = this.viewport.getFitToScreenData(commonBound, options.padding, ZOOM_INITIAL, FIT_TO_SCREEN_PADDING);
