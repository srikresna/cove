...
import { createInteractionContext } from './event.js';
import { InteractivityExtensionIdentifier, } from './extension/base.js';
import { GfxViewInteractionIdentifier, } from './extension/view.js';
import { GfxViewEventManager } from './gfx-view-event-handler.js';
import { DEFAULT_HANDLES, ResizeController, } from './resize/manager.js';
export const InteractivityIdentifier = GfxExtensionIdentifier('interactivity-manager');
const DRAG_MOVE_RAF_THRESHOLD = 100;
const DRAG_MOVE_HEAVY_COST_MS = 4;
const shouldAllowDragMoveCoalescing = (elements) => {
...
export class InteractivityManager extends GfxExtension {
    constructor() {
        ...
        this.activeInteraction$ = signal(null);
    }
    static { this.key = 'interactivity-manager'; }
    mounted() {
        this.canvasEventHandler = new GfxViewEventManager(this.gfx);
    ...
    dispatchEvent(eventName, evt) {
        ...
        extensions.forEach(ext => {
            ext.event.emit(eventName, context);
        });
    ...
    _getSelectionConfig(models) {
        ...
        models.forEach(model => {
            ...
            const selectionConfig = config?.handleSelection?.({
                gfx: this.gfx,
                std: this.std,
                view,
                model,
    ...
    _getSuggestedTarget(context) {
        ...
        extensions
            ...
            .toArray()
            .forEach(ext => {
            return ext.action.emit('elementSelect', {
                candidates,
                target,
                suggest,
            });
        ...
        if (suggestedElements.length) {
            ...
            const suggested = last(suggestedElements);
    ...
    handleElementSelection(evt) {
        ...
        candidates = candidates.filter(model => {
            ...
            return (selectionConfigs.has(model.id) &&
                selectionConfigs.get(model.id)?.handlers.selectable({
                    ...context,
                    view: config.view,
                    model,
                    default: config.defaultHandlers.selectable,
                }));
        });
        {
            ...
            const result = config.handlers.onSelect({
                ...
                view: config.view,
                model: target,
                default: config.defaultHandlers.onSelect,
            });
            return result ?? true;
        }
    }
    ...
    handleElementMove(options) {
        ...
        const context = {
            ...
            dragStartPos: Point.from(this.gfx.viewport.toModelCoordFromClientCoord([
                options.event.x,
                options.event.y,
            ])),
        };
        const extension = this.interactExtensions;
        const activeExtensionHandlers = Array.from(extension.values().map(ext => {
            return ext.action.emit('dragInitialize', context);
        }));
        if (cancelledByExt) {
        ...
        const canCoalesceDragMove = shouldAllowDragMoveCoalescing(internal.elements);
        let shouldCoalesceDragMove = canCoalesceDragMove &&
            internal.elements.length >= DRAG_MOVE_RAF_THRESHOLD;
        const applyDragMove = (event) => {
    ...
    handleElementRotate(options) {
        ...
        const elements = Array.from(viewConfigMap.values()).map(config => config.view.model);
        handler.startRotate({
            ...options,
            elements,
            onRotateStart: payload => {
                ...
                payload.data.forEach(({ model }) => {
                    ...
                    handlers.onRotateStart({
                        default: defaultHandlers.onRotateStart,
                        constraint,
                        model,
                        view,
                    });
            ...
            onRotateUpdate: payload => {
                ...
                payload.data.forEach(({ model, newBound, originalBound, newRotate, originalRotate, matrix, }) => {
                    if (!viewConfigMap.has(model.id)) {
                        return;
                    }
                    const { handlers, defaultHandlers, view, constraint } = viewConfigMap.get(model.id);
                    handlers.onRotateMove({
                        ...
                        newRotate,
                        originalRotate,
                        default: defaultHandlers.onRotateMove,
                        constraint,
                        view,
                        matrix,
                    });
                ...
                this.std.store.transact(() => {
                    payload.data.forEach(({ model }) => {
                        ...
                        handlers.onRotateEnd({
                            default: defaultHandlers.onRotateEnd,
                            view,
                            model,
                            constraint,
                        });
    ...
    _getViewResizeConfig(elements) {
        ...
        viewConfigMap.forEach(config => {
            const currConstraint = config.constraint;
        ...
        return {
            allowedHandlers: Array.from(allowedHandlers),
            viewConfigMap,
        };
    }
    ...
    handleElementResize(options) {
        ...
        const elements = Array.from(viewConfigMap.values()).map(config => config.view.model);
        const extensionHandlers = this.interactExtensions.values().reduce((handlers, ext) => {
            const extHandlers = ext.action.emit('elementResize', {
                elements,
            });
            if (extHandlers) {
        ...
        controller.startResize({
            ...
            lockRatio,
            elements,
            onResizeMove: ({ scaleX, scaleY, originalBound, handleSign, handlePos, currentHandlePos, lockRatio, }) => {
                const suggested = [];
                const suggest = (distance) => {
            ...
            onResizeStart: ({ handleSign, handlePos, data }) => {
                ...
                data.forEach(({ model }) => {
                    ...
                    handlers.onResizeStart({
                        handle,
                        default: defaultHandlers.onResizeStart,
                        constraint,
                        model,
                        view,
                    });
            ...
            onResizeUpdate: ({ data, scaleX, scaleY, lockRatio }) => {
                ...
                data.forEach(({ model, newBound, originalBound, lockRatio, matrix }) => {
                    ...
                    handlers.onResizeMove({
                        ...
                        originalBound,
                        handle,
                        default: defaultHandlers.onResizeMove,
                        constraint,
                        view,
                        lockRatio,
                        matrix,
                ...
                this.std.store.transact(() => {
                    data.forEach(({ model }) => {
                        ...
                        handlers.onResizeEnd({
                            default: defaultHandlers.onResizeEnd,
                            view,
                            model,
                            constraint,
                            handle,
    ...
    requestElementClone(options) {
        const extensions = this.interactExtensions;
        for (let ext of extensions.values()) {
            const cloneData = ext.action.emit('elementsClone', options);
            if (cloneData) {
                return cloneData;
            }
        }
