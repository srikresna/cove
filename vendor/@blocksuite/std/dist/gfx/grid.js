...
const typeFilters = {
    block: (model) => model instanceof GfxBlockElementModel,
    canvas: (model) => model instanceof GfxPrimitiveElementModel,
    local: (model) => model instanceof GfxLocalElementModel,
};
export class GridManager extends GfxExtension {
    ...
    _toFilterFunc(filters) {
        ...
        return (model) => filterFuncs.some(filter => filter(model));
    }
    add(element) {
    ...
    search(bound, options = {
        ...
        const filterFunc = (Array.isArray(options.filter)
            ? this._toFilterFunc(options.filter)
            : options.filter) ?? this._toFilterFunc(['canvas', 'block']);
        const results = this._searchExternal(bound, {
            filterFunc,
            strict,
        });
        for (let i = minRow; i <= maxRow; i++) {
    ...
    mounted() {
        ...
        const watchSurface = (surface) => {
            ...
            disposables.add(surface.childMap.subscribe(currentChildMap => {
            ...
            disposables.add(surface.elementAdded.subscribe(payload => {
                this.add(surface.getElementById(payload.id));
            }));
            disposables.add(surface.elementRemoved.subscribe(payload => {
                this.remove(payload.model);
            }));
            disposables.add(surface.elementUpdated.subscribe(payload => {
            ...
            disposables.add(surface.localElementAdded.subscribe(elm => {
                this.add(elm);
            }));
            disposables.add(surface.localElementUpdated.subscribe(payload => {
                if (payload.props['xywh'] || payload.props['responseExtension']) {
                    this.update(payload.model);
                }
            }));
            disposables.add(surface.localElementDeleted.subscribe(elm => {
                this.remove(elm);
            }));
        ...
        if (this.gfx.surface) {
            ...
            disposables.add(this.gfx.surface$.subscribe(surface => {
                if (surface) {
