...
const retainedShapeDom = new WeakMap();
function getRetainedShapeDom(element) {
...
function applyShapeSpecificStyles(model, element, zoom) {
    ...
    switch (model.shapeType) {
        case 'rect': {
            ...
            const borderRadius = r < 1 ? `${Math.min(w * r, h * r)}px` : `${r * zoom}px`;
...
function getOrCreateSvg(retained, element) {
...
function removeSvg(retained) {
    retained.svg?.remove();
    retained.svg = null;
    retained.polygon = null;
}
function getOrCreateText(retained, element) {
    if (retained.text) {
...
function removeText(retained) {
    retained.text?.remove();
    retained.text = null;
}
function applyBorderStyles(model, element, strokeColor, zoom) {
    element.style.border =
        model.strokeStyle !== 'none'
            ? `${model.strokeWidth * zoom}px ${model.strokeStyle === 'dash' ? 'dashed' : 'solid'} ${strokeColor}`
            : 'none';
}
function applyTransformStyles(model, element) {
...
function applyShadowStyles(model, element, renderer) {
    if (model.shadow) {
 ...
 */
export const shapeDomRenderer = (model, element, renderer) => {
    ...
    if (model.shapeType === 'diamond' || model.shapeType === 'triangle') {
        ...
        if (model.shapeType === 'diamond') {
            ...
            svgPoints = SVGShapeBuilder.diamond(unscaledWidth, unscaledHeight, strokeW);
        }
        else {
            // triangle - generate triangle points using shared utility
            svgPoints = SVGShapeBuilder.triangle(unscaledWidth, unscaledHeight, strokeW);
        }
        // Determine if stroke should be visible and its color
        const finalStrokeColor = model.strokeStyle !== 'none' && strokeW > 0 ? strokeColor : 'transparent';
        // Determine dash array, only if stroke is visible and style is 'dash'
        const finalStrokeDasharray = model.strokeStyle === 'dash' && finalStrokeColor !== 'transparent'
            ? '12, 12'
            : 'none';
    ...
    if (model.textDisplay && model.text) {
        ...
        textElement.style.color = renderer.getColorValue(model.color, DefaultTheme.shapeTextColor, true);
