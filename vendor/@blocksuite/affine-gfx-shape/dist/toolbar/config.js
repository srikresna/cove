...
import { DefaultTheme, FontFamily, getShapeName, getShapeRadius, getShapeType, isTransparent, LineWidth, MindmapElementModel, resolveColor, ShapeElementModel, ShapeStyle, ShapeType, StrokeStyle, } from '@blocksuite/affine-model';
    ...
    actions: [
        {
            ...
            content(ctx) {
                ...
                if (!models.length)
                    return null;
                const shapeStyle = ctx.features.getFlag('enable_edgeless_scribbled_style')
                    ? (getMostCommonValue(models, 'shapeStyle') ?? ShapeStyle.General)
                    : ShapeStyle.General;
                const shapeName = getMostCommonValue(models.map(model => ({
                    shapeName: getShapeName(model.shapeType, model.radius),
                })), 'shapeName') ?? ShapeType.Rect;
                const onPick = (shapeName) => {
                ...
                return renderMenu({
                    ...
                    items: ShapeComponentConfig.map(item => ({
                        key: item.tooltip,
                        value: item.name,
                        icon: shapeStyle === ShapeStyle.General
                            ? item.generalIcon
                            : item.scribbledIcon,
        ...
        },
        {
            ...
            content(ctx) {
                ...
                const shapeStyle = getMostCommonValue(models, field) ?? ShapeStyle.General;
                const onPick = (value) => {
        ...
        ...createTextActions(ShapeElementModel, 'shape', (ctx, model, props) => {
            ...
            const xywh = normalizeShapeBound(model, Bound.fromXYWH(model.deserializedXYWH)).serialize();
            ctx.std
                .get(EdgelessCRUDIdentifier)
...
function getTextColor(fillColor, isNotTransparent = false) {
    if (isNotTransparent) {
...
export function hasGrouped(model) {
