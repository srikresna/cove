...
import { BlockElementCommentManager, DocModeProvider, } from '@blocksuite/affine-shared/services';
...
let EmbedBlockComponent = (() => {
    ...
    return class EmbedBlockComponent extends _classSuper {
        constructor() {
            ...
            this.showOverlay$ = computed(() => this.isDraggingOnHost$.value ||
            ...
            this.renderEmbed = (content) => {
                if (this._cardStyle === 'horizontal' ||
                    ...
                    const insideNote = findAncestorModel(this.model, m => m.flavour === 'affine:note');
                    if (!insideNote &&
    ...
    `;
            ...
            this.#selectedStyle_accessor_storage = (__runInitializers(this, _embedBlock_extraInitializers), SelectedStyle.Border);
...
export const createEmbedEdgelessBlockInteraction = (flavour, config) => {
