...
import { BookmarkStyles, EmbedGithubModel, EmbedGithubStyles, isExternalEmbedModel, } from '@blocksuite/affine-model';
import { EMBED_CARD_HEIGHT, EMBED_CARD_WIDTH, } from '@blocksuite/affine-shared/consts';
import { ActionPlacement, EmbedOptionProvider, ToolbarModuleExtension, } from '@blocksuite/affine-shared/services';
...
const previewAction = {
    ...
    content(ctx) {
        ...
        if (!model || !isExternalEmbedModel(model))
            return null;
...
function createBuiltinToolbarConfigForExternal(klass) {
    return {
        actions: [
            previewAction,
            {
                ...
                actions: [
                    ...
                    },
                    {
                        ...
                        run(ctx) {
                            ...
                            const blockId = ctx.store.addBlock(flavour, { url, caption, style }, parent, index);
                            ctx.store.deleteBlock(model);
                            // Selects new block
                            ctx.select('note', [
                ...
                content(ctx) {
                    ...
                    if (!model || !isExternalEmbedModel(model))
                        return null;
                    const { url } = model.props;
                    const viewType = ctx.std.get(EmbedOptionProvider).getEmbedBlockOptions(url)
                        ?.viewType ?? 'card';
                    const actions = this.actions.map(action => ({ ...action }));
                    const viewType$ = signal(`${viewType === 'card' ? 'Card' : 'Embed'} view`);
...
const createBuiltinSurfaceToolbarConfigForExternal = (klass) => {
    return {
        actions: [
            previewAction,
            {
                ...
                actions: [
                    {
                        ...
                        run(ctx) {
                            ...
                            const newId = ctx.store.addBlock(flavour, { url, caption, style, xywh: bounds.serialize() }, parent);
...
export const createBuiltinToolbarConfigExtension = (flavour, klass) => {
