...
import { BookmarkStyles, EmbedIframeBlockModel, } from '@blocksuite/affine-model';
...
import { CaptionIcon, CopyIcon, DeleteIcon, DuplicateIcon, LinkedPageIcon, OpenInNewIcon, ResetIcon, } from '@blocksuite/icons/lit';
import { BlockFlavourIdentifier, BlockSelection } from '@blocksuite/std';
import { Slice, Text, toDraftModel, } from '@blocksuite/store';
import { computed, signal } from '@preact/signals-core';
import { html } from 'lit';
import { keyed } from 'lit/directives/keyed.js';
import * as Y from 'yjs';
import { convertSelectedBlocksToLinkedDoc, getTitleFromSelectedModels, notifyDocCreated, promptDocTitle, } from '../../common/render-linked-doc';
import { EmbedIframeBlockComponent } from '../embed-iframe-block';
const trackBaseProps = {
    category: 'embed iframe block',
};
const showWhenUrlExists = (ctx) => {
    const model = ctx.getCurrentModelByType(EmbedIframeBlockModel);
    if (!model)
        return false;
    return !!model.props.url;
};
const openLinkAction = (id) => {
    return {
        id,
            ...
            actions: [
                ...
                },
                {
                    ...
                    run(ctx) {
                        ...
                        const style = BookmarkStyles.find(s => s !== 'vertical' && s !== 'cube') ??
                            BookmarkStyles[1];
