...
const GOOGLE_DRIVE_EMBED_FOLDER_URL = 'https://drive.google.com/embeddedfolderview';
const GOOGLE_DRIVE_EMBED_FILE_URL = 'https://drive.google.com/file/d/';
const googleDriveUrlValidationOptions = {
    protocols: ['https:'],
 ...
 */
function hasValidSharingParam(parsedUrl) {
 ...
 */
function isValidGoogleDriveFileUrl(parsedUrl) {
    const pathSegments = parsedUrl.pathname.split('/').filter(Boolean);
    return (pathSegments[0] === 'file' &&
        pathSegments[1] === 'd' &&
 ...
 */
function isValidGoogleDriveFolderUrl(parsedUrl) {
    const pathSegments = parsedUrl.pathname.split('/').filter(Boolean);
    return (pathSegments[0] === 'drive' &&
 ...
 */
function isValidGoogleDrivePathUrl(parsedUrl) {
    const pathSegments = parsedUrl.pathname.split('/').filter(Boolean);
 ...
 */
function isValidGoogleDriveUrl(url, strictMode = true) {
    try {
...
function isValidGoogleDriveIframeUrl(url) {
    try {
        if (!validateEmbedIframeUrl(url, googleDriveUrlValidationOptions)) {
            return false;
        }
        ...
        return (parsedUrl.pathname === '/embeddedfolderview' &&
 ...
 * @returns Embed URL
 */
function buildGoogleDriveFileEmbedUrl(fileId) {
    const embedUrl = new URL('preview', `${GOOGLE_DRIVE_EMBED_FILE_URL}${fileId}/`);
 ...
 * @returns Embed URL
 */
function buildGoogleDriveFolderEmbedUrl(folderId) {
 ...
 */
function buildGoogleDriveEmbedUrl(url) {
    try {
        const parsedUrl = new URL(url);
        const pathSegments = parsedUrl.pathname.split('/').filter(Boolean);
        // Should have at least 2 segments
...
export const googleDriveConfig = {
    name: 'google-drive',
    match: (url) => isValidGoogleDriveUrl(url),
    buildOEmbedUrl: (url) => {
    ...
    validateIframeUrl: (iframeUrl) => isValidGoogleDriveIframeUrl(iframeUrl),
    options: {
...
export const GoogleDriveEmbedConfig = EmbedIframeConfigExtension(googleDriveConfig);
