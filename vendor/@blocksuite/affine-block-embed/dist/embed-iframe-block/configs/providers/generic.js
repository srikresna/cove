...
const genericUrlValidationOptions = {
    protocols: ['https:'],
    hostnames: [],
};
/**
 ...
 */
function isValidGenericEmbedUrl(url) {
    try {
        const parsedUrl = new URL(url);
        if (!validateEmbedIframeUrl(url, {
        ...
        if (AFFINE_DOMAINS.some(domain => hostname === domain || hostname.endsWith(`.${domain}`))) {
            return false;
        }
        return true;
    }
...
export const genericConfig = {
    name: 'generic',
    match: (url) => isValidGenericEmbedUrl(url),
    buildOEmbedUrl: (url) => {
    ...
    validateIframeUrl: (iframeUrl) => isValidGenericEmbedUrl(iframeUrl),
    options: {
