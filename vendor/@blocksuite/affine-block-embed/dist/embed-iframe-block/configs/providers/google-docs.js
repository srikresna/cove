...
const googleDocsUrlValidationOptions = {
    protocols: ['https:'],
 ...
 */
function isValidGoogleDocsUrl(url, strictMode = true) {
    try {
        ...
        return (pathSegments[0] === 'document' &&
            pathSegments[1] === 'd' &&
...
export const googleDocsConfig = {
    name: 'google-docs',
    match: (url) => isValidGoogleDocsUrl(url),
    buildOEmbedUrl: (url) => {
    ...
    validateIframeUrl: (iframeUrl) => isValidGoogleDocsUrl(iframeUrl),
    options: {
...
export const GoogleDocsEmbedConfig = EmbedIframeConfigExtension(googleDocsConfig);
