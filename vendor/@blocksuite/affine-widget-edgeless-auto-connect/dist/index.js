...
import { EdgelessCRUDIdentifier, EdgelessLegacySlotIdentifier, isNoteBlock, } from '@blocksuite/affine-block-surface';
import { SmallDocIcon } from '@blocksuite/affine-components/icons';
import { FrameBlockModel, NoteBlockModel, NoteDisplayMode, SurfaceRefBlockModel, } from '@blocksuite/affine-model';
import { FeatureFlagService } from '@blocksuite/affine-shared/services';
import { matchModels, stopPropagation } from '@blocksuite/affine-shared/utils';
import { Bound } from '@blocksuite/global/gfx';
import { ArrowLeftSmallIcon, ArrowRightSmallIcon, InvisibleIcon, } from '@blocksuite/icons/lit';
...
let EdgelessAutoConnectWidget = (() => {
  ...
  `; }
        ...
        _initLabels() {
            ...
            const getVisibility = () => {
                ...
                if (selectedElements.length === 1 &&
                    ...
                    (isNoteBlock(selectedElements[0]) ||
                        surfaceRefs.some(ref => ref.props.reference === selectedElements[0].id))) {
                    this._show = true;
                }
                else {
                    this._show = false;
            ...
            this._disposables.add(this._selection.slots.updated.subscribe(() => {
                getVisibility();
            }));
            this._disposables.add(this.store.slots.blockUpdated.subscribe(payload => {
            ...
            if (surface) {
                this._disposables.add(surface.elementUpdated.subscribe(payload => {
      ...
      >
        ...
        firstUpdated() {
            ...
            _disposables.add(gfx.viewport.viewportUpdated.subscribe(() => {
                this.requestUpdate();
            }));
            _disposables.add(gfx.selection.slots.updated.subscribe(() => {
                const { selectedElements } = gfx.selection;
                if (!(selectedElements.length === 1 && isNoteBlock(selectedElements[0]))) {
                    this._index = -1;
                }
            }));
            _disposables.add(std.event.add('dragStart', () => {
                this._dragging = true;
            }));
            _disposables.add(std.event.add('dragEnd', () => {
                this._dragging = false;
            }));
            _disposables.add(slots.elementResizeStart.subscribe(() => {
                this._dragging = true;
            }));
            _disposables.add(slots.elementResizeEnd.subscribe(() => {
                this._dragging = false;
            }));
        }
        render() {
    ...
    ${this._index >= 0 && this._index < elements.length
        ...
        constructor() {
            ...
            this._updateLabels = () => {
                ...
                const notes = root.children.filter(child => matchModels(child, [NoteBlockModel]));
                ...
                notes.forEach(note => {
                    if (isNoteBlock(note)) {
                        ...
                        else if (note.props.displayMode$.value === NoteDisplayMode.DocAndEdgeless) {
...
export const autoConnectWidget = WidgetViewExtension('affine:page', AFFINE_EDGELESS_AUTO_CONNECT_WIDGET, literal `${unsafeStatic(AFFINE_EDGELESS_AUTO_CONNECT_WIDGET)}`);
