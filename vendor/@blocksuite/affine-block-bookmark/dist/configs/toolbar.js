import { canEmbedAsEmbedBlock, canEmbedAsIframe, EMBED_IFRAME_DEFAULT_HEIGHT_IN_SURFACE, EMBED_IFRAME_DEFAULT_WIDTH_IN_SURFACE, } from '@blocksuite/affine-block-embed';
import { reassociateConnectorsCommand } from '@blocksuite/affine-block-surface';
import { toast } from '@blocksuite/affine-components/toast';
import { BookmarkBlockModel, BookmarkStyles, } from '@blocksuite/affine-model';
import { EMBED_CARD_HEIGHT, EMBED_CARD_WIDTH, } from '@blocksuite/affine-shared/consts';
import { ActionPlacement, EmbedIframeService, EmbedOptionProvider, ToolbarModuleExtension, } from '@blocksuite/affine-shared/services';
import { getBlockProps } from '@blocksuite/affine-shared/utils';
import { Bound } from '@blocksuite/global/gfx';
import { CaptionIcon, CopyIcon, DeleteIcon, DuplicateIcon, ResetIcon, } from '@blocksuite/icons/lit';
...
const createOnToggleFn = (ctx, name, control) => (e) => {
    e.stopPropagation();
    ...
    actions: [
        previewAction,
        {
            ...
            actions: [
                ...
                },
                {
                    ...
                    disabled(ctx) {
                        ...
                        return (!canEmbedAsIframe(ctx.std, url) &&
                    ...
                    run(ctx) {
                        ...
                        if (canEmbedAsEmbedBlock(ctx.std, url)) {
                            ...
                            blockId = ctx.store.addBlock(flavour, {
                                url,
                                caption,
                                title,
                                description,
                            ...
                            blockId = embedIframeService.addEmbedIframeBlock({ url, caption, title, description }, parent.id, index);
                        }
                        if (!blockId)
                            return;
                        ctx.store.deleteBlock(model);
            ...
            content(ctx) {
                ...
                return html `${keyed(model, html `<affine-view-dropdown-menu
            @toggle=${onToggle}
            .actions=${actions}
            .context=${ctx}
        ...
        },
        {
            ...
            content(ctx) {
                ...
                return html `${keyed(model, html `<affine-card-style-dropdown-menu
            @toggle=${onToggle}
            .actions=${actions}
            .context=${ctx}
        ...
        previewAction,
        {
                    ...
                    run(ctx) {
                        ...
                        if (canEmbedAsEmbedBlock(ctx.std, url)) {
                            const options = ctx.std
                                .get(EmbedOptionProvider)
                                .getEmbedBlockOptions(url);
                            if (options?.viewType !== 'embed')
                                return;
                            const { flavour, styles } = options;
                            let style = model.props.style;
                            ...
                            newId = ctx.store.addBlock(flavour, {
                                url,
                                caption,
                                title,
                                description,
                            ...
                            newId = ctx.store.addBlock('affine:embed-iframe', { url, caption, title, description, xywh: bound.serialize() }, parent);
                        }
                        if (!newId)
                            return;
            ...
            when(ctx) {
                ...
                if (!model)
                    return false;
                const { url } = model.props;
                return (canEmbedAsIframe(ctx.std, url) || canEmbedAsEmbedBlock(ctx.std, url));
            },
            content(ctx) {
                const model = ctx.getCurrentModelByType(BookmarkBlockModel);
                if (!model)
        ...
        },
        {
            ...
            content(ctx) {
                ...
                const scale$ = computed(() => {
                    const { xywh$: { value: xywh }, } = model;
                    const { style$: { value: style }, } = model.props;
                ...
                return html `${keyed(model, html `<affine-size-dropdown-menu
            @select=${onSelect}
            @toggle=${onToggle}
            .format=${format}
            .sizeSignal=${scale$}
...
export const createBuiltinToolbarConfigExtension = (flavour) => {
