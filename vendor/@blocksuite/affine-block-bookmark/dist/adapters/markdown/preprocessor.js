...
function formatFootnoteDefinition(reference, data) {
...
class FootnoteParser {
        ...
        for (const token of tokens) {
            switch (token.type) {
                ...
                case 'JSON_CONTENT':
                    if (isParsingFootnote) {
                        try {
                            currentFootnote.content = FootNoteReferenceParamsSchema.parse(JSON.parse(token.value));
    ...
    process(input) {
        ...
        const footnoteMap = new Map(processedFootnotes.map(fn => [
            fn.reference,
 ...
 */
export function footnoteUrlPreprocessor(content) {
    const parser = new FootnoteParser();
    return parser.process(content);
}
const bookmarkBlockPreprocessor = {
    name: 'bookmark-block',
    levels: ['block', 'slice', 'doc'],
    preprocess: footnoteUrlPreprocessor,
};
export const BookmarkBlockMarkdownPreprocessorExtension = MarkdownPreprocessorExtension(bookmarkBlockPreprocessor);
