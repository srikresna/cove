...
import { EDGELESS_TEXT_BLOCK_MIN_HEIGHT, EDGELESS_TEXT_BLOCK_MIN_WIDTH, EdgelessTextBlockModel, } from '@blocksuite/affine-model';
import { focusTextModel } from '@blocksuite/affine-rich-text';
import { matchModels } from '@blocksuite/affine-shared/utils';
import { Bound } from '@blocksuite/global/gfx';
import { GfxControllerIdentifier } from '@blocksuite/std/gfx';
export const insertEdgelessTextCommand = (ctx, next) => {
    const { std, x, y } = ctx;
    const host = std.host;
    ...
    const textId = std.get(EdgelessCRUDIdentifier).addBlock('affine:edgeless-text', {
    ...
    host.updateComplete
        ...
        host.updateComplete
            ...
            edgelessText.addEventListener('focusout', e => {
                if (edgelessText.model.children.length > 1)
                    return;
            ...
            edgelessText.addEventListener('beforeinput', () => {
                abortController.abort();
            }, {
                once: true,
            });
