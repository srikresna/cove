...
import { convert, field, GfxGroupLikeElementModel, observe, watch, } from '@blocksuite/std/gfx';
...
let MindmapElementModel = (() => {
    ...
    return class MindmapElementModel extends _classSuper {
        ...
        parent, sibling, position = 'after', props = {}) {
            ...
            props['text'] = new Y.Text(props['text'] ?? 'New node');
        ...
        stashTree(node) {
            ...
            const stashedNodeIds = new Set();
            const traverse = (node) => {
                if (this._stashedNode.has(node.id))
                    return;
        ...
        constructor() {
            ...
            this.#children_accessor_storage = __runInitializers(this, _children_initializers, new Y.Map());
