...
import { ActionPlacement, EmbedIframeService, EmbedOptionProvider, } from '@blocksuite/affine-shared/services';
import { CopyIcon, DeleteIcon, EditIcon, UnlinkIcon, } from '@blocksuite/icons/lit';
    ...
    actions: [
        {
            ...
            content(cx) {
                ...
                if (!(target instanceof AffineLink))
                    return null;
                const { link } = target;
                if (!link)
                    return null;
            ...
            actions: [
                ...
                },
                {
                    ...
                    run(ctx) {
                        ...
                        const popover = toggleLinkPopup(ctx.std, 'edit', inlineEditor, selfInlineRange, abortController);
            ...
            actions: [
                ...
                },
                {
                    ...
                    run(ctx) {
                        ...
                        if (!url)
                            return;
                        const { block: { model }, inlineEditor, selfInlineRange, } = target;
                        const { parent } = model;
                        if (!inlineEditor || !selfInlineRange || !parent)
                            return;
                        // Clears
                        ctx.reset();
                        const title = inlineEditor.yTextString.slice(selfInlineRange.index, selfInlineRange.index + selfInlineRange.length);
                        const options = ctx.std
                            .get(EmbedOptionProvider)
                            .getEmbedBlockOptions(url);
                        const flavour = options?.viewType === 'card'
                            ? options.flavour
                            : 'affine:bookmark';
          ...
          ></affine-view-dropdown-menu>`)}`;
            ...
            when(ctx) {
                ...
                if (target.block.closest('affine-database') ||
                    ...
                    return false;
                if (!target.link.startsWith('http'))
                    return false;
