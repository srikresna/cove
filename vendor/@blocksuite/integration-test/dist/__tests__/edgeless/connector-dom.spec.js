...
function hasConnectorPath(surfaceView, connectorId) {
    const connector = surfaceView.model.getElementById(connectorId);
    if (!connector || !('path' in connector))
        return false;
    const { path } = connector;
    return Array.isArray(path) && path.length >= 2;
}
async function waitForConnectorElement(surfaceView, connectorId, timeout = 5000) {
    ...
    while (Date.now() - startedAt < timeout) {
        ...
        const connectorElement = surfaceView.renderRoot.querySelector(`[data-element-id="${connectorId}"]`);
        if (connectorElement)
...
async function waitForConnectorElementRemoval(surfaceView, connectorId, timeout = 5000) {
    const startedAt = Date.now();
    while (Date.now() - startedAt < timeout) {
        const connectorElement = surfaceView.renderRoot.querySelector(`[data-element-id="${connectorId}"]`);
        if (!connectorElement)
...
describe('Connector rendering with DOM renderer', () => {
    ...
    test('should render a connector element as a DOM node', async () => {
        ...
        const connectorElement = await waitForConnectorElement(surfaceView, connectorId);
    ...
    test('should remove connector DOM node when element is deleted', async () => {
        ...
        const connectorId = surfaceModel.addElement(connectorProps);
        let connectorElement = await waitForConnectorElement(surfaceView, connectorId);
        expect(connectorElement).not.toBeNull();
        surfaceModel.deleteElement(connectorId);
        const removed = await waitForConnectorElementRemoval(surfaceView, connectorId);
