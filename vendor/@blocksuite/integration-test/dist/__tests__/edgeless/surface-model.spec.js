...
beforeEach(async () => {
    ...
    const models = doc.getModelsByFlavour('affine:surface');
    model = models[0];
    return cleanup;
});
describe('group', () => {
...
describe('connector', () => {
    test('should get connector', () => {
        ...
        expect(connector.forceFullRender).toBe(true);
    });
    test('multiple connectors are supported', () => {
        ...
        expect(connector.forceFullRender).toBe(true);
        expect(connector2.forceFullRender).toBe(true);
    });
    test('should return null if connector are updated', () => {
        ...
        expect(connectorBeforeUpdate.forceFullRender).toBe(true);
    ...
    test('getConnectors should purge stale connector ids from endpoint cache', () => {
        ...
        surfaceModel._connectorIdsByEndpoint.set(shapeId, new Set(['missing-connector-id']));
        surfaceModel._connectorEndpoints.set('missing-connector-id', {
            sourceId: shapeId,
            targetId: null,
        });
        expect(model.getConnectors(shapeId)).toEqual([]);
        expect(surfaceModel._connectorIdsByEndpoint
            .get(shapeId)
            ?.has('missing-connector-id') ?? false).toBe(false);
        expect(surfaceModel._connectorEndpoints.has('missing-connector-id')).toBe(false);
    });
    test('should return null if connector are deleted', async () => {
        ...
        expect(connectorBeforeDelete.forceFullRender).toBe(true);
...
describe('brush', () => {
    test('same lineWidth should have same xywh', () => {
        ...
        const brush = model.getElementById(id);
