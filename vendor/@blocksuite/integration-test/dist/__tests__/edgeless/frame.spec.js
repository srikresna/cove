...
describe('frame', () => {
    ...
    test('frame should have title', async () => {
        ...
        await wait();
        const getFrameTitle = (frameId) => {
            const frameTitleWidget = service.std.view.getWidget('affine-frame-title-widget', frameId);
        ...
        const [nestedTitleX, nestedTitleY] = service.viewport.toModelCoordFromClientCoord([
            nestedTitleRect.x,
            nestedTitleRect.y,
        ]);
    ...
    test('frame should have externalXYWH after moving viewport to contains frame', async () => {
        const frameId = service.doc.addBlock('affine:frame', {
    ...
    test('descendant of frame should not contain itself', async () => {
        const frameIds = [1, 2, 3].map(i => {
            return service.doc.addBlock('affine:frame', {
                xywh: '[0,0,300,300]',
                title: new Text(`Frame ${i}`),
            }, service.surface.id);
        });
        await wait();
        const frames = frameIds.map(id => service.doc.getBlock(id)?.model);
