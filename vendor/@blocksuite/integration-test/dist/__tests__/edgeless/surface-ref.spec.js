...
describe('basic', () => {
    ...
    beforeEach(async () => {
        ...
        frameId = service.crud.addBlock('affine:frame', {
            xywh: '[0, 0, 800, 200]',
            index: service.generateIndex(),
        }, service.surface.id);
        return cleanup;
    ...
    test('content in frame should be rendered in the correct order', async () => {
        ...
        const refBlocks = Array.from(surfaceRef.querySelectorAll('affine-edgeless-note'));
        const stackingCanvas = Array.from(surfaceRef.querySelectorAll('.indexable-canvas'));
    ...
    test('frame should be rendered in surface-ref viewport', async () => {
        ...
        const frame = surfaceRef.querySelector('affine-frame');
        expect(edgeless.viewport.isInViewport(frame.model.elementBound)).toBeTruthy();
    });
...
describe('clipboard', () => {
    test('import surface-ref snapshot should render content correctly', async () => {
        ...
        const newDoc = await importFromSnapshot(doc.workspace, snapshot);
        expect(newDoc).toBeTruthy();
        editor.doc = newDoc;
        await wait();
        const surfaceRefs = newDoc.getBlocksByFlavour('affine:surface-ref');
        expect(surfaceRefs).toHaveLength(2);
        const surfaceRefBlocks = surfaceRefs.map(({ id }) => editor.std.view.getBlock(id));
