...
describe('mindmap', () => {
    ...
    test('should update mindmap node size during IME composition', async () => {
        ...
        const mindmap = () => gfx.getElementById(mindmapId);
        ...
        await wait();
        const shapeEditor = root.querySelector('edgeless-shape-text-editor');
        ...
        updateSpy.mockClear();
        const preedit = shapeEditor.inlineEditorContainer.querySelector('[data-v-text="true"]');
        expect(preedit).toBeTruthy();
        shapeEditor.inlineEditorContainer?.dispatchEvent(new CompositionEvent('compositionstart', {
            data: '',
            bubbles: true,
        }));
        preedit.textContent =
        ...
        const compositionWidths = updateSpy.mock.calls
            .filter((call) => call[0] === rootNode.id &&
        ...
        expect(Math.max(...compositionHeights)).toBeLessThan(exaggeratedScrollHeight);
    ...
    test('mindmap collapse and expand should work correctly', async () => {
        ...
        const mindmap = () => gfx.getElementById(mindmapId);
        const mindmapView = () => gfx.view.get(mindmapId);
        doc.captureSync();
        await wait(100);
        ...
        {
            const childButton = mindmapView().getCollapseButton(mindmap().getNodeByPath([0, 2]));
            // collapse child node
            moveAndClick(gfx.viewport.toViewBound(childButton.elementBound));
            await wait(500);
            doc.captureSync();
