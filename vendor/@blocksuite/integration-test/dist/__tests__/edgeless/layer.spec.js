...
test('change element should update layer automatically', async () => {
    ...
    service.crud.updateElement(id, {
        index: service.layer.getReorderedIndex(service.crud.getElementById(id), 'forward'),
    });
    expect(service.layer.layers[service.layer.layers.length - 1].type).toBe('block');
    service.crud.updateElement(canvasElId, {
        index: service.layer.getReorderedIndex(service.crud.getElementById(canvasElId), 'forward'),
    });
...
test('blocks should rerender when their z-index changed', async () => {
    ...
    const assertBlocksContent = () => {
        const blocks = Array.from(document.querySelectorAll('affine-edgeless-root gfx-viewport > [data-block-id]'));
    ...
    service.crud.addElement('shape', {
        shapeType: 'rect',
        index: generateKeyBetween(service.crud.getElementById(blocks[1]).index, service.crud.getElementById(blocks[2]).index),
    });
    await wait();
    assertBlocksContent();
});
test('block host z-index should update after reordering', async () => {
    ...
    await wait();
    const getBlockHost = (id) => document.querySelector(`affine-edgeless-root gfx-viewport > [data-block-id="${id}"]`);
    ...
    expect(Number(backHost.style.zIndex)).toBeLessThan(Number(frontHost.style.zIndex));
    service.crud.updateElement(backId, {
        index: service.layer.getReorderedIndex(service.crud.getElementById(backId), 'front'),
    });
    await wait();
    expect(Number(backHost.style.zIndex)).toBeGreaterThan(Number(frontHost.style.zIndex));
});
describe('layer reorder functionality', () => {
    ...
    test('forward', async () => {
        service.crud.updateElement(ids[0], {
            index: service.layer.getReorderedIndex(service.crud.getElementById(ids[0]), 'forward'),
        });
        expect(service.layer.layers.findIndex(layer => layer.set.has(service.crud.getElementById(ids[0])))).toBe(1);
        expect(service.layer.layers.findIndex(layer => layer.set.has(service.crud.getElementById(ids[1])))).toBe(0);
        await wait();
        service.crud.updateElement(ids[1], {
            index: service.layer.getReorderedIndex(service.crud.getElementById(ids[1]), 'forward'),
        });
        expect(service.layer.layers.findIndex(layer => layer.set.has(service.crud.getElementById(ids[0])))).toBe(0);
        expect(service.layer.layers.findIndex(layer => layer.set.has(service.crud.getElementById(ids[1])))).toBe(1);
    });
    test('front', async () => {
        service.crud.updateElement(ids[0], {
            index: service.layer.getReorderedIndex(service.crud.getElementById(ids[0]), 'front'),
        });
        await wait();
        expect(service.layer.layers.findIndex(layer => layer.set.has(service.crud.getElementById(ids[0])))).toBe(3);
        service.crud.updateElement(ids[1], {
            index: service.layer.getReorderedIndex(service.crud.getElementById(ids[1]), 'front'),
        });
        expect(service.layer.layers.findIndex(layer => layer.set.has(service.crud.getElementById(ids[1])))).toBe(3);
    });
    test('backward', async () => {
        service.crud.updateElement(ids[3], {
            index: service.layer.getReorderedIndex(service.crud.getElementById(ids[3]), 'backward'),
        });
        expect(service.layer.layers.findIndex(layer => layer.set.has(service.crud.getElementById(ids[3])))).toBe(1);
        expect(service.layer.layers.findIndex(layer => layer.set.has(service.crud.getElementById(ids[2])))).toBe(2);
        await wait();
        service.crud.updateElement(ids[2], {
            index: service.layer.getReorderedIndex(service.crud.getElementById(ids[2]), 'backward'),
        });
        expect(service.layer.layers.findIndex(layer => layer.set.has(service.crud.getElementById(ids[3])))).toBe(3);
        expect(service.layer.layers.findIndex(layer => layer.set.has(service.crud.getElementById(ids[2])))).toBe(2);
    });
    test('back', async () => {
        service.crud.updateElement(ids[3], {
            index: service.layer.getReorderedIndex(service.crud.getElementById(ids[3]), 'back'),
        });
        expect(service.layer.layers.findIndex(layer => layer.set.has(service.crud.getElementById(ids[3])))).toBe(0);
        await wait();
        service.crud.updateElement(ids[2], {
            index: service.layer.getReorderedIndex(service.crud.getElementById(ids[2]), 'back'),
        });
        expect(service.layer.layers.findIndex(layer => layer.set.has(service.crud.getElementById(ids[2])))).toBe(0);
    });
});
describe('group related functionality', () => {
    const createGroup = (service, childIds) => {
    ...
    test("new added group should effect it children's layer", async () => {
        ...
        await wait(0);
        expect(edgeless.querySelectorAll('.indexable-canvas').length).toBe(2);
        Array.from(edgeless.querySelectorAll('.indexable-canvas')).forEach(canvas => {
            const rect = canvas.getBoundingClientRect();
            expect(rect.width).toBeGreaterThan(0);
            expect(rect.height).toBeGreaterThan(0);
        });
        createGroup(service, elements.filter((_, idx) => idx !== 1 && idx !== 3));
        expect(service.layer.layers.length).toBe(2);
        expect(service.layer.layers[0].type).toBe('block');
        expect(service.layer.layers[0].set.size).toBe(2);
        expect(service.layer.layers[1].type).toBe('canvas');
        expect(service.layer.layers[1].set.size).toBe(4);
        expect(edgeless.querySelectorAll('.indexable-canvas').length).toBe(0);
        const topCanvas = edgeless.querySelector('affine-surface canvas');
        expect(Number(edgeless.querySelector(`[data-block-id="${elements[1]}"]`).style.zIndex)).toBeLessThan(Number(topCanvas.style.zIndex));
        expect(Number(edgeless.querySelector(`[data-block-id="${elements[3]}"]`).style.zIndex)).toBeLessThan(Number(topCanvas.style.zIndex));
    });
    test("change group index should update its children's layer", () => {
        ...
        const groupId = createGroup(service, elements.filter((_, idx) => idx !== 1 && idx !== 3));
    ...
    test("change mindmap index should update its nodes' layer", async () => {
        ...
        expect(service.layer.getZIndex(root)).toBeGreaterThan(service.layer.getZIndex(note));
        mindmap.index = service.layer.getReorderedIndex(mindmap, 'back');
        await wait();
        expect(service.layer.getZIndex(root)).toBeLessThan(service.layer.getZIndex(note));
    });
    test('should keep relative index order of elements after group, ungroup, undo, redo', () => {
        ...
        service.std.command.exec(ungroupCommand, {
            group: service.crud.getElementById(groupId),
        });
        expect(isKeptRelativeOrder()).toBeTruthy();
        service.doc.undo();
        expect(isKeptRelativeOrder()).toBeTruthy();
...
describe('compare function', () => {
    ...
    test('compare two different elements', () => {
        ...
        const note1 = service.crud.getElementById(note1Id);
        const note2Id = addNote(doc);
        const note2 = service.crud.getElementById(note2Id);
    ...
    test('compare two nested elements', () => {
        ...
        expect(service.layer.compare(groupAShape, groupBShape)).toBe(SORT_ORDER.BEFORE);
        expect(service.layer.compare(groupBShape, groupAShape)).toBe(SORT_ORDER.AFTER);
        expect(service.layer.compare(groupANote, groupBNote)).toBe(SORT_ORDER.BEFORE);
        expect(service.layer.compare(groupBNote, groupANote)).toBe(SORT_ORDER.AFTER);
        ...
        expect(service.layer.compare(groupAShape, groupBShape)).toBe(SORT_ORDER.AFTER);
        expect(service.layer.compare(groupBShape, groupAShape)).toBe(SORT_ORDER.BEFORE);
        expect(service.layer.compare(groupANote, groupBNote)).toBe(SORT_ORDER.AFTER);
        expect(service.layer.compare(groupBNote, groupANote)).toBe(SORT_ORDER.BEFORE);
...
test('indexed canvas should be inserted into edgeless portal when switch to edgeless mode', async () => {
    ...
    const indexedCanvas = edgeless.querySelectorAll('.indexable-canvas')[0];
    expect(indexedCanvas.width).toBeLessThanOrEqual(surface.renderer.canvas.width);
    expect(indexedCanvas.height).toBeLessThanOrEqual(surface.renderer.canvas.height);
...
test('the actual rendering z-index should satisfy the logic order of their indexes', async () => {
    ...
    await wait(500);
    const edgeless = getDocRootBlock(doc, editor, 'edgeless');
    const blocks = Array.from(edgeless.querySelectorAll('gfx-viewport > [data-block-id]'));
    ...
    blocks
        ...
        const nextModel = blocks[index + 1].model;
        const zIndex = Number(block.style.zIndex);
        const nextZIndex = Number(blocks[index + 1].style.zIndex);
        expect(model.props.index <= nextModel.props.index).equals(zIndex <= nextZIndex);
    });
});
describe('index generator', () => {
    ...
    beforeEach(() => {
        ...
        const noteId = addNote(doc);
        preinsertedShape = service.crud.getElementById(shapeId);
        preinsertedNote = service.crud.getElementById(noteId);
    });
