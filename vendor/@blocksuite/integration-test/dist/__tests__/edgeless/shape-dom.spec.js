...
function expectPxCloseTo(value, expected, precision = 2) {
    expect(Number.parseFloat(value)).toBeCloseTo(expected, precision);
}
async function waitForShapeElement(surfaceView, shapeId, timeout = 5000) {
    ...
    while (Date.now() - startedAt < timeout) {
        ...
        const shapeElement = surfaceView.renderRoot.querySelector(`[data-element-id="${shapeId}"]`);
        if (shapeElement)
            return shapeElement;
        await wait(50);
    }
...
describe('Shape rendering with DOM renderer', () => {
    ...
    test('should remove shape DOM node when element is deleted', async () => {
        ...
        await wait(100);
        shapeElement = surfaceView.renderRoot.querySelector(`[data-element-id="${shapeId}"]`);
