...
const painterWorkers = new Set();
export function getRenderer() {
    return editor.std.get(ViewportTurboRendererIdentifier);
}
...
async function createEditor(collection, mode = 'page', extensions = []) {
...
export function createPainterWorker() {
    const worker = new Worker(new URL('./turbo-painter.worker.ts', import.meta.url), {
        type: 'module',
    });
    painterWorkers.add(worker);
    return worker;
}
export async function setupEditor(mode = 'page', extensionsInput, optionsInput) {
