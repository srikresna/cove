...
function addParagraph(doc, noteId, content) {
    const props = { text: new Text(content) };
    doc.addBlock('affine:paragraph', props, noteId);
}
function addSampleNote(doc, noteId, i) {
    addParagraph(doc, noteId, `Note ${i + 1}`);
    addParagraph(doc, noteId, 'Hello World!');
    addParagraph(doc, noteId, 'Hello World! Lorem ipsum dolor sit amet. Consectetur adipiscing elit. Sed do eiusmod tempor incididunt.');
    addParagraph(doc, noteId, '你好这是测试，这是一个为了换行而写的中文段落。这个段落会自动换行。');
}
export function addSampleNotes(doc, n) {
