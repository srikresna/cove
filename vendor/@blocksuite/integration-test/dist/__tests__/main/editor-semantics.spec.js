...
function findSlashActionItem(items, name) {
...
function getRichTextByBlockId(blockId) {
...
function setTextSelection(blockId, index, length) {
    const to = length
        ? {
            blockId,
            index: index + length,
...
async function triggerMarkdown(blockId, input, matcherName) {
    ...
    await wait();
    const richText = getRichTextByBlockId(blockId);
    const matcher = richText.markdownMatches.find(item => item.name === matcherName);
    if (!matcher) {
...
describe('markdown/list/paragraph/quote/code/link', () => {
    ...
    test('markdown heading and quote shortcuts convert paragraph type', async () => {
        const { noteId: headingNoteId, paragraphId: headingParagraphId } = await createParagraph();
        ...
        const headingModel = headingNote.children[0];
        expect(headingModel.flavour).toBe('affine:paragraph');
        expect(headingModel.props.type).toBe('h1');
        const { noteId: quoteNoteId, paragraphId: quoteParagraphId } = await createParagraph();
        ...
        const quoteModel = quoteNote.children[0];
    ...
    test('inline markdown converts style and link attributes', async () => {
        ...
        expect(boldRichText.inlineEditor.getFormat({ index: 1, length: 0 })).toMatchObject({
            bold: true,
        });
        ...
        expect(codeRichText.inlineEditor.getFormat({ index: 1, length: 0 })).toMatchObject({
            code: true,
        });
        const { paragraphId: linkParagraphId } = await createParagraph();
        await triggerMarkdown(linkParagraphId, '[AFFiNE](https://affine.pro) ', 'link');
        const linkRichText = getRichTextByBlockId(linkParagraphId);
        expect(linkRichText.inlineEditor.yTextString).toBe('AFFiNE');
        expect(linkRichText.inlineEditor.getFormat({ index: 1, length: 0 })).toMatchObject({
...
describe('hotkey/bracket/linked-page', () => {
    ...
    test('consecutive linked-page reference nodes render as separate references', async () => {
        ...
        const paragraphModel = doc.getBlock(paragraphId)?.model;
        if (!paragraphModel) {
            throw new Error('Cannot find paragraph model');
        }
        ...
        const danglingReferences = richText.querySelectorAll('affine-reference');
        expect(danglingReferences.length).toBe(2);
        expect([...danglingReferences].every(reference => !reference.refMeta)).toBe(true);
    });
});
describe('slash-menu action semantics', () => {
    test('date and move actions mutate block content/order as expected', async () => {
        ...
        const secondId = doc.addBlock('affine:paragraph', { text: new Text('second') }, noteId);
        const second = doc.getBlock(secondId)?.model;
        if (!second) {
        ...
        const items = typeof slashItems === 'function'
