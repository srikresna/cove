...
export function scrollToBlock(host, blockId) {
...
update) => {
    const handler = () => {
        ...
        const headings = getHeadingBlocksFromDoc(host.store, [NoteDisplayMode.DocAndEdgeless, NoteDisplayMode.DocOnly], true);
...
function highlightBlock(host, blockId) {
    const emptyClear = () => { };
    if (host.store.root?.id === blockId)
        return emptyClear;
...
export async function scrollToBlockWithHighlight(host, blockId, timeout = 3000) {
