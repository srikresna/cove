...
function getRangeRects(range, fullText) {
    ...
    const totalWidth = Math.floor(rects.reduce((sum, rect) => sum + rect.width, 0));
    ...
    segments.forEach(segment => {
        ...
        if (currentWidth + segmentWidth > rects[currentRect]?.width &&
...
export function getSentenceRects(element, sentence) {
    ...
    while ((startIndex = text.indexOf(sentence, startIndex)) !== -1) {
        ...
        rects = rects.concat(getRangeRects(range, text.slice(startIndex, endIndex)));
        startIndex = endIndex;
    }
    return rects;
}
export function segmentSentences(text) {
    return Array.from(sentenceSegmenter.segment(text)).map(({ segment }) => segment);
}
