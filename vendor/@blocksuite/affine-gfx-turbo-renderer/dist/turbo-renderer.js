...
import { getEffectiveDpr, GfxExtension, GfxExtensionIdentifier, viewportRuntimeConfig, } from '@blocksuite/std/gfx';
import { BehaviorSubject, distinctUntilChanged, merge, Subject, take, tap, timer, } from 'rxjs';
import { debounceTime } from 'rxjs/operators';
import { debugLog, getViewportLayoutTree, paintPlaceholder, syncCanvasSize, } from './renderer-utils';
...
export const TurboRendererConfigFactory = ConfigExtensionFactory('viewport-turbo-renderer');
/**
...
export class ViewportTurboRendererExtension extends GfxExtension {
    static { this.key = 'viewportTurboRenderer'; }
    get currentState() {
        return this.state$.value;
    }
    constructor(gfx) {
        super(gfx);
        this.state$ = new BehaviorSubject('inactive');
        ...
        this.isRecentlyZoomed$ = new BehaviorSubject(false);
        ...
        this.state$
            .pipe(distinctUntilChanged(), tap(state => this.debugLog(`State changed to: ${state}`)))
            .subscribe();
    ...
    mounted() {
        ...
        this.viewport.elementReady.pipe(take(1)).subscribe(element => {
            ...
            this.disposables.add(this.viewport.sizeUpdated.subscribe(() => this.handleResize()));
            this.disposables.add(this.viewport.viewportUpdated.subscribe(() => {
            ...
            this.disposables.add(this.viewport.zooming$
                .pipe(tap(isZooming => {
                ...
                if (isZooming) {
                    ...
                    this.disposables.add(timer(defaultOptions.postZoomDelay).subscribe(() => {
            ...
            if (this.viewport.SKIP_REFRESH_DURING_GESTURE) {
                ...
                const scheduleRefresh = () => {
                    ...
                    pendingTimerId = setTimeout(() => {
                        pendingTimerId = null;
                        if (!this.viewport.panning$.value &&
        ...
        this.disposables.add(merge(selectionUpdates$, blockUpdates$).subscribe(() => this.invalidate()));
    }
    unmounted() {
    ...
    async refresh() {
        ...
        if (this.viewport.zoom > this.options.zoomThreshold) {
            ...
            nextState = 'zooming';
            if (shouldPreferBitmapCacheDuringLowZoomGesture({
                isIOS: IS_IOS,
            ...
            this.worker.onmessage = (e) => {
                if (e.data.type === 'bitmapPainted') {
                    if (e.data.version === this.layoutVersion) {
                        this.debugLog(`Bitmap painted successfully (version=${e.data.version})`);
                        this.clearBitmap();
                        ...
                        this.debugLog(`Received outdated bitmap (got=${e.data.version}, current=${this.layoutVersion})`);
                        e.data.bitmap.close();
                        this.state$.next('pending');
                        resolve();
                    }
                    ...
                    this.debugLog(`Paint error: ${e.data.error} for blockType: ${e.data.blockType}`);
                    this.state$.next('pending');
                    resolve();
                }
            };
    ...
    canUseBitmapCache() {
        if (!this.options.enableBitmapRendering || this.isZooming())
            return false;
    ...
    drawCachedBitmap() {
        if (!this.options.enableBitmapRendering || !this.bitmap) {
            this.debugLog('Bitmap drawing skipped (disabled or no cached bitmap available)');
            return;
        }
        const layout = this.layoutCache;
        ...
        const dpr = getEffectiveDpr(this.viewport.zoom);
        ctx.drawImage(bitmap, layoutViewCoord[0] * dpr, layoutViewCoord[1] * dpr, layout.overallRect.w * dpr * this.viewport.zoom, layout.overallRect.h * dpr * this.viewport.zoom);
        this.debugLog('Bitmap drawn to canvas');
    }
    canOptimize() {
        if (this.isRecentlyZoomed$.value)
            return false;
        const isBelowZoomThreshold = this.viewport.zoom <= this.options.zoomThreshold;
        return this.state$.value === 'zooming' && isBelowZoomThreshold;
    }
    updateOptimizedBlocks() {
        if (!this.canOptimize())
            return;
        if (!shouldIdleTurboBlocksDuringZooming({
            isIOS: IS_IOS,
            zoom: this.viewport.zoom,
        })) {
            this.clearOptimizedBlocks();
        ...
        requestAnimationFrame(() => {
            if (!this.viewportElement || !this.layoutCache)
                return;
    ...
    clearOptimizedBlocks() {
        if (!this.viewportElement || this.optimizedBlockIds.length === 0)
            return;
        this.viewportElement.setBlocksActive(this.optimizedBlockIds);
        this.optimizedBlockIds = [];
...
export const ViewportTurboRendererIdentifier = GfxExtensionIdentifier('viewportTurboRenderer');
