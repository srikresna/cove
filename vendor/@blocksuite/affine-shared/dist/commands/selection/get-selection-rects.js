...
export const getSelectionRectsCommand = (ctx, next) => {
    let textSelection;
    let blockSelections;
    // priority parameters
    if (ctx.textSelection) {
        ...
        console.error('No selection provided, may forgot to call getTextSelection or getBlockSelections or provide the selection directly.');
        return;
    }
    const { std } = ctx;
...
export function getRangeRects(range, container) {
