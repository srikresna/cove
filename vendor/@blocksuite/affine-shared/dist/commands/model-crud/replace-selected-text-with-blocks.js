 ...
 */
const canMergeBlocks = (blockA, blockB) => {
    // Paragraphs can always be merged to
...
const getBlocksPattern = (blocks, startBlockModel, endBlockModel) => {
    ...
    return {
        ...
        canMergeWithEnd: isLastMergable && endBlockModel
            ? canMergeBlocks(endBlockModel, lastBlock)
            : false,
    };
};
const mergeText = (targetModel, sourceBlock, offset) => {
...
const splitParagraph = (doc, parent, blockModel, index, splitOffset) => {
...
const getSelectedBlocks = (doc, textSelection) => {
...
const deleteSelectedText = (doc, textSelection) => {
...
const addBlocks = (doc, blocks, parent, from) => {
 ...
 * @returns
 */
export const replaceSelectedTextWithBlocksCommand = (ctx, next) => {
    ...
    match(pattern)
         ...
         * );
         */
        const nextBlockModel = splitParagraph(doc, parent, startBlockModel, startIndex, startOffset);
        mergeText(startBlockModel, blocks[0], startOffset);
