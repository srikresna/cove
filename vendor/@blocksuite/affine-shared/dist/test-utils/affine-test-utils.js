...
function blockToTemplate(block, indent = '') {
...
function compareBlocks(actual, expected, compareId = false) {
    if (actual.flavour !== expected.flavour)
        return false;
    ...
    for (const [i, child] of actual.children.entries()) {
        if (!compareBlocks(child, expected.children[i], compareId))
            return false;
    }
    return true;
}
function compareDocs(actual, expected, compareId = false) {
...
expect.extend({
    toEqualDoc(received, expected, options = { compareId: false }) {
        ...
        if (pass) {
            ...
            return {
                message: () => `Documents are not equal.\n\nActual:\n${actualTemplate}\n\nExpected:\n${expectedTemplate}`,
                pass: false,
            };
        }
    },
