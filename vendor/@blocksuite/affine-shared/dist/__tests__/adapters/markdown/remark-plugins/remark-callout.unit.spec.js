...
describe('remarkCallout plugin', () => {
    ...
    const assertCallout = (root, expectedEmoji, expectedText) => {
    ...
    const assertRegularBlockquote = (root, expectedText) => {
        const firstChild = root.children[0];
        expect(isBlockQuote(firstChild)).toBe(true);
    ...
    it('should not transform when callout marker is in the middle of text', async () => {
        const root = process('> This is a regular blockquote with [!💡] in the middle');
        assertRegularBlockquote(root, 'This is a regular blockquote with [!💡] in the middle');
    });
    it('should handle multiple callouts in the same document', async () => {
        const root = process(`> [!💡] First callout\n\n> [!] Second callout without emoji`);
        expect(root.children).toHaveLength(2);
        assertCallout({ children: [root.children[0]] }, '💡', 'First callout');
        assertCallout({ children: [root.children[1]] }, '', 'Second callout without emoji');
    });
    it('should handle multiple callouts and regular blockquote in the same document', async () => {
        ...
        assertCallout({ children: [root.children[0]] }, '💡', 'First callout');
        assertCallout({ children: [root.children[1]] }, '', 'Second callout without emoji');
        assertRegularBlockquote({ children: [root.children[2]] }, 'This is a regular blockquote');
    });
});
