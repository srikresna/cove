...
const NATIVE_IMAGE_FILES_PICKER_KEY = '__AFFINE_NATIVE_IMAGE_FILES_PICKER__';
export function registerNativeImageFilesPicker(picker) {
    if (picker) {
...
export async function snapshotFile(file, relativePath) {
    let bytes = null;
...
export async function snapshotFiles(files) {
    const results = await Promise.allSettled(files.map(file => snapshotFile(file)));
    const snapshots = [];
...
function canUseFileSystemAccessAPI(api) {
...
export async function openFilesWith(acceptType = 'Any', multiple = true, options) {
    const supportsFileSystemAccess = options?.fileSystemAccess !== false &&
...
export async function openDirectory(options) {
    const supportsFileSystemAccess = canUseFileSystemAccessAPI('showDirectoryPicker');
    if (options?.fileSystemAccess !== false &&
            ...
            const readDirectory = async (directoryHandle, path) => {
                for await (const handle of directoryHandle.values()) {
                    ...
                    if (handle.kind === 'file') {
                        ...
                        await readDirectory(handle, relativePath);
                    }
                }
            };
...
export async function openSingleFileWith(acceptType) {
