...
function resolveURL(str, baseUrl, padded = false) {
    const url = toURL(str);
    if (!url)
        return null;
 ...
 */
export function isValidUrl(str, baseUrl = location.origin) {
    str = str.trim();
...
const URL_SCHEME_IN_TOKEN_REGEXP = /(?:https?:\/\/|ftp:\/\/|sftp:\/\/|mailto:|tel:|www\.)/i;
const URL_LEADING_DELIMITER_REGEXP = /^[-([{<'"~]+/;
const URL_TRAILING_DELIMITER_REGEXP = /[)\]}>.,;:!?'"]+$/;
function appendUrlTextSegment(segments, segment) {
    if (!segment.text)
        return;
...
function splitTokenByUrl(token, baseUrl) {
    ...
    const trailing = withoutLeading.match(URL_TRAILING_DELIMITER_REGEXP)?.[0] ?? '';
    const core = trailing
 ...
 */
export function splitTextByUrl(text, baseUrl = location.origin) {
