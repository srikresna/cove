export async function printToPdf(rootElement = document.querySelector('.affine-page-viewport'), options = {}) {
        ...
        iframe.onload = async () => {
            ...
            for (const element of document.styleSheets) {
                try {
                    ...
                    if (element.href) {
                        console.warn('css cannot be applied when printing pdf, this may be because of CORS policy from its domain.', element.href);
                    }
                    else {
                        reject(e);
                    }
            ...
            const canvasImgObjectUrlMap = new Map();
            const allCanvas = findAllCanvases(rootElement);
            let canvasKey = 1;
            const canvasToKeyMap = new Map();
            for (const canvas of allCanvas) {
                const key = canvasKey.toString();
                canvasToKeyMap.set(canvas, key);
                canvasKey++;
                const canvasImgObjectUrl = await new Promise(resolve => {
                    try {
                ...
                if (!canvasImgObjectUrl) {
                    console.warn('canvas cannot be converted to image when printing pdf, this may be because of CORS policy');
                    continue;
                }
            ...
            const deepCloneWithShadows = (node) => {
                ...
                const appendChildren = (source) => {
                    source.childNodes.forEach(child => {
                        clone.append(deepCloneWithShadows(child));
                    });
                };
                if (node instanceof HTMLElement && node.shadowRoot) {
            ...
            const importedRoot = deepCloneWithShadows(rootElement);
