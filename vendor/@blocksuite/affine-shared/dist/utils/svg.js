...
const DEFAULT_SVG_SANITIZE_CONFIG = {
    USE_PROFILES: { svg: true },
    ADD_TAGS: ['use'],
    ADD_ATTR: ['href', 'xlink:href', 'class', 'style', 'id'],
};
const DEFAULT_FOREIGN_OBJECT_HTML_SANITIZE_CONFIG = {
    USE_PROFILES: { html: true },
};
const SAFE_LINK_PROTOCOLS = new Set(['http:', 'https:', 'mailto:']);
const SVG_DATA_URL_PATTERN = /^data:image\/svg\+xml(?:;charset=[^;,]+)?(?<base64>;base64)?,(?<data>[\s\S]*)$/i;
const SAFE_IMAGE_DATA_URL_PATTERN = /^data:image\/(?:png|jpe?g|gif|webp|svg\+xml);base64,[a-z0-9+/=]+$/i;
const UNSAFE_CSS_PATTERN = /(?:url\s*\(|@import|javascript\s*:|expression\s*\(|-moz-binding)/i;
...
function getSvgSanitizeConfig(options) {
    return {
        ...DEFAULT_SVG_SANITIZE_CONFIG,
        ...options?.svg,
    };
}
function getForeignObjectHtmlSanitizeConfig(options) {
    return {
...
function startsWithIgnoreCase(value, search, index) {
    return value.slice(index, index + search.length).toLowerCase() === search;
}
function getSvgRootStartIndex(value) {
    ...
    if (!startsWithIgnoreCase(value, '<svg', index))
        return -1;
...
function hasSvgRoot(value) {
    return getSvgRootStartIndex(value) !== -1;
}
function getOriginalSvgRoot(svg, parser) {
...
function ensureSvgRoot(originalRoot, sanitized, parser) {
...
function sanitizeForeignObjects(root, options) {
    root.querySelectorAll('foreignObject, foreignobject').forEach(element => {
        element.innerHTML = DOMPurify.sanitize(element.innerHTML, getForeignObjectHtmlSanitizeConfig(options));
    });
}
...
function isSafeHref(element, value) {
    if (value.startsWith('#'))
        return true;
...
function decodeSvgDataUrl(value) {
    ...
    try {
        if (groups.base64) {
            return new TextDecoder().decode(Uint8Array.from(atob(groups.data), char => char.charCodeAt(0)));
        }
...
function encodeSvgDataUrl(svg) {
    const binary = Array.from(new TextEncoder().encode(svg), byte => String.fromCharCode(byte)).join('');
    return `data:image/svg+xml;base64,${btoa(binary)}`;
}
function getHrefAttributes(element) {
    return Array.from(element.attributes).filter(attribute => attribute.name === 'href' || attribute.name === 'xlink:href');
}
function tightenSvgTree(root, options, depth) {
    root.querySelectorAll('*').forEach(element => {
        getHrefAttributes(element).forEach(attribute => {
            const href = attribute.value.trim();
            const nestedSvg = element.tagName.toLowerCase() === 'image'
...
function sanitizeSvgWithDepth(svg, options, depth) {
