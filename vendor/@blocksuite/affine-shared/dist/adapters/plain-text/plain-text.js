...
import { BlockPlainTextAdapterMatcherIdentifier, } from './block-adapter';
import { InlineDeltaToPlainTextAdapterMatcherIdentifier, PlainTextDeltaConverter, } from './delta-converter';
export class PlainTextAdapter extends BaseAdapter {
    constructor(job, provider) {
        super(job, provider);
        const blockMatchers = Array.from(provider.getAll(BlockPlainTextAdapterMatcherIdentifier).values());
        const inlineDeltaToPlainTextAdapterMatchers = Array.from(provider.getAll(InlineDeltaToPlainTextAdapterMatcherIdentifier).values());
...
export const PlainTextAdapterFactoryExtension = {
    setup: di => {
        di.addImpl(PlainTextAdapterFactoryIdentifier, provider => ({
            get: (job) => new PlainTextAdapter(job, provider),
        }));
    },
};
