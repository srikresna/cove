...
import { getSelectedRect, isTopLevelBlock, requestThrottledConnectedFrame, } from '@blocksuite/affine-shared/utils';
...
let EdgelessRemoteSelectionWidget = (() => {
  ...
  `; }
        ...
        connectedCallback() {
            ...
            if (this.surface) {
                _disposables.add(this.surface.elementAdded.subscribe(this._updateOnElementChange));
                _disposables.add(this.surface.elementRemoved.subscribe(this._updateOnElementChange));
                _disposables.add(this.surface.elementUpdated.subscribe(this._updateOnElementChange));
            }
            _disposables.add(store.slots.blockUpdated.subscribe(this._updateOnElementChange));
            _disposables.add(this.selection.slots.remoteUpdated.subscribe(this._updateRemoteRects));
            _disposables.add(this.selection.slots.remoteCursorUpdated.subscribe(this._updateRemoteCursor));
            _disposables.add(this.gfx.viewport.viewportUpdated.subscribe(() => {
    ...
    `;
        ...
        constructor() {
            ...
            this._updateTransform = requestThrottledConnectedFrame(() => {
                ...
                this.style.setProperty('transform', `translate(${translateX / viewScale}px, ${translateY / viewScale}px) scale(var(--v-zoom))`);
            }, this);
