...
import { EditPropsStore, ThemeProvider, ViewportElementProvider, } from '@blocksuite/affine-shared/services';
import { EdgelessDraggableElementController, EdgelessToolbarToolMixin, } from '@blocksuite/affine-widget-edgeless-toolbar';
...
import { getMindmapRender, mediaConfig, mediaRender, mindmapConfig, textConfig, textRender, toolConfig2StyleObj, } from './basket-elements.js';
import { basketIconDark, basketIconLight, mindmapMenuMediaIcon, textIcon, } from './icons.js';
...
let EdgelessMindmapToolButton = (() => {
  ...
  `; }
            ...
            Object.assign(menu.element, {
                ...
                onImportMindMap: (bound) => {
                    return importMindmap(bound).then(mindmap => {
                        ...
                        if (!id)
                            return;
                        const element = this.crud.getElementById(id);
        ...
        initDragController() {
            ...
            this.disposables.add(this.gfx.tool.currentToolName$.subscribe(toolName => {
                // FIXME: remove the assertion after gfx tool refactor
                if (toolName !== 'empty' && this.readyToDrop) {
