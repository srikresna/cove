...
import { applyNodeStyle, LayoutType, } from '@blocksuite/affine-model';
 ...
 */
export function hideNodeConnector(mindmap, 
/**
 ...
 * @returns
 */
function moveNodePosition(mindmap, node, parent, targetIndex, layout) {
    parent = mindmap.nodeMap.get(typeof parent === 'string' ? parent : parent.id);
    ...
    const index = preSibling || afterSibling
        ? generateKeyBetween(preSibling?.detail.index ?? null, afterSibling?.detail.index ?? null)
...
export function applyStyle(mindmap, shouldFitContent = false) {
    mindmap.surface.store.transact(() => {
        ...
        if (shouldFitContent) {
            fitContent(mindmap.tree.element);
        }
 ...
 * @returns
 */
export function addNode(mindmap, parent, node, targetIndex) {
    const parentNode = mindmap.getNode(typeof parent === 'string' ? parent : parent.id);
    if (!parentNode) {
        return;
    }
    ...
    const index = before || after
        ? generateKeyBetween(before?.detail.index ?? null, after?.detail.index ?? null)
        : node.detail.index;
    mindmap.surface.store.transact(() => {
...
export function addTree(mindmap, parent, tree, 
/**
...
sibling) {
    ...
    assertType(parent);
    const traverse = (node, parent, sibling) => {
        let nodeId;
        if ('text' in node) {
 ...
 */
export function detachMindmap(mindmap, subtree) {
    subtree =
...
export function handleLayout(mindmap, tree, shouldApplyStyle = true, layoutType) {
...
export function createFromTree(tree, style, layoutType, surface) {
 ...
 */
export function moveNode(from, subtree, to, parent, index) {
    if (from === to) {
        return moveNodePosition(from, subtree, parent, index);
    }
    if (!detachMindmap(from, subtree))
        return;
    return addNode(to, parent, subtree, index);
}
export function findTargetNode(mindmap, position) {
    const find = (node) => {
        if (!node.responseArea) {
            return null;
        }
        const layoutDir = mindmap.getLayoutDir(node);
        if (layoutDir === LayoutType.BALANCE ||
...
function determineInsertPosition(mindmap, mindmapNode, position) {
    if (!mindmapNode.responseArea ||
    ...
    const targetLayout = layoutDir === LayoutType.BALANCE
        ? position[0] > elementBound.x + elementBound.w / 2
            ? LayoutType.RIGHT
            : LayoutType.LEFT
        : layoutDir;
    if (elementBound.containsPoint(position) ||
        (layoutDir === LayoutType.RIGHT
    ...
    if (mindmap.layoutType === LayoutType.BALANCE &&
        mindmap.getPath(mindmapNode.id).length === 2) {
        return {
            type: 'sibling',
            layoutDir: targetLayout,
            position: targetLayout === LayoutType.LEFT
    ...
    return {
        type: 'sibling',
        layoutDir: targetLayout,
        position: position[1] > elementBound.y + elementBound.h / 2 ? 'next' : 'prev',
    };
}
function showMergeIndicator(targetMindMap, target, sourceMindMap, source, insertPosition, callback) {
    const newParent = insertPosition.type === 'child'
        ? target
    ...
    if (insertPosition.type === 'sibling') {
        ...
        const idx = parent.children
            ...
            .indexOf(target);
        path.push(idx === -1
            ? Math.max(0, last(curPath) + (insertPosition.position === 'next' ? 1 : 0))
 ...
 */
export function tryMoveNode(targetMindMap, target, sourceMindMap, source, position, callback) {
    const insertInfo = determineInsertPosition(targetMindMap, target, position);
    if (!insertInfo) {
        return null;
    }
    return showMergeIndicator(targetMindMap, target, sourceMindMap, source, insertInfo, callback);
}
/**
 ...
 * @returns
 */
export function containsNode(mindmap, targetNode, searchParent) {
    searchParent = searchParent ?? mindmap.tree;
    const find = (checkAgainstNode) => {
