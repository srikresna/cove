import { LayoutType, LocalShapeElementModel, } from '@blocksuite/affine-model';
...
export class MindMapView extends GfxElementModelView {
    ...
    _initCollapseButtons() {
        ...
        this.disposable.add(this.model.propsUpdated.subscribe(({ key }) => {
            if (key === 'layoutType' || key === 'style') {
                updateButtons();
            }
        }));
        this.disposable.add(this.surface.elementUpdated.subscribe(payload => {
    ...
    _needToUpdateButtonStyle(options) {
    ...
    _setLayoutMethod() {
        this.model.setLayoutMethod(function (tree, options) {
    ...
    _setVisibleOnSelection() {
        let lastNode = null;
        this.disposable.add(this.gfx.selection.slots.updated.subscribe(() => {
            const elm = this.gfx.selection.firstElement;
            if (lastNode) {
                this._updateButtonVisibility(lastNode);
            }
            if (this.gfx.selection.selectedElements.length === 1 &&
                elm?.id &&
    ...
    _updateButtonVisibility(node) {
        ...
        const isNodeSelected = this.gfx.selection.firstElement === latestNode.element;
    ...
    _updateCollapseButton(node) {
        ...
        if (this._needToUpdateButtonStyle({
            ...
            Object.entries(buttonStyle).forEach(([key, value]) => {
                ...
                collapseButton[key] = value;
            });
            const nodeElementBound = node.element.elementBound;
            const buttonBound = nodeElementBound.moveDelta(layout === LayoutType.LEFT
                ? -6 - buttonStyle.width
                : 6 + nodeElementBound.w, (nodeElementBound.h - buttonStyle.height) / 2);
        ...
        if (!alreadyCreated) {
            ...
            const buttonView = this.gfx.view.get(id);
            const isOnElementBound = (evt) => {
                const [x, y] = this.gfx.viewport.toModelCoord(evt.x, evt.y);
                return buttonView.model.includesPoint(x, y, { useElementBound: true }, this.gfx.std.host);
            };
            ...
            const nodeView = this.gfx.view.get(node.id);
...
export const MindMapInteraction = GfxViewInteractionExtension(MindMapView.type, {
    resizeConstraint: {
        allowedHandlers: [],
    },
    handleSelection: () => {
