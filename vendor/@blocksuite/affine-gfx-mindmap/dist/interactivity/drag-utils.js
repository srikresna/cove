...
const fillResponseArea = (node, layoutType, parent) => {
    // root node
    if (!parent) {
        const rootElmBound = node.element.elementBound;
        const width = layoutType === LayoutType.BALANCE
            ? rootElmBound.w + TAIL_RESPONSE_AREA * 2
            : rootElmBound.w + TAIL_RESPONSE_AREA;
        node.responseArea = new Bound(layoutType === LayoutType.BALANCE || layoutType === LayoutType.LEFT
            ? rootElmBound.x - TAIL_RESPONSE_AREA
            : rootElmBound.x, rootElmBound.y, width, rootElmBound.h);
        if (node.detail.collapsed) {
            return;
        }
        if (layoutType === LayoutType.BALANCE) {
            node.right.forEach(child => {
                fillResponseArea(child, LayoutType.RIGHT, node);
            });
            node.left.forEach(child => {
        ...
        const isLast = idx === (parent.children.length || -1) - 1 && isOnEdge(node, 'tail');
        ...
        const w = (layoutType === LayoutType.RIGHT
            ? node.element.x +
                node.element.w -
                (parent.element.x + parent.element.w)
            : parent.element.x - node.element.x) + TAIL_RESPONSE_AREA;
        node.responseArea = new Bound(layoutType === LayoutType.RIGHT
            ? parent.element.x + parent.element.w
            : parent.element.x - w, node.element.y - upperSpacing, w, h);
...
export const balanceLeftRightResponseArea = (tree) => {
    const leftTreeArea = tree.left.reduce((pre, node) => {
        if (pre) {
            return pre.unite(node.responseArea);
        }
        return node.responseArea;
    }, null);
    const rightTreeArea = tree.right.reduce((pre, node) => {
        if (pre) {
            return pre.unite(node.responseArea);
        }
        return node.responseArea;
...
export const calculateResponseArea = (mindmap) => {
