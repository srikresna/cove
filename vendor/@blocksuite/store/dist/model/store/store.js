...
import { BlockSchemaIdentifier, HistoryExtension, StoreExtensionIdentifier, StoreSelectionExtension, } from '../../extension/index.js';
...
export class Store {
     ...
     */
    constructor({ readonly, query, provider, extensions }) {
        /**
        ...
        this._readonly = signal(false);
        this._isEmpty = computed(() => {
        ...
        this._subscribeToSlots = () => {
            this.disposableGroup.add(this.slots.yBlockUpdated.subscribe(({ type, id, isLocal }) => {
                switch (type) {
                    case 'add': {
     ...
     * @category Block CRUD
     */
    addSiblingBlocks(targetModel, props, placement = 'after') {
        if (!props.length)
            return [];
     ...
     * @category Block CRUD
     */
    updateBlock(modelOrId, callBackOrProps) {
        if (this.readonly) {
        ...
        const model = typeof modelOrId === 'string'
            ...
            : modelOrId;
        if (!model) {
            throw new BlockSuiteError(ErrorCode.ModelCRUDError, `updating block: ${modelOrId} not found`);
        }
        if (!isCallback) {
     ...
     * @category Block CRUD
     */
    getModelsByFlavour(blockFlavour) {
     ...
     */
    getTransformer(middlewares = []) {
        return new Transformer({
            schema: this.schema,
            blobCRUD: this.workspace.blobSync,
            docCRUD: {
                create: (id) => this.workspace.createDoc(id).getStore({ id }),
                get: (id) => this.workspace.getDoc(id)?.getStore({ id }) ?? null,
                delete: (id) => this.workspace.removeDoc(id),
            },
            middlewares,
        });
    }
    ...
    _handleYBlockAdd(id, isLocal) {
        this.slots.yBlockUpdated.next({ type: 'add', id, isLocal });
    }
    _handleYBlockDelete(id, isLocal) {
