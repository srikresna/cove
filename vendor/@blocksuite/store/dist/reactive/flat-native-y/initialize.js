...
export const initializeData = ({ getProxy, transform, yMap, }) => {
    const root = {};
    Array.from(yMap.entries()).forEach(([key, value]) => {
        ...
        if (Boxed.is(value)) {
            ...
            throw new BlockSuiteError(BlockSuiteError.ErrorCode.ReactiveProxyError, 'flatY2Native does not support Y.Map as value of Y.Map');
        }
        else {
