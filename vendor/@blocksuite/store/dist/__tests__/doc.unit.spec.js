...
import { DividerBlockSchemaExtension, ListBlockSchemaExtension, NoteBlockSchemaExtension, ParagraphBlockSchemaExtension, RootBlockSchemaExtension, } from './test-schema.js';
const extensions = [
...
test('query', () => {
    ...
    const list1 = store1.addBlock('affine:list', {}, note);
    ...
    const list2 = store1.addBlock('affine:list', {}, note);
...
describe('move blocks', () => {
    beforeEach((context) => {
        ...
        const noteIds = store.addBlocks([1, 2, 3].map(i => ({
            flavour: 'affine:note',
            blockProps: { id: `${i}` },
        })), page);
    ...
    test('move block to itself', ({ doc, page, notes }) => {
