...
describe('block model should has signal props', () => {
    ...
    test('nested', () => {
        ...
        expect(yBlock.get('prop:style').toJSON()).toEqual({
            color: 'red',
        });
...
test('deep sync', () => {
    ...
    const getColsMap = () => yBlock.get('prop:cols');
    const getRowsArr = () => yBlock.get('prop:rows');
...
describe('flat', () => {
    test('flat crud', async () => {
        ...
        onChange.mockClear();
        onColUpdated.mockClear();
        delete model.props.cols.a.color;
        ...
        expect(yBlock.get('prop:title').toJSON()).toBe('test');
        ...
        const getLabels = () => yBlock.get('prop:labels');
        ...
        expect(yBlock.get('prop:textCols.a').toJSON()).toBe('test');
