...
import { EdgelessLegacySlotIdentifier, getSurfaceComponent, isNoteBlock, } from '@blocksuite/affine-block-surface';
...
let NoteSlicer = (() => {
    ...
    return class NoteSlicer extends _classSuper {
        ...
        _sliceNote() {
            ...
            const { index: originalIndex, xywh, background, displayMode, } = this._anchorNote.props;
            const originalNoteIndex = this._anchorNote.parent?.children.findIndex(child => child === this._anchorNote);
            const { children } = this._anchorNote;
            const { collapse: _, collapsedHeight: __, ...restOfEdgeless } = this._anchorNote.props.edgeless;
            ...
            const newNoteId = this.store.addBlock('affine:note', {
                background,
                displayMode,
                xywh: serializeXYWH(x, newY + NEW_NOTE_GAP, width, DEFAULT_NOTE_HEIGHT),
                index: originalIndex + 1,
                edgeless: restOfEdgeless,
            }, doc.root?.id, originalNoteIndex ? originalNoteIndex + 1 : undefined);
            doc.moveBlocks(resetBlocks, doc.getModelById(newNoteId));
        ...
        connectedCallback() {
            ...
            disposables.add(slots.elementResizeStart.subscribe(() => {
                this._isResizing = true;
            }));
            disposables.add(slots.elementResizeEnd.subscribe(() => {
            ...
            disposables.add(gfx.viewport.viewportUpdated.subscribe(() => {
                this._hidden = true;
                this.requestUpdate();
            }));
            disposables.add(gfx.selection.slots.updated.subscribe(() => {
            ...
            disposables.add(slots.toggleNoteSlicer.subscribe(() => {
        ...
        updated(_changedProperties) {
            ...
            if (_changedProperties.has('anchorNote')) {
                ...
                if (this._anchorNote) {
                    ...
                    this._noteDisposables.add(this._anchorNote.propsUpdated.subscribe(({ key }) => {
...
export { NoteSlicer };
export const noteSlicerWidget = WidgetViewExtension('affine:page', NOTE_SLICER_WIDGET, literal `${unsafeStatic(NOTE_SLICER_WIDGET)}`);
