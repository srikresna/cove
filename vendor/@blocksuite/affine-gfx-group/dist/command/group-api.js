...
import { batchAddChildren, batchRemoveChildren, GfxControllerIdentifier, measureOperation, } from '@blocksuite/std/gfx';
import { generateKeyBetween, generateNKeysBetween } from 'fractional-indexing';
const getTopLevelOrderedElements = (gfx) => {
    const topLevelElements = gfx.layer.layers.reduce((elements, layer) => {
...
const buildUngroupIndexes = (orderedElements, afterIndex, beforeIndex, fallbackAnchorIndex) => {
    if (orderedElements.length === 0) {
        return [];
    }
    const count = orderedElements.length;
    const tryGenerateN = (left, right) => {
        try {
    ...
    const tryGenerateOneByOne = (left, right) => {
        try {
            let cursor = left;
    ...
    return (tryGenerateN(afterIndex, beforeIndex) ??
...
export const ungroupCommand = (ctx, next) => {
    measureOperation('edgeless:ungroup', () => {
        ...
        const beforeSiblingIndex = groupPosition > 0 ? (siblings[groupPosition - 1]?.index ?? null) : null;
        const afterSiblingIndex = groupPosition === -1
            ? null
            : (siblings[groupPosition + 1]?.index ?? null);
        const nextIndexes = buildUngroupIndexes(orderedElements, beforeSiblingIndex, afterSiblingIndex, group.index);
        std.store.transact(() => {
            if (parent !== null) {
