...
export class GroupInteractionExtension extends InteractivityExtension {
    static { this.key = 'group-selection'; }
    mounted() {
            ...
            if (activeGroup && activeGroup.hasDescendant(target)) {
                ...
                if (activeGroupIdx !== -1) {
                    target =
                        groups
                            ...
                            .findLast(group => group instanceof GroupElementModel &&
                            candidates.includes(group)) ?? target;
                }
            }
            else {
                const groups = target.groups;
                target =
                    groups.findLast(group => {
                        return (group instanceof GroupElementModel && candidates.includes(group));
                    }) ?? target;
            }
