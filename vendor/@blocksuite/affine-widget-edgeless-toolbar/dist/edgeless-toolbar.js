...
import { ArrowLeftSmallIcon, ArrowRightSmallIcon, MoreHorizontalIcon, } from '@blocksuite/icons/lit';
...
import { QuickToolIdentifier, SeniorToolIdentifier, } from './extension/index.js';
...
let EdgelessToolbarWidget = (() => {
  ...
  `; }
        ...
        get _quickTools() {
            ...
            const quickTools = Array.from(this.std.provider.getAll(QuickToolIdentifier).values());
            const gfx = this.std.get(GfxControllerIdentifier);
            return quickTools
                .map(tool => tool({ block, gfx, toolbarContainer: this.toolbarContainer }))
        ...
        get _seniorTools() {
            ...
            const seniorTools = Array.from(this.std.provider.getAll(SeniorToolIdentifier).values());
            const gfx = this.std.get(GfxControllerIdentifier);
            return seniorTools
                .map(tool => tool({ block, gfx, toolbarContainer: this.toolbarContainer }))
                .filter(({ enable = true }) => enable);
        ...
        constructor() {
            ...
            this._slotsProvider = new ContextProvider(this, {
                ...
                initialValue: { resize: new Subject() },
            });
    ...
    `;
            ...
            this._disposables.add(this.block.bindHotKey({
                Escape: () => {
                    if (this.gfx.selection.editing)
                        return;
                    if (this.edgelessTool === 'frameNavigator')
                        return;
        ...
        firstUpdated() {
            ...
            _disposables.add(gfx.viewport.viewportUpdated.subscribe(() => this.requestUpdate()));
            _disposables.add(slots.readonlyUpdated.subscribe(() => {
                this.requestUpdate();
            }));
            _disposables.add(slots.toolbarLocked.subscribe(disabled => {
            ...
            _disposables.add(editPropsStore.slots.storageUpdated.subscribe(({ key }) => {
...
export const edgelessToolbarWidget = WidgetViewExtension('affine:page', EDGELESS_TOOLBAR_WIDGET, literal `${unsafeStatic(EDGELESS_TOOLBAR_WIDGET)}`);
