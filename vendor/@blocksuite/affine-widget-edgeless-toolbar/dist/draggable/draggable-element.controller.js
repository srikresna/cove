...
export class EdgelessDraggableElementController {
    ...
    _createOverlay({ x, y }) {
        ...
        const { elementInfo, elementRectOriginal, offsetPos, edgelessRect } = this.info;
        this.reset();
        ...
        const shapeName = elementInfo.data
            .name;
        const { fillColor, strokeColor } = edgeless.host.std.get(EditPropsStore).lastProps$.value[`shape:${shapeName}`] || {};
    ...
    _onDragEnd() {
        ...
        if (this.states.dragOut && !this.states.cancelled && overlay) {
            ...
            const [modelX, modelY] = this.gfx.viewport.toModelCoord(rect.left - edgelessRect.left, rect.top - edgelessRect.top);
    ...
    hostConnected() {
        this.host.disposables.add(this.gfx.viewport.viewportUpdated.subscribe(({ zoom }) => {
            this._updateOverlayScale(zoom);
        }));
