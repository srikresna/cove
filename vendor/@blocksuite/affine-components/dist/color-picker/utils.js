...
export const defaultPoint = (x = 0, y = 0) => ({ x, y });
export const defaultHsva = () => ({ ...rgbToHsv(FIRST_COLOR), a: 1 });
...
// Drops alpha value
export const keepColor = (color) => color.length > 7 && !color.endsWith('transparent')
    ? color.substring(0, 7)
    : color;
 ...
 * ```
 */
export const packColor = (key, color) => {
 ...
 */
export const packColorsWith = (colorScheme, value, oldColor) => {
    const colors = [
        { type: 'normal', value },
        { type: 'light', value },
        { type: 'dark', value },
...
export const calcCustomButtonStyle = (color, isCustomColor, ele) => {
    let b = 'transparent';
    let c = 'transparent';
    if (!isCustomColor) {
        return { '--b': b, '--c': c };
    ...
    if (color.startsWith('--')) {
        if (!color.endsWith('transparent')) {
            ...
            c = keepColor(rgbaToHex8(preprocessColor(window.getComputedStyle(ele))({
                type: 'normal',
                value: color,
            }).rgba));
        }
...
export const adjustColorAlpha = (color, a) => {
    let newColor;
