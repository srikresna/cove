...
import { autoUpdate, computePosition, flip, hide, inline, limitShift, offset, shift, size, } from '@floating-ui/dom';
...
export function autoUpdatePosition(signal, toolbar, referenceElement, flavour, placement, sideOptions, options = { elementResize: false }) {
    ...
    return autoUpdate(referenceElement, toolbar, () => {
        update().catch(console.error);
    }, { animationFrame: hasSurfaceScope, ...options });
}
export function combine(actions, context) {
...
function group(actions) {
    ...
    const paired = toPairs(grouped).map(([_, items]) => {
        ...
        if (others.length === 1)
            return merge({ ...first }, others[0]);
        return others.reduce(merge, { ...first });
    });
    return paired;
}
const merge = (a, b) => mergeWith(a, b, (obj, src) => Array.isArray(obj) ? group(obj.concat(src)) : src);
/**
 * Renders toolbar
 *
 ...
 * 4. `custom:affine:*`
 */
export function renderToolbar(toolbar, context, flavour) {
    const hasSurfaceScope = flavour.includes('surface');
    ...
    const actions = [
        ...
        .filter(module => typeof module.config.when === 'function'
    ...
    const [moreActionGroup, primaryActionGroup] = partition(ordered, a => a.placement === ActionPlacement.More);
    // Resets
    ...
    if (moreActionGroup.length) {
        ...
        if (moreMenuItems.length) {
            ...
            primaryActionGroup.push({
                id: 'more',
                content: html `${keyed(`${flavour}:${key}`, html `
            <editor-menu-button
    ...
    render(join(renderActions(primaryActionGroup, context), innerToolbar ? null : renderToolbarSeparator()), toolbar);
    // Avoids shaking
...
function renderActions(actions, context, render = renderActionItem) {
    return actions
        .map(action => {
        ...
        if ('actions' in action && action.actions.length) {
            ...
            return repeat(ordered, a => a.id, a => {
                if ('content' in a) {
...
function renderActionItem(action, context) {
    const innerToolbar = context.placement$.value === 'inner';
    const ids = action.id.split('.');
    const id = ids[ids.length - 1];
    const label = action.label ?? action.tooltip ?? id;
    const actived = typeof action.active === 'function'
...
function renderMenuActionItem(action, context) {
    const innerToolbar = context.placement$.value === 'inner';
    const ids = action.id.split('.');
    const id = ids[ids.length - 1];
    const label = action.label ?? action.tooltip ?? id;
