...
import { darkToolbarStyles, EditorToolbar, lightToolbarStyles, } from '@blocksuite/affine-components/toolbar';
import { CodeBlockModel, ImageBlockModel, ListBlockModel, ParagraphBlockModel, } from '@blocksuite/affine-model';
import { ToolbarContext, ToolbarFlag as Flag, ToolbarRegistryIdentifier, } from '@blocksuite/affine-shared/services';
import { unsafeCSSVar, unsafeCSSVarV2 } from '@blocksuite/affine-shared/theme';
import { matchModels } from '@blocksuite/affine-shared/utils';
import { IS_MOBILE } from '@blocksuite/global/env';
import { Bound, getCommonBound, getCommonBoundWithRotation, } from '@blocksuite/global/gfx';
import { nextTick } from '@blocksuite/global/utils';
import { BlockSelection, TextSelection, WidgetComponent, } from '@blocksuite/std';
import { GfxBlockElementModel, GfxPrimitiveElementModel, } from '@blocksuite/std/gfx';
...
export class AffineToolbarWidget extends WidgetComponent {
    constructor() {
        super(...arguments);
        this.sideOptions$ = signal(null);
        this.referenceElement$ = signal(null);
  ...
  `; }
    setReferenceElementWithRange(range) {
        this.referenceElement$.value = range
            ? () => ({
                getBoundingClientRect: () => range.getBoundingClientRect(),
                getClientRects: () => Array.from(range.getClientRects()).filter(rect => Math.round(rect.width)),
            })
            : null;
    }
    setReferenceElementWithHtmlElement(element) {
        this.referenceElement$.value = element ? () => element : null;
    }
    setReferenceElementWithBlocks(blocks) {
        let cachedClientRects = null;
    ...
    setReferenceElementWithElements(gfx, elements) {
    ...
    updateWithSurface(ctx, activated, elementIds) {
        const gfx = ctx.gfx;
        ...
        if (activated && surface) {
            ...
            const grouped = groupBy(elements.map(model => {
    ...
    connectedCallback() {
        ...
        const { sideOptions$, referenceElement$, disposables, toolbar, toolbarRegistry, host, std, } = this;
        ...
        if (IS_MOBILE) {
            ...
            if (window.visualViewport) {
                disposables.addFromEvent(window.visualViewport, 'resize', updateMobilePosition);
                disposables.addFromEvent(window.visualViewport, 'scroll', updateMobilePosition);
            }
            ...
            const syncMobileTextSelection = () => {
                ...
                const inEditor = Boolean(range && host.contains(range.commonAncestorContainer));
                batch(() => {
            ...
            const scheduleSyncMobileTextSelection = (delay) => {
                if (selectionTimeout)
                    clearTimeout(selectionTimeout);
                selectionTimeout = setTimeout(syncMobileTextSelection, delay);
            };
            const scheduleTouchSync = (delay) => {
                if (touchTimeout)
        ...
        disposables.add(std.selection.find$(TextSelection).subscribe(result => {
            ...
            const activated = Boolean(context.activated &&
                range &&
                result &&
                !result.isCollapsed() &&
        ...
        disposables.add(std.selection.filter$(BlockSelection).subscribe(selections => {
            ...
            if (activated) {
                ...
                if (block) {
                    ...
                    const existed = toolbarRegistry.modules.has(modelFlavour) ||
            ...
            batch(() => {
                ...
                if (!activated)
                    return;
                this.setReferenceElementWithBlocks(blockIds
                    .map(id => std.view.getBlock(id))
                    .filter(block => block !== null));
                sideOptions$.value = null;
                flavour$.value = flavour;
                placement$.value = toolbarRegistry.getModulePlacement(flavour, flavour === 'affine:note' ? 'top' : 'top-start');
                flags.refresh(Flag.Block);
            });
        }));
        // Selects elements in edgeless.
        // Triggered only when not in editing state.
        disposables.add(context.gfx.selection.slots.updated.subscribe(selections => {
        ...
        disposables.add(std.selection.slots.changed.subscribe(selections => {
            if (!context.activated)
                return;
            const value = flags.value$.peek();
            if (flags.contains(Flag.Hovering | Flag.Hiding, value))
                return;
            if (!flags.check(Flag.Text, value))
                return;
        ...
        disposables.add(std.view.viewUpdated.subscribe(record => {
        ...
        std.store.slots.blockUpdated.subscribe(record => {
            ...
            if (flags.isSurface()) {
                ...
                this.updateWithSurface(context, context.activated && Boolean(elementIds.length), elementIds);
                return;
            }
        }));
        // Handles elements when updating
        disposables.add(context.gfx.surface$.subscribe(surface => {
            if (!surface)
                return;
        ...
        disposables.addFromEvent(host, 'dragleave', throttle((event) => {
            ...
            if (x >= rect.left &&
                ...
                return;
            dragEnd();
        }, 144, { trailing: true }), eventOptions);
        ...
        disposables.add(message$.subscribe(data => {
            if (!context.activated ||
        ...
        disposables.add(context.theme.app$.subscribe(theme => {
        ...
        disposables.add(effect(() => {
            ...
            if (Flag.None === value || flags.check(Flag.Hiding, value)) {
                ...
                toolbar
                    .querySelector('editor-menu-button[data-open]')
                    ?.hide();
                return;
            }
        ...
        disposables.add(effect(() => {
            if (IS_MOBILE) {
                ...
                if (!context.activated)
                    return;
                if (Flag.None === value || flags.contains(Flag.Hiding, value))
                    return;
                updateMobilePosition?.();
                return;
            }
            ...
            const cleanup = autoUpdatePosition(signal, toolbar, referenceElement, flavour, placement, sideOptions);
