...
import { BlockMarkdownAdapterExtension, FOOTNOTE_DEFINITION_PREFIX, isFootnoteDefinitionNode, } from '@blocksuite/affine-shared/adapters';
const isRootNode = (node) => node.type === 'root';
const createFootnoteDefinition = (identifier, content) => ({
 ...
 */
const createNoteBlockMarkdownAdapterMatcher = (displayModeToSkip) => ({
    ...
    toBlockSnapshot: {
        enter: (o, context) => {
            ...
            const footnoteDefinitionIndex = noteAst.children.findIndex(child => isFootnoteDefinitionNode(child));
...
export const docNoteBlockMarkdownAdapterMatcher = createNoteBlockMarkdownAdapterMatcher(NoteDisplayMode.EdgelessOnly);
export const edgelessNoteBlockMarkdownAdapterMatcher = createNoteBlockMarkdownAdapterMatcher(NoteDisplayMode.DocOnly);
export const DocNoteBlockMarkdownAdapterExtension = BlockMarkdownAdapterExtension(docNoteBlockMarkdownAdapterMatcher);
export const EdgelessNoteBlockMarkdownAdapterExtension = BlockMarkdownAdapterExtension(edgelessNoteBlockMarkdownAdapterMatcher);
