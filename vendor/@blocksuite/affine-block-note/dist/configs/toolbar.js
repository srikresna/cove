...
import { NotificationProvider, SidebarExtensionIdentifier, ToolbarModuleExtension, } from '@blocksuite/affine-shared/services';
import { Bound } from '@blocksuite/global/gfx';
import { AutoHeightIcon, CustomizedHeightIcon, InsertIntoPageIcon, ScissorsIcon, } from '@blocksuite/icons/lit';
        ...
        },
        {
            ...
            generate(ctx) {
                ...
                if (!models.length)
                    return null;
                const firstModel = models[0];
                const shouldShowTooltip$ = computed(() => firstModel.props.displayMode$.value ===
                    NoteDisplayMode.DocAndEdgeless);
                const label$ = computed(() => firstModel.props.displayMode$.value === NoteDisplayMode.EdgelessOnly
                    ? 'Display in Page'
                    : 'Displayed in Page');
                const onSelect = () => {
                    const newMode = firstModel.props.displayMode === NoteDisplayMode.EdgelessOnly
                        ? NoteDisplayMode.DocAndEdgeless
...
function setDisplayMode(ctx, model, newMode) {
    ...
    const data = newMode === NoteDisplayMode.EdgelessOnly
        ? {
