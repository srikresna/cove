import { formatBlockCommand, textFormatConfigs, } from '@blocksuite/affine-inline-preset';
import { textAlignConfigs, textConversionConfigs, } from '@blocksuite/affine-rich-text';
import { getSelectedModelsCommand, getTextSelectionCommand, } from '@blocksuite/affine-shared/commands';
...
const noteSlashMenuConfig = {
    items: [
        ...
        ...textConversionConfigs
            ...
            when: ({ model }) => model.store.schema.flavourSchemaMap.has(config.flavour) &&
...
function createConversionItem(config, group) {
...
function createAlignItem(config, group) {
...
function createTextFormatItem(config, group) {
...
export const NoteSlashMenuConfigExtension = SlashMenuConfigExtension('affine:note', noteSlashMenuConfig);
