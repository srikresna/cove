...
import { BlockMarkdownAdapterExtension, CALLOUT_MARKDOWN_EXPORT_OPTIONS_KEY, CalloutAdmonitionTypeSet, CalloutExportStyle, calloutMarkdownExportOptionsSchema, DEFAULT_ADMONITION_TYPE, getCalloutEmoji, isCalloutNode, } from '@blocksuite/affine-shared/adapters';
 ...
 */
function getCalloutExportOptions(configs) {
    let exportOptions = DEFAULT_OPTIONS;
    try {
...
export const calloutBlockMarkdownAdapterMatcher = {
    ...
    toBlockSnapshot: {
        enter: (o, context) => {
            ...
            o.node.children = o.node.children.filter(child => CALLOUT_BLOCK_CHILDREN_TYPES.has(child.type));
    ...
    fromBlockSnapshot: {
        enter: (o, context) => {
            const emoji = o.node.props.emoji;
            ...
            if (style === CalloutExportStyle.Admonitions) {
                ...
                if (isTextNode) {
                    const textDelta = (firstChild.props.text ?? { delta: [] });
                ...
                const admonitionSymbol = `${ADMONITION_SYMBOL} ${type} ${customTitle}`.trim();
                walkerContext
                    .openNode({
                    type: 'paragraph',
                    children: [
...
export const CalloutBlockMarkdownAdapterExtension = BlockMarkdownAdapterExtension(calloutBlockMarkdownAdapterMatcher);
