...
import { ActionPlacement, IconPickerServiceIdentifier, ToolbarModuleExtension, } from '@blocksuite/affine-shared/services';
...
const iconPickerAction = {
    ...
    content(ctx) {
        ...
        if (!model)
            return null;
        const handleIconPickerClick = (event) => {
            // Get IconPickerService from the framework
            const iconPickerService = ctx.std.getOptional(IconPickerServiceIdentifier);
