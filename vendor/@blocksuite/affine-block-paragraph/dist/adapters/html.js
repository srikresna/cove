...
const tagsInAncestor = (o, tagNames) => {
    let parent = o.parent;
    while (parent) {
        if (HastUtils.isElement(parent.node) &&
...
const splitDeltaByNewline = (delta) => {
    const lines = [[]];
    const pending = [...delta];
...
const hasBlockElementDescendant = (node) => {
    ...
    return node.children.some(child => {
        ...
        return ((HastUtils.isTagBlock(child.tagName) && child.tagName !== 'br') ||
            hasBlockElementDescendant(child));
    });
};
const getParagraphDeltas = (node, delta) => {
    if (!HastUtils.isElement(node))
        return [delta];
    if (hasBlockElementDescendant(node))
        return [delta];
    const hasBr = !!HastUtils.querySelector(node, 'br');
    if (!hasBr)
        return [delta];
    const hasNewline = delta.some(op => typeof op.insert === 'string' && op.insert.includes('\n'));
    if (!hasNewline)
        return [delta];
    return splitDeltaByNewline(delta);
};
const openParagraphBlocks = (deltas, type, 
...
const MULTI_PARAGRAPH_EMITTED_NODES_CONTEXT_KEY = 'affine:paragraph:multi-emitted-nodes';
const markMultiParagraphEmitted = (walkerContext, node) => {
    const emittedNodes = walkerContext.getGlobalContext(MULTI_PARAGRAPH_EMITTED_NODES_CONTEXT_KEY) ?? new WeakSet();
    emittedNodes.add(node);
    walkerContext.setGlobalContext(MULTI_PARAGRAPH_EMITTED_NODES_CONTEXT_KEY, emittedNodes);
};
const consumeMultiParagraphEmittedMark = (walkerContext, node) => {
    const emittedNodes = walkerContext.getGlobalContext(MULTI_PARAGRAPH_EMITTED_NODES_CONTEXT_KEY);
    if (!emittedNodes) {
        return false;
    }
    return emittedNodes.delete(node);
};
export const paragraphBlockHtmlAdapterMatcher = {
...
export const ParagraphBlockHtmlAdapterExtension = BlockHtmlAdapterExtension(paragraphBlockHtmlAdapterMatcher);
