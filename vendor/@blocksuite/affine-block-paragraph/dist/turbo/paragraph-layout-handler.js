...
export class ParagraphLayoutHandlerExtension extends BlockLayoutHandlerExtension {
    constructor() {
        super(...arguments);
        this.blockType = 'affine:paragraph';
    }
    static setup(di) {
        di.addImpl(BlockLayoutHandlersIdentifier('paragraph'), ParagraphLayoutHandlerExtension);
    }
    queryLayout(model, host, viewportRecord) {
        const component = host.std.view.getBlock(model.id);
        if (!component)
            return null;
        const paragraphSelector = '.affine-paragraph-rich-text-wrapper [data-v-text="true"]';
