...
class ParagraphLayoutPainter {
    ...
    paint(ctx, layout, layoutBaseX, layoutBaseY) {
        ...
        if (!isParagraphLayout(layout)) {
            console.warn('Expected paragraph layout but received different format:', layout);
            return;
        }
        ...
        layout.sentences.forEach(sentence => {
            ...
            if (fontSize !== 15)
                return; // TODO: fine-tune for heading font sizes
...
export const ParagraphLayoutPainterExtension = BlockLayoutPainterExtension('affine:paragraph', ParagraphLayoutPainter);
