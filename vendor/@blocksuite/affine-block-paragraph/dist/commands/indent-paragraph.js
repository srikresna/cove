...
import { calculateCollapsedSiblings, getNearestHeadingBefore, matchModels, } from '@blocksuite/affine-shared/utils';
import { TextSelection } from '@blocksuite/std';
export const canIndentParagraphCommand = (cxt, next) => {
...
export const indentParagraphCommand = (ctx, next) => {
    ...
    if (!indentContext ||
        ...
        console.warn('you need to use `canIndentParagraph` command before running `indentParagraph` command');
        return;
    }
    ...
    if (matchModels(previousSibling, [ListBlockModel]) &&
