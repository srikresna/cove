import { AttachmentBlockModel, BookmarkBlockModel, CalloutBlockModel, CodeBlockModel, DatabaseBlockModel, DividerBlockModel, EdgelessTextBlockModel, ImageBlockModel, ListBlockModel, NoteBlockModel, ParagraphBlockModel, } from '@blocksuite/affine-model';
import { asyncSetInlineRange, focusTextModel, } from '@blocksuite/affine-rich-text';
import { EMBED_BLOCK_MODEL_LIST } from '@blocksuite/affine-shared/consts';
import { focusTitle, getDocTitleInlineEditor, getPrevContentBlock, matchModels, } from '@blocksuite/affine-shared/utils';
 ...
 *   - line3
 */
export function mergeWithPrev(editorHost, model) {
    ...
    if (matchModels(prevBlock, [ParagraphBlockModel, ListBlockModel])) {
        ...
        prevBlock.props.text.join(model.text);
...
function handleNoPreviousSibling(editorHost, model) {
    ...
    if (matchModels(parent, [NoteBlockModel])) {
        ...
        const rootModel = model.store.root;
