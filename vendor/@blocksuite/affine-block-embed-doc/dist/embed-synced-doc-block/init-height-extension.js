...
export class HeightInitializationExtension extends LifeCycleWatcher {
    constructor() {
        super(...arguments);
        this._initQueue = new Set();
        this._disposables = new DisposableGroup();
    }
    static { this.key = 'embed-synced-doc-block-height-initialization'; }
    mounted() {
        super.mounted();
        this._disposables.add(this.std.store.slots.blockUpdated.subscribe(payload => {
            if (payload.type === 'add' &&
                payload.isLocal &&
        ...
        this._disposables.add(this.std.view.viewUpdated.subscribe(payload => {
