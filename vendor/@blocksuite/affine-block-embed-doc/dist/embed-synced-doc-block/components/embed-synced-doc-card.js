...
import { BlockSelection, isGfxBlockComponent, ShadowlessElement, } from '@blocksuite/std';
...
let EmbedSyncedDocCard = (() => {
    ...
    return class EmbedSyncedDocCard extends _classSuper {
        static {
        ...
        connectedCallback() {
            ...
            if (isCycle && syncedDoc) {
                ...
                this.disposables.add(syncedDoc.workspace.slots.docListUpdated.subscribe(() => {
                    renderLinkedDocInCard(this);
                }));
                ...
                this.disposables.add(syncedDoc.slots.blockUpdated.subscribe(throttle(payload => {
                    if (this._dragging) {
                        return;
                    }
        ...
        render() {
            ...
            const { SyncedDocErrorIcon, SyncedDocEmptyBanner, SyncedDocErrorBanner, SyncedDocDeletedBanner, } = getSyncedDocIcons(theme, this.editorMode);
            const icon = error
                ? SyncedDocErrorIcon
                : isLoading
                    ? LoadingIcon()
