...
import { DEFAULT_NOTE_HEIGHT, DEFAULT_NOTE_WIDTH, EmbedSyncedDocModel, NoteBlockModel, NoteDisplayMode, } from '@blocksuite/affine-model';
import { draftSelectedModelsCommand, duplicateSelectedModelsCommand, } from '@blocksuite/affine-shared/commands';
import { REFERENCE_NODE } from '@blocksuite/affine-shared/consts';
import { ActionPlacement, EditorSettingProvider, ToolbarModuleExtension, } from '@blocksuite/affine-shared/services';
import { getBlockProps, matchModels } from '@blocksuite/affine-shared/utils';
import { Bound } from '@blocksuite/global/gfx';
import { CaptionIcon, CopyIcon, DeleteIcon, DuplicateIcon, ExpandFullIcon, InsertIntoPageIcon, OpenInNewIcon, } from '@blocksuite/icons/lit';
        ...
        },
        {
            ...
            run(ctx) {
                ...
                if (isGfxBlockComponent(block)) {
                    ...
                    editorSetting?.set?.('docCanvasPreferView', 'affine:embed-linked-doc');
                }
                block?.convertToCard();
                ctx.track('SelectedView', {
                    ...trackBaseProps,
        ...
        conversionsActionGroup,
        {
            ...
            run: ctx => {
                ...
                const lastVisibleNote = ctx.store
                    ...
                    .findLast((note) => matchModels(note, [NoteBlockModel]) &&
        ...
        },
        {
            ...
            run: ctx => {
                ...
                let contentModels = [];
                {
                    ...
                    contentModels = store
                        .getModelsByFlavour('affine:note')
                        .filter((note) => matchModels(note, [NoteBlockModel]) &&
                        note.props.displayMode !== NoteDisplayMode.EdgelessOnly)
                ...
                ctx.chain
                    ...
                    (async () => {
                        ...
                        await std.clipboard.duplicateSlice(Slice.fromModels(std.store, children), std.store, noteId);
                        gfx.selection.set({
                            elements: [noteId],
                            editing: false,
                        });
