...
import { ImageBlockModel, ListBlockModel, ParagraphBlockModel, } from '@blocksuite/affine-model';
...
export function renderLinkedDocInCard(card) {
    ...
    if (!linkedDoc) {
        console.error(`Trying to load page ${card.model.props.pageId} in linked page block, but the page is not found.`);
        return;
    }
...
async function renderPageAsBanner(card) {
    const linkedDoc = card.linkedDoc;
    if (!linkedDoc) {
        console.error(`Trying to load page ${card.model.props.pageId} in linked page block, but the page is not found.`);
        return;
    ...
    const target = notes.flatMap(note => note.children.filter(child => matchModels(child, [ImageBlockModel])))[0];
    if (target) {
...
async function renderImageAsBanner(card, image) {
