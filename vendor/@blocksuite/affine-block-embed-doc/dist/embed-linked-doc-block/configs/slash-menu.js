...
const linkedDocSlashMenuConfig = {
    items: [
        {
            name: 'New Doc',
        ...
        },
        {
            ...
            when: ({ std, model }) => {
                ...
                if (!root)
                    return false;
                const linkedDocWidget = std.view.getWidget('affine-linked-doc-widget', root.id);
                if (!linkedDocWidget)
                    return false;
...
export const LinkedDocSlashMenuConfigIdentifier = SlashMenuConfigIdentifier(EmbedLinkedDocBlockSchema.model.flavour);
export const LinkedDocSlashMenuConfigExtension = {
    setup: di => {
