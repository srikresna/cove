...
import { EmbedLinkedDocModel, EmbedLinkedDocStyles, } from '@blocksuite/affine-model';
import { EMBED_CARD_HEIGHT, EMBED_CARD_WIDTH, } from '@blocksuite/affine-shared/consts';
import { ActionPlacement, DocDisplayMetaProvider, EditorSettingProvider, ToolbarModuleExtension, } from '@blocksuite/affine-shared/services';
import { getBlockProps, referenceToNode, } from '@blocksuite/affine-shared/utils';
import { Bound } from '@blocksuite/global/gfx';
import { CaptionIcon, CopyIcon, DeleteIcon, DuplicateIcon, ExpandFullIcon, OpenInNewIcon, } from '@blocksuite/icons/lit';
...
const docTitleAction = {
    ...
    content(ctx) {
        ...
        if (!model.props.title)
            return null;
        const originalTitle = ctx.std.get(DocDisplayMetaProvider).title(model.props.pageId).value ||
            'Untitled';
        const open = (event) => block.open({ event });
...
const openDocActionGroup = {
    ...
    content(ctx) {
        ...
        if (!block)
            return null;
        const actions = openDocActions.map(action => {
      ...
      >
        <div data-size="small" data-orientation="vertical">
          ${repeat(actions, action => action.id, ({ label, icon, run, disabled }) => html `
              <editor-menu-action
    ...
    actions: [
        ...
        },
        {
            ...
            disabled(ctx) {
                ...
                if (!block)
                    return true;
                if (block.closest('affine-embed-synced-doc-block'))
                    return true;
                const model = block.model;
                // same doc
                if (model.props.pageId === ctx.store.id)
                    return true;
                // linking to block
                if (referenceToNode(model.props))
                    return true;
                return false;
            },
            run(ctx) {
                ...
                if (isGfxBlockComponent(block)) {
                    ...
                    editorSetting?.set?.('docCanvasPreferView', 'affine:embed-synced-doc');
                }
