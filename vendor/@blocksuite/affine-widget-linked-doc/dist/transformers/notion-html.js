import { defaultImageProxyMiddleware, NotionHtmlAdapter, } from '@blocksuite/affine-shared/adapters';
...
function buildFolderHierarchy(pagePaths) {
    const root = {
        name: '',
        path: '',
        children: new Map(),
...
function toImportIconData(icon) {
    if (!icon)
        return undefined;
    if (icon.type === 'emoji') {
        return {
...
async function planNotionHtmlZip({ collection, schema, imported, extensions, }) {
    ...
    const parseZipFile = async (path) => {
        ...
        for (const { path, content, index } of unzip) {
            ...
            if (index === 0 && fileName.endsWith('.csv')) {
                warnings.push({
                    ...
                    message: 'The imported Notion export appears to be CSV instead of HTML.',
                    sourcePath: path,
                });
                continue;
            }
        ...
        promises.push(blobsFromAssets(pendingAssets, pendingPathBlobIdMap).then(importBlobs => {
    ...
    const folderHierarchy = pagePathsWithIds.length > 0
