import { defaultImageProxyMiddleware, docLinkBaseURLMiddleware, fileNameMiddleware, filePathMiddleware, FULL_FILE_PATH_KEY, getImageFullPath, MarkdownAdapter, MarkdownASTToDeltaExtension, normalizeFilePathReference, titleMiddleware, } from '@blocksuite/affine-shared/adapters';
...
function isLocalMarkdownDocLink(url) {
...
function getMarkdownZipPageIdConfigKey(path) {
    return `${MARKDOWN_ZIP_PAGE_ID_CONFIG_PREFIX}${normalizeMarkdownZipLookupPath(path)}`;
}
function getMarkdownZipTargetPageId(configs, currentFilePath, url) {
...
const markdownZipDocLinkToDeltaMatcher = MarkdownASTToDeltaExtension({
    ...
    toDelta: (ast, context) => {
        ...
        const targetPageId = typeof currentFilePath === 'string'
...
function buildMetaFromFrontmatter(data) {
    const meta = {};
...
export function applyMetaPatch(collection, docId, meta) {
    const metaPatch = {};
    if (meta.title)
        metaPatch.title = meta.title;
    if (meta.createDate !== undefined)
        metaPatch.createDate = meta.createDate;
    if (meta.updatedDate !== undefined)
        metaPatch.updatedDate = meta.updatedDate;
    if (meta.tags)
        metaPatch.tags = meta.tags;
    if (meta.favorite !== undefined)
        metaPatch.favorite = meta.favorite;
    if (meta.trash !== undefined)
        metaPatch.trash = meta.trash;
    if (Object.keys(metaPatch).length) {
        collection.meta.setDocMeta(docId, metaPatch);
    }
}
export function getProvider(extensions) {
...
function parseNotionMarkdownTitle(markdown) {
...
function prepareDefaultMarkdownFile({ filename, markdown, }) {
...
function prepareNotionMarkdownFile({ filename, markdown, }) {
...
function applySnapshotTitle(snapshot, title) {
 ...
 */
export function createMarkdownImportJob({ collection, schema, preferredTitle, fullPath, }) {
 ...
 */
export async function stageImportedAsset({ pendingAssets, pendingPathBlobIdMap, path, content, fileName, }) {
 ...
 */
export function bindImportedAssetsToJob(job, pendingAssets, pendingPathBlobIdMap) {
...
function bindImportedMarkdownPagesToJob(job, pagePathIdMap) {
    for (const [path, pageId] of pagePathIdMap.entries()) {
        job.adapterConfigs.set(getMarkdownZipPageIdConfigKey(path), pageId);
    }
}
function registerMarkdownZipPagePath(pagePathIdMap, path, pageId) {
 ...
 */
async function importMarkdownToBlock({ doc, markdown, blockId, extensions, }) {
    ...
    if (!snapshot) {
        throw new BlockSuiteError(BlockSuiteError.ErrorCode.ValueNotExists, 'import markdown failed, expected to get a snapshot');
    }
 ...
 */
async function importMarkdownToDoc({ collection, schema, markdown, fileName, extensions, }) {
    const { content, meta } = parseFrontmatter(markdown);
...
async function planMarkdownZip({ collection, schema, imported, extensions, }) {
...
async function planNotionMarkdownZip({ collection, schema, imported, extensions, }) {
    return planMarkdownZipInternal({
        collection,
        schema,
        imported,
...
async function planMarkdownZipInternal({ collection, schema, imported, extensions, createRootFolderForTopLevelDocs = false, normalizeFolderName, prepareMarkdownFile = prepareDefaultMarkdownFile, preserveCommonRoot = false, recursiveZip = false, }) {
    ...
    const docPathMap = [];
    const pendingPagePathIdMap = new Map();
    const markdownBlobs = [];
    async function collectZipEntries(zipBlob, basePath = '') {
    ...
    const folderHierarchy = buildMarkdownZipFolderHierarchy(docPathMap, normalizeFolderName, preserveCommonRoot, createRootFolderForTopLevelDocs);
    return {
        docIds,
        folderHierarchy,
        batch: {
 ...
 */
function buildMarkdownZipFolderHierarchy(entries, normalizeFolderName, preserveCommonRoot = false, createRootFolderForTopLevelDocs = false) {
    if (entries.length === 0)
        return undefined;
    // Check once whether all entries share a common root directory
    const candidateRoot = entries[0]?.fullPath.split('/').find(Boolean);
    const skipRoot = !preserveCommonRoot &&
        !!candidateRoot &&
    ...
    const hasSubfolders = entries.some(e => {
        ...
        return (folderParts.length > 0 || (createRootFolderForTopLevelDocs && !!fileName));
    });
    if (!hasSubfolders) {
...
function flattenFolderHierarchy(root) {
    const folders = [];
    const visit = (node) => {
        if (node.name) {
            folders.push({
