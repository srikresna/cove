import { replaceIdMiddleware, titleMiddleware, } from '@blocksuite/affine-shared/adapters';
import { sha } from '@blocksuite/global/utils';
import { extMimeMap, getAssetName, Transformer } from '@blocksuite/store';
import { download, Unzip, Zip } from './utils.js';
async function exportDocs(collection, schema, docs) {
    const zip = new Zip();
    ...
    const results = await Promise.all(Array.from(pathBlobIdMap.values()).map(async (blobId) => {
        try {
...
async function importDocs(collection, schema, imported) {
    const unzip = new Unzip();
    await unzip.load(imported);
    const assetBlobs = [];
    const snapshotsBlobs = [];
    ...
    return Promise.all(snapshotsBlobs.map(async (blob) => {
