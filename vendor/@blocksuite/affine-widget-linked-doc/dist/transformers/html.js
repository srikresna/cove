import { defaultImageProxyMiddleware, docLinkBaseURLMiddleware, fileNameMiddleware, filePathMiddleware, HtmlAdapter, titleMiddleware, } from '@blocksuite/affine-shared/adapters';
 ...
 */
async function importHTMLToDoc({ collection, schema, html, fileName, extensions, }) {
 ...
 */
async function importHTMLZip({ collection, schema, imported, extensions, }) {
    const provider = getProvider(extensions);
    ...
    await Promise.all(htmlBlobs.map(async (htmlFile) => {
