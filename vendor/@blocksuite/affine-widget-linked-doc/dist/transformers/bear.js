import { defaultImageProxyMiddleware, docLinkBaseURLMiddleware, fileNameMiddleware, filePathMiddleware, MarkdownAdapter, } from '@blocksuite/affine-shared/adapters';
...
function getProvider(extensions) {
 ...
 */
function parseBearTags(markdown) {
    const lines = markdown.split('\n');
    const codeFenceState = [];
...
function extractTagsFromLine(line) {
    ...
    while (remaining.length > 0) {
        ...
        if (remaining.startsWith('#')) {
            ...
            const openMatch = remaining.match(/^#([\p{L}\p{N}_][\p{L}\p{N}_/-]*)(.*)$/u);
            if (openMatch) {
 ...
 */
function buildFolderHierarchyFromTags(tagDocMap) {
    const root = {
        name: '',
        path: '',
        children: new Map(),
...
function tagsToBatchTags(tags) {
    return Array.from(tags, ([name, docIds]) => ({ name, docIds }));
}
const GFM_CALLOUT_MAP = {
    IMPORTANT: '\u26A0',
    NOTE: '\uD83D\uDCDD',
    WARNING: '\u26A0',
    TIP: '\uD83D\uDCA1',
 ...
 */
function convertGfmCallouts(markdown) {
    const lines = markdown.split('\n');
    let inCodeBlock = false;
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!inCodeBlock) {
            lines[i] = lines[i].replace(/^(>\s*)\[!(\w+)\]/, (match, prefix, type) => {
...
function stripBearMetadataComments(markdown) {
...
function convertHighlights(markdown) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!inCodeBlock) {
            lines[i] = lines[i].replace(/==(\S(?:[^=]|=[^=])*?)==/g, (...[, content]) => {
...
function extractTitle(markdown, bundleName) {
    const lines = markdown.split('\n');
    let inCodeBlock = false;
    for (const line of lines) {
        if (line.trimStart().startsWith('```')) {
 ...
 */
async function planBearBackup({ collection, schema, imported, extensions, }) {
    ...
    for (const entry of bundleMap.values()) {
        ...
        const bearMeta = info['net.shinyfrog.bear'];
        if (bearMeta?.trashed === 1)
            continue;
        validBundles.push({ entry, bearMeta });
    }
    if (validBundles.length === 0) {
        throw new Error('No valid Bear textbundles found in the archive. Please select a .bear2bk backup file.');
    }
    const docIds = [];
    const docs = [];
    const importBlobs = new Map();
    const warnings = [];
    const tagDocMap = new Map();
    ...
    for (const { entry, bearMeta } of validBundles) {
        try {
            ...
            const bundleDirName = entry.bundlePath.split('/').findLast(Boolean) ?? 'Untitled';
            const title = extractTitle(cleanedMarkdown, bundleDirName);
            const markdown = convertHighlights(convertGfmCallouts(stripBearMetadataComments(cleanedMarkdown)));
            ...
            for (const blob of await blobsFromAssets(pendingAssets, pendingPathBlobIdMap)) {
