...
let RichText = (() => {
    ...
    let _markdownMatches_decorators;
    let _markdownMatches_initializers = [];
    let _markdownMatches_extraInitializers = [];
    ...
    return class RichText extends _classSuper {
        static {
            ...
            _markdownMatches_decorators = [property({ attribute: false })];
            ...
            __esDecorate(this, null, _markdownMatches_decorators, { kind: "accessor", name: "markdownMatches", static: false, private: false, access: { has: obj => "markdownMatches" in obj, get: obj => obj.markdownMatches, set: (obj, value) => { obj.markdownMatches = value; } }, metadata: _metadata }, _markdownMatches_initializers, _markdownMatches_extraInitializers);
  ...
  `; }
        ...
        _init() {
            ...
            // init inline editor
            this._inlineEditor$.value = new InlineEditor(this._yText, {
            ...
            if (markdownMatches) {
                const markdownTransform = (isEnter = false) => {
                ...
                inlineEditor.disposables.add(inlineEditor.slots.inputting.subscribe(data => {
                    if (!inlineEditor.isComposing && data === ' ') {
                        markdownTransform();
                    }
                }));
                inlineEditor.disposables.add(inlineEditor.slots.keydown.subscribe(event => {
            ...
            // init auto scroll
            inlineEditor.disposables.add(effect(() => {
                ...
                inlineEditor
                    .waitForUpdate()
                    .then(() => {
                    if (!inlineEditor.mounted || inlineEditor.rendering)
                        return;
                    const range = inlineEditor.toDomRange(inlineRange);
                    if (!range)
                        return;
                    // Cove: the upstream vertical caret auto-scroll is removed.
                    // Our scroll container is outside the editor (the app's
                    // doc scroller), so its rect math compared caret positions
                    // against the wrong viewport and repeatedly fought itself
                    // - clicking a block near the bottom made the page glide
                    // down and snap back in a loop. The app layer follows the
                    // caret with minimal one-way scrolling instead.
    ...
    ></div>`;
        ...
        #markdownMatches_accessor_storage;
        get markdownMatches() { return this.#markdownMatches_accessor_storage; }
        set markdownMatches(value) { this.#markdownMatches_accessor_storage = value; }
        ...
        constructor() {
            ...
            this._inlineEditor$ = signal(null);
            this._onCopy = (e) => {
                const inlineEditor = this.inlineEditor;
                if (!inlineEditor)
                    return;
            ...
            this.#markdownMatches_accessor_storage = (__runInitializers(this, _inlineRangeProvider_extraInitializers), __runInitializers(this, _markdownMatches_initializers, []));
            this.#readonly_accessor_storage = (__runInitializers(this, _markdownMatches_extraInitializers), __runInitializers(this, _readonly_initializers, false));
