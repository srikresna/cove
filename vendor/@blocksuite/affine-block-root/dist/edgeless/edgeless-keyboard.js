...
import { DefaultTool, EdgelessLegacySlotIdentifier, isNoteBlock, } from '@blocksuite/affine-block-surface';
import { toast } from '@blocksuite/affine-components/toast';
import { BrushTool, EraserTool, HighlighterTool, } from '@blocksuite/affine-gfx-brush';
import { ConnectorTool, mountConnectorLabelEditor, } from '@blocksuite/affine-gfx-connector';
import { createGroupFromSelectedCommand, ungroupCommand, } from '@blocksuite/affine-gfx-group';
import { getNearestTranslation, isElementOutsideViewport, isSingleMindMapNode, } from '@blocksuite/affine-gfx-mindmap';
...
import { ConnectorElementModel, EdgelessTextBlockModel, GroupElementModel, LayoutType, MindmapElementModel, NoteBlockModel, NoteDisplayMode, } from '@blocksuite/affine-model';
...
export class EdgelessPageKeyboardManager extends PageKeyboardManager {
    ...
    constructor(rootComponent) {
        ...
        this.rootComponent.bindHotKey({
            ...
            c: () => {
                ...
                let mode;
                if (this.gfx.tool.currentToolName$.peek() === ConnectorTool.toolName) {
            ...
            k: () => {
                if (this.rootComponent.service.locked)
                    return;
                ...
                if (selection.selectedElements.length === 1 &&
                    ...
                    matchModels(selection.firstElement, [
                        NoteBlockModel,
                    ])) {
            ...
            f: () => {
                if (this.rootComponent.service.locked)
                    return;
                if (this.rootComponent.service.selection.selectedElements.length !==
                    0 &&
            ...
            '-': () => {
                if (this.rootComponent.service.locked)
                    return;
                const { selectedElements: elements } = rootComponent.service.selection;
                if (!rootComponent.service.selection.editing &&
                    elements.length === 1 &&
            ...
            '@': () => {
                ...
                const [_, { insertedLinkType }] = std.command.exec(insertLinkByQuickSearchCommand);
                insertedLinkType
                    ?.then(type => {
            ...
            'Mod-g': ctx => {
                if (this.rootComponent.service.locked)
                    return;
                if (this.rootComponent.service.selection.selectedElements.length > 1 &&
                    !this.rootComponent.service.selection.editing) {
            ...
            'Alt-2': ctx => {
                ...
                this.gfx.viewport.setViewportByBound(bound, [0.12, 0.12, 0.12, 0.12], true);
            },
            'Mod-=': ctx => {
                ctx.get('defaultState').event.preventDefault();
            ...
            Enter: () => {
                ...
                const target = service.crud.getElementById(id);
                requestAnimationFrame(() => {
    ...
    _bindToggleHand() {
        this.rootComponent.handleEvent('keyDown', ctx => {
        ...
        this.rootComponent.handleEvent('keyUp', ctx => {
            const event = ctx.get('keyboardState').raw;
            if (event.code === 'Space' && !event.repeat) {
                this._space(event);
            }
    ...
    _delete() {
        ...
        if (isSingleMindMapNode(selectedElements)) {
            ...
            const focusNode = mindmap.getSiblingNode(node.id, 'prev') ??
    ...
    _move(key, shift = false) {
        ...
        const movedElements = new Set([
            ...selectedElements,
            ...selectedElements.flatMap(el => isGfxGroupCompatibleModel(el) ? el.descendantElements : []),
        ]);
    ...
    _setEdgelessTool(toolType, ...options) {
        const ignoreActiveState = typeof options === 'boolean'
            ? options[0]
            : options[1] === undefined
                ? false
                : options[1];
        ...
        this.rootComponent.gfx.tool.setTool(toolType, options[0] !== undefined && typeof options[0] !== 'boolean'
            ? options[0]
            : undefined);
    }
    _space(event) {
      ...
      */
        ...
        const revertToPrevTool = (ev) => {
            if (ev.code === 'Space') {
                const toolConstructor = currentTool.constructor;
        ...
        if (isKeyDown) {
            ...
            this.std.event.disposables.addFromEvent(document, 'keyup', revertToPrevTool);
        }
    }
}
