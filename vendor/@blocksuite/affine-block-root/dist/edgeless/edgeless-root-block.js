...
import { DefaultTool, getBgGridGap, normalizeWheelDeltaY, } from '@blocksuite/affine-block-surface';
...
import { EditorSettingProvider, EditPropsStore, FontLoaderService, ThemeProvider, ViewportElementProvider, } from '@blocksuite/affine-shared/services';
import { isTouchPadPinchEvent, matchModels, requestConnectedFrame, requestThrottledConnectedFrame, } from '@blocksuite/affine-shared/utils';
...
let EdgelessRootBlockComponent = (() => {
  ...
  `; }
        ...
        _initLayerUpdateEffect() {
            ...
            this._disposables.add(this.gfx.layer.slots.layerUpdated.subscribe(() => updateLayers()));
        }
        _initPanEvent() {
            this.disposables.add(this.std.event.add('pan', ctx => {
                const { viewport } = this.gfx;
                if (viewport.locked)
                    return;
                const multiPointersState = ctx.get('multiPointerState');
                const [p1, p2] = multiPointersState.pointers;
                const dx = (0.25 * (p1.delta.x + p2.delta.x)) /
                    viewport.zoom /
                    viewport.viewScale;
                const dy = (0.25 * (p1.delta.y + p2.delta.y)) /
                    viewport.zoom /
                    viewport.viewScale;
        ...
        _initPinchEvent() {
            this.disposables.add(this.std.event.add('pinch', ctx => {
                const { viewport } = this.gfx;
                if (viewport.locked)
                    return;
                const multiPointersState = ctx.get('multiPointerState');
        ...
        _initViewport() {
            const { std, gfx } = this;
            const run = () => {
                const animationFn = std.getOptional(NoteConfigExtension.identifier)?.pageBlockViewportFitAnimation;
                if (animationFn) {
                    const pageBlock = this.model.children.find((child) => matchModels(child, [NoteBlockModel]) && child.isPageBlock());
        ...
        _initWheelEvent() {
            this._disposables.add(this.std.event.add('wheel', ctx => {
