import { autoArrangeElementsCommand, autoResizeElementsCommand, EdgelessCRUDIdentifier, updateXYWH, } from '@blocksuite/affine-block-surface';
import { EditorChevronDown } from '@blocksuite/affine-components/toolbar';
import { renderMenuItems } from '@blocksuite/affine-widget-edgeless-toolbar';
import { Bound } from '@blocksuite/global/gfx';
import { AlignBottomIcon, AlignHorizontalCenterIcon, AlignLeftIcon, AlignRightIcon, AlignTopIcon, AlignVerticalCenterIcon, AutoTidyUpIcon, DistributeHorizontalIcon, DistributeVerticalIcon, ResizeTidyUpIcon, } from '@blocksuite/icons/lit';
import { html } from 'lit';
import { styleMap } from 'lit/directives/style-map.js';
var Alignment;
(function (Alignment) {
    Alignment[Alignment["None"] = 0] = "None";
    Alignment[Alignment["AutoArrange"] = 1] = "AutoArrange";
    Alignment[Alignment["AutoResize"] = 2] = "AutoResize";
    Alignment[Alignment["Bottom"] = 3] = "Bottom";
    Alignment[Alignment["DistributeHorizontally"] = 4] = "DistributeHorizontally";
    Alignment[Alignment["DistributeVertically"] = 5] = "DistributeVertically";
    Alignment[Alignment["Horizontally"] = 6] = "Horizontally";
    Alignment[Alignment["Left"] = 7] = "Left";
    Alignment[Alignment["Right"] = 8] = "Right";
    Alignment[Alignment["Top"] = 9] = "Top";
    Alignment[Alignment["Vertically"] = 10] = "Vertically";
})(Alignment || (Alignment = {}));
...
const alignment = {
    ...
    [Alignment.Horizontally](ctx, elements) {
        const bounds = elements.map(a => a.elementBound);
        const left = Math.min(...bounds.map(b => b.minX));
    ...
    [Alignment.DistributeHorizontally](ctx, elements) {
    ...
    [Alignment.Vertically](ctx, elements) {
        const bounds = elements.map(a => a.elementBound);
        const top = Math.min(...bounds.map(b => b.minY));
    ...
    [Alignment.DistributeVertically](ctx, elements) {
    ...
    // Auto
    [Alignment.AutoArrange](ctx) {
        ctx.command.exec(autoArrangeElementsCommand);
    },
    [Alignment.AutoResize](ctx) {
        ctx.command.exec(autoResizeElementsCommand);
    },
};
const updateXYWHWith = (ctx, model, bound) => {
    updateXYWH(model, bound, ctx.std.get(EdgelessCRUDIdentifier).updateElement, ctx.store.updateBlock);
};
export function renderAlignmentMenu(ctx, models, { label, tooltip, icon }, onPick = (type) => alignment[type](ctx, models)) {
    return html `
    <editor-menu-button
