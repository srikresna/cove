...
import { isExternalEmbedBlockComponent, notifyDocCreated, promptDocTitle, } from '@blocksuite/affine-block-embed';
import { EdgelessFrameManagerIdentifier } from '@blocksuite/affine-block-frame';
import { ImageBlockComponent } from '@blocksuite/affine-block-image';
import { EdgelessCRUDIdentifier, getSurfaceComponent, } from '@blocksuite/affine-block-surface';
import { createGroupFromSelectedCommand } from '@blocksuite/affine-gfx-group';
import { AttachmentBlockModel, BookmarkBlockModel, EmbedLinkedDocBlockSchema, EmbedLinkedDocModel, EmbedSyncedDocBlockSchema, EmbedSyncedDocModel, FrameBlockModel, ImageBlockModel, isExternalEmbedModel, MindmapElementModel, NoteBlockModel, ParagraphBlockModel, } from '@blocksuite/affine-model';
import { matchModels, } from '@blocksuite/affine-shared/utils';
import { Bound, getCommonBoundWithRotation } from '@blocksuite/global/gfx';
import { ArrowDownBigBottomIcon, ArrowDownBigIcon, ArrowUpBigIcon, ArrowUpBigTopIcon, CopyIcon, DeleteIcon, DuplicateIcon, FrameIcon, GroupIcon, LinkedPageIcon, ResetIcon, } from '@blocksuite/icons/lit';
...
import { createLinkedDocFromEdgelessElements, createLinkedDocFromNote, } from './render-linked-doc';
        ...
        actions: [
            {
                ...
                run(ctx) {
                    ...
                    if (!models.length)
                        return;
                    const edgelessClipboard = ctx.std.getOptional(EdgelessClipboardController);
                    if (!edgelessClipboard)
                        return;
        ...
        actions: [
            {
                ...
                run(ctx) {
                    ...
                    const create = async () => {
                        ...
                        const cardId = ctx.std.get(EdgelessCRUDIdentifier).addBlock(EmbedSyncedDocBlockSchema.model.flavour, {
                            xywh: model.xywh,
                            style: 'syncedDoc',
                            pageId: linkedDoc.id,
                            index: model.index,
            ...
            },
            {
                ...
                run(ctx) {
                    ...
                    const create = async () => {
                        ...
                        const linkedDoc = createLinkedDocFromEdgelessElements(ctx.host, clonedModels, title);
                        ctx.store.transact(() => {
                        ...
                        const cardId = ctx.std.get(EdgelessCRUDIdentifier).addBlock(EmbedLinkedDocBlockSchema.model.flavour, {
...
function reorderElements(ctx, models, type) {
    if (!models.length)
        return;
    const normalizedModels = Array.from(new Map(models.map(model => {
        const reorderTarget = model.group instanceof MindmapElementModel ? model.group : model;
...
function isRefreshableModel(model) {
...
function isRefreshableBlock(block) {
    return (!!block &&
