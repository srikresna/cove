...
import { CanvasRenderer, DefaultTool, EdgelessCRUDIdentifier, ExportManager, getSurfaceComponent, } from '@blocksuite/affine-block-surface';
import { splitIntoLines } from '@blocksuite/affine-gfx-text';
import { BookmarkStyles, DEFAULT_NOTE_HEIGHT, DEFAULT_NOTE_WIDTH, FrameBlockModel, MAX_IMAGE_WIDTH, } from '@blocksuite/affine-model';
import { ClipboardAdapter, decodeClipboardBlobs, } from '@blocksuite/affine-shared/adapters';
import { CANVAS_EXPORT_IGNORE_TAGS, EMBED_CARD_HEIGHT, EMBED_CARD_WIDTH, } from '@blocksuite/affine-shared/consts';
import { EmbedOptionProvider, ParseDocUrlProvider, TelemetryProvider, } from '@blocksuite/affine-shared/services';
import { convertToPng, isInsidePageEditor, isTopLevelBlock, isUrlInClipboard, matchModels, referenceToNode, } from '@blocksuite/affine-shared/utils';
import { DisposableGroup } from '@blocksuite/global/disposable';
import { BlockSuiteError, ErrorCode } from '@blocksuite/global/exceptions';
import { Bound, getCommonBound, Vec, } from '@blocksuite/global/gfx';
import { compareLayer, GfxControllerIdentifier, } from '@blocksuite/std/gfx';
...
import { isPureFileInClipboard, prepareClipboardData, tryGetSvgFromClipboard, } from './utils.js';
...
export class EdgelessClipboardController extends PageClipboard {
    constructor() {
        ...
        this._onCopy = async (_context, surfaceSelection) => {
            const event = _context.get('clipboardState').raw;
            event.preventDefault();
            const elements = getSortedCloneElements(this.selectionManager.selectedElements);
        ...
        this._onPaste = async (_context) => {
            ...
            if (isUrlInClipboard(data)) {
                ...
                options.xywh = Bound.fromCenter(Vec.toVec({
                    x,
                    y,
    ...
    static { this.key = 'affine-edgeless-clipboard'; }
    get _exportManager() {
    ...
    async _edgelessToCanvas(bound, nodes, canvasElements = [], { background, padding = IMAGE_PADDING, dpr = window.devicePixelRatio || 1, } = {}) {
        ...
        if (!(this.surface.renderer instanceof CanvasRenderer)) {
            console.warn('Skipping canvas generation for clipboard: DOM renderer active.');
            return canvas; // Return the empty canvas or handle error
        }
        const surfaceCanvas = this.surface.renderer.getCanvasByBound(bound, canvasElements);
    ...
    async _pasteTextContentAsNote(content) {
        if (content === '') {
            return;
        }
        const { x, y } = this.toolManager.lastMousePos$.peek();
        ...
        const noteId = this.crud.addBlock('affine:note', noteProps, this.doc.root.id);
        this.std.getOptional(TelemetryProvider)?.track('CanvasElementAdded', {
            control: 'canvas:paste',
            page: 'whiteboard editor',
            module: 'toolbar',
        ...
        if (typeof content === 'string') {
            splitIntoLines(content).forEach((line, idx) => {
                this.crud.addBlock('affine:paragraph', { text: new Y.Text(line) }, noteId, idx);
            });
        }
        else {
            // oxlint-disable-next-line @typescript-eslint/prefer-for-of
