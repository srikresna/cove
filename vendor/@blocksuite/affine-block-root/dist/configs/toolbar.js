import { convertToDatabase, DATABASE_CONVERT_WHITE_LIST, } from '@blocksuite/affine-block-database';
...
import { updateBlockAlign, updateBlockType, } from '@blocksuite/affine-block-note';
import { toast } from '@blocksuite/affine-components/toast';
import { EditorChevronDown } from '@blocksuite/affine-components/toolbar';
import { insertInlineLatex } from '@blocksuite/affine-inline-latex';
import { deleteTextCommand, formatBlockCommand, formatNativeCommand, formatTextCommand, isFormatSupported, textFormatConfigs, } from '@blocksuite/affine-inline-preset';
import { EmbedLinkedDocBlockSchema, EmbedSyncedDocBlockSchema, } from '@blocksuite/affine-model';
import { textAlignConfigs, textConversionConfigs, } from '@blocksuite/affine-rich-text';
import { copySelectedModelsCommand, deleteSelectedModelsCommand, draftSelectedModelsCommand, duplicateSelectedModelsCommand, getBlockSelectionsCommand, getImageSelectionsCommand, getSelectedBlocksCommand, getSelectedModelsCommand, getTextSelectionCommand, } from '@blocksuite/affine-shared/commands';
import { ActionPlacement, blockCommentToolbarButton, } from '@blocksuite/affine-shared/services';
import { getMostCommonValue } from '@blocksuite/affine-shared/utils';
import { tableViewMeta } from '@blocksuite/data-view/view-presets';
import { CopyIcon, DatabaseTableViewIcon, DeleteIcon, DuplicateIcon, LinkedPageIcon, TeXIcon, } from '@blocksuite/icons/lit';
import { BlockSelection, BlockViewIdentifier, } from '@blocksuite/std';
...
const conversionsActionGroup = {
    ...
    generate({ chain }) {
        ...
        const conversion = textConversionConfigs.find(({ flavour, type }) => flavour === model.flavour &&
            (type ? 'type' in model.props && type === model.props.type : true)) ?? textConversionConfigs[0];
        const update = (flavour, type) => {
            chain
        ...
        >
          <div data-size="large" data-orientation="vertical">
            ${repeat(textConversionConfigs.filter(c => c.flavour !== 'affine:divider'), item => item.name, ({ flavour, type, name, icon }) => html `
                <editor-menu-action
                  aria-label=${name}
...
const alignActionGroup = {
    ...
    generate({ chain }) {
        ...
        if (!ok)
            return null;
        const alignment = textAlignConfigs.find(({ textAlign }) => textAlign ===
            getMostCommonValue(selectedModels.map(({ props }) => props), 'textAlign')) ?? textAlignConfigs[0];
        const update = (textAlign) => {
        ...
        >
          <div data-size="large" data-orientation="vertical">
            ${repeat(textAlignConfigs, item => item.name, ({ textAlign, name, icon }) => html `
                <editor-menu-action
                  aria-label=${name}
...
const inlineTextActionGroup = {
    id: 'b.inline-text',
    when: ({ chain }) => isFormatSupported(chain).run()[0],
    actions: textFormatConfigs.flatMap(({ id, name, action, activeWhen, icon }, score) => {
        const textAction = {
            id,
            icon,
            score,
    ...
    when({ chain }) {
        const middleware = (count = 0) => {
            return (ctx, next) => {
                ...
                if (!selectedBlocks || selectedBlocks.length === count)
                    return;
                const allowed = selectedBlocks.every(block => DATABASE_CONVERT_WHITE_LIST.includes(block.flavour));
                if (!allowed)
                    return;
                next();
            };
...
const turnIntoLinkedDoc = {
    ...
    when({ chain, std }) {
        ...
        if (supportFlavours.some(flavour => !std.getOptional(BlockViewIdentifier(flavour))))
            return false;
        const [ok, { selectedModels }] = chain
    ...
    run({ chain, store, selection, std, track }) {
        const [ok, { draftedModels, selectedModels }] = chain
            ...
            .run();
        if (!ok || !draftedModels || !selectedModels?.length)
            return;
        selection.clear();
        ...
        promptDocTitle(std, autofill)
            ...
            if (title === null)
                return;
            await convertSelectedBlocksToLinkedDoc(std, store, draftedModels, title);
                ...
                },
                {
                    ...
                    run({ chain, store, selection }) {
                        ...
                        if (ok && selectedBlocks.length) {
                            selection.setGroup('note', selectedBlocks.map(block => selection.create(BlockSelection, {
