...
import { generateKeyBetweenV2, getTopElements, GfxBlockElementModel, GfxExtension, GfxExtensionIdentifier, isGfxGroupCompatibleModel, renderableInEdgeless, } from '@blocksuite/std/gfx';
import { Text } from '@blocksuite/store';
import * as Y from 'yjs';
const FRAME_PADDING = 40;
export class FrameOverlay extends Overlay {
    static { this.overlayName = 'frame'; }
    get _frameManager() {
    ...
    clear() {
        if (this._frame === null && this._innerElements.size === 0)
            return;
        this._reset();
        this._renderer?.refresh();
    }
    highlight(frame, highlightElementsInBound = false, highlightOutline = true) {
        if (!highlightElementsInBound && !highlightOutline)
            return;
        let needRefresh = false;
        if (highlightOutline && this._prevXYWH !== frame.xywh) {
            needRefresh = true;
        }
        let innerElements = new Set();
        if (highlightElementsInBound) {
            innerElements = new Set(getTopElements(this._frameManager.getElementsInFrameBound(frame)).concat(this._frameManager.getChildElementsInFrame(frame).filter(child => {
...
export const EdgelessFrameManagerIdentifier = GfxExtensionIdentifier('frame-manager');
export class EdgelessFrameManager extends GfxExtension {
    static { this.key = 'frame-manager'; }
    /**
    ...
    static framePresentationComparator(a, b) {
        ...
        if (isModel(a) &&
            ...
            return stringCompare(a.props.presentationIndex$.value, b.props.presentationIndex$.value);
        }
        else if (a.props.presentationIndex && b.props.presentationIndex) {
            return stringCompare(a.props.presentationIndex, b.props.presentationIndex);
        }
    ...
    _addChildrenToLegacyFrame(frame) {
        if (frame.props.childElementIds !== undefined)
            return;
        const elements = this.getElementsInFrameBound(frame);
        const childElements = elements.filter(element => this.getParentFrame(element) === null && element !== frame);
        frame.addChildren(childElements);
    }
    _addFrameBlock(bound) {
        const surfaceModel = this.gfx.surface;
    ...
    _watchElementAdded() {
        ...
        this._disposable.add(surfaceModel.elementAdded.subscribe(({ id, local }) => {
        ...
        this._disposable.add(doc.slots.blockUpdated.subscribe(payload => {
            if (payload.type === 'add' &&
                payload.model instanceof GfxBlockElementModel &&
                renderableInEdgeless(doc, surfaceModel, payload.model)) {
                const frame = this.getFrameFromPoint(payload.model.elementBound.center, isFrameBlock(payload.model) ? [payload.model] : []);
                if (!frame)
                    return;
                if (isFrameBlock(payload.model) &&
     ...
     */
    addElementsToFrame(frame, elements) {
        if (frame.isLocked())
            return;
        if (frame.props.childElementIds === undefined) {
            this._addChildrenToLegacyFrame(frame);
        }
        elements = elements.filter(el => el !== frame && !frame.childElements.includes(el));
        if (elements.length === 0)
            return;
        frame.addChildren(elements);
    }
    createFrameOnBound(bound) {
        const frameModel = this._addFrameBlock(bound);
        this.addElementsToFrame(frameModel, getTopElements(this.getElementsInFrameBound(frameModel)));
    ...
    createFrameOnElements(elements) {
        // make sure all elements are in the same level
        for (const element of elements) {
            if (element.group !== elements[0].group)
                return;
        }
        ...
        if (parentFrameBound?.contains(bound)) {
            ...
            bound.w += Math.min(0.5 * (parentFrameBound.x + parentFrameBound.w - bound.x - bound.w), FRAME_PADDING);
            bound.h += Math.min(0.5 * (parentFrameBound.y + parentFrameBound.h - bound.y - bound.h), FRAME_PADDING);
        }
        else {
    ...
    createFrameOnViewportCenter(wh) {
    ...
    generatePresentationIndex() {
        const before = this.frames[this.frames.length - 1]?.props.presentationIndex ?? null;
     ...
     */
    getChildElementsInFrame(frame) {
        if (frame.props.childElementIds === undefined) {
            return this.getElementsInFrameBound(frame).filter(element => this.getParentFrame(element) !== null);
        }
     ...
     */
    getElementsInFrameBound(frame, fullyContained = true) {
        const bound = Bound.deserialize(frame.xywh);
        const elements = this.gfx.grid
     ...
     */
    getFrameFromPoint([x, y], ignoreFrames = []) {
    ...
    refreshLegacyFrameOrder() {
        ...
        if (splitIndex === 0)
            return;
        if (splitIndex === -1)
            splitIndex = frames.length;
        let afterPreIndex = frames[splitIndex]?.props.presentationIndex ||
    ...
    removeAllChildrenFromFrame(frame) {
        this.gfx.doc.transact(() => {
            frame.props.childElementIds = {};
        });
    }
    removeFromParentFrame(element) {
