...
import { DefaultTheme, FrameBlockSchema, isTransparent, } from '@blocksuite/affine-model';
import { ThemeProvider } from '@blocksuite/affine-shared/services';
import { Bound } from '@blocksuite/global/gfx';
import { GfxBlockComponent } from '@blocksuite/std';
import { getTopElements, GfxViewInteractionExtension, } from '@blocksuite/std/gfx';
...
let FrameBlockComponent = (() => {
    ...
    return class FrameBlockComponent extends _classSuper {
        ...
        onBoxSelected(context) {
            ...
            return (this.model.childElements.length === 0 || bound.contains(elementBound));
        }
        renderGfxBlock() {
...
export const FrameBlockInteraction = GfxViewInteractionExtension(FrameBlockSchema.model.flavour, {
    handleResize: context => {
        ...
        return {
            ...
            onResizeMove(context) {
                ...
                const overlay = std.getOptional(OverlayIdentifier('frame'));
                model.xywh = newBound.serialize();
                if (!frameManager) {
                    return;
                }
                const oldChildren = frameManager.getChildElementsInFrame(model);
                const newChildren = getTopElements(frameManager.getElementsInFrameBound(model)).concat(oldChildren.filter(oldChild => {
    ...
    handleSelection: () => {
        return {
            selectable(context) {
                const { model } = context;
                const onTitle = model.externalBound?.containsPoint([
                    context.position.x,
                    context.position.y,
                ]) ?? false;
