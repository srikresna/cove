...
export class AffineScrollAnchoringWidget extends WidgetComponent {
    constructor() {
        super(...arguments);
        this.#listened = false;
        this.#requestUpdateFn = () => this.requestUpdate();
        this.#resizeObserver = new ResizeObserver(this.#requestUpdateFn);
        this.anchor$ = signal(null);
        this.anchorBounds$ = signal(null);
        this.highlighted$ = computed(() => this.std.selection.find(HighlightSelection));
    }
  ...
  `; }
    ...
    #moveToAnchorInEdgeless(id) {
        const controller = this.std.get(GfxControllerIdentifier);
        const surface = controller.surface;
        if (!surface)
            return;
        const xywh = controller.getElementById(id)?.xywh;
        if (!xywh) {
            if (!this.#listened)
                return;
    ...
    connectedCallback() {
        ...
        this.disposables.add(this.anchor$.subscribe(anchor => {
            if (!anchor)
                return;
        ...
        this.disposables.add(this.highlighted$.subscribe(highlighted => {
            if (!highlighted)
                return;
            const { mode, blockIds: [bid], elementIds: [eid], highlight, } = highlighted;
