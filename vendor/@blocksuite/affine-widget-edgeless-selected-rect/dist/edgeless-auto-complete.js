...
import { CanvasElementType, EdgelessCRUDIdentifier, getSurfaceBlock, getSurfaceComponent, isNoteBlock, Overlay, OverlayIdentifier, } from '@blocksuite/affine-block-surface';
import { ConnectorPathGenerator, } from '@blocksuite/affine-gfx-connector';
import { mountShapeTextEditor } from '@blocksuite/affine-gfx-shape';
import { DEFAULT_NOTE_HEIGHT, DefaultTheme, LayoutType, MindmapElementModel, ShapeElementModel, shapeMethods, } from '@blocksuite/affine-model';
...
import { ArrowUpBigIcon, PlusIcon, SiblingNodeIcon, SubNodeIcon, } from '@blocksuite/icons/lit';
  ...
  `; }
        ...
        _addMindmapNode(target) {
            ...
            requestAnimationFrame(() => {
                mountShapeTextEditor(this.crud.getElementById(newNode), this.edgeless);
            });
        }
        ...
        _generateElementOnClick(type) {
            ...
            if (isShape(this.current)) {
                ...
                const [x, y] = this.gfx.viewport.toViewCoord(bound.center[0], bound.y + DEFAULT_NOTE_HEIGHT / 2);
                requestAnimationFrame(() => {
        ...
        _getConnectedElements(element) {
            ...
            return this._surface.getConnectors(element.id).reduce((prev, current) => {
                if (current.target.id === element.id && current.source.id) {
                    prev.push(this.crud.getElementById(current.source.id));
                }
                if (current.source.id === element.id && current.target.id) {
                    prev.push(this.crud.getElementById(current.target.id));
                }
                return prev;
            }, []);
        }
      ...
      >
        ...
        _showNextShape(current, bound, path, targetType) {
            ...
            if (!surface)
                return;
            this._autoCompleteOverlay.stroke = surface.renderer.getColorValue(current.strokeColor, DefaultTheme.shapeStrokeColor, true);
        ...
        firstUpdated() {
            ...
            _disposables.add(this.gfx.selection.slots.updated.subscribe(() => {
        ...
        _canAutoComplete() {
            ...
            return (selection.selectedElements.length === 1 &&
      ...
      ${isMindMap ? this._renderMindMapButtons() : this._renderArrow()}
        ...
        constructor() {
            ...
            this._onPointerDown = (e, type) => {
                ...
                const start = this.gfx.viewport.toModelCoord(e.clientX - viewportRect.left, e.clientY - viewportRect.top);
                if (!this.edgeless.std.event)
                    return;
                let connector;
                this._disposables.addFromEvent(document, 'pointermove', e => {
                    const point = this.gfx.viewport.toModelCoord(e.clientX - viewportRect.left, e.clientY - viewportRect.top);
