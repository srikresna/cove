...
export class EdgelessPenToolButton extends EdgelessToolbarToolMixin(SignalWatcher(LitElement)) {
    constructor() {
        super(...arguments);
        this.colors$ = computed(() => {
            ...
            return {
                brush: keepColor(this.themeProvider.generateColorProperty(brush, undefined, theme)),
                highlighter: keepColor(this.themeProvider.generateColorProperty(highlighter, undefined, theme)),
            };
        });
        this.color$ = computed(() => {
  ...
  `; }
    ...
    firstUpdated() {
        this.disposables.add(this.gfx.tool.currentToolName$.subscribe(name => {
    ...
    render() {
        const { active, penInfo$: { value: { type, color, icon, tip, shortcut }, }, } = this;
        return html `
      <edgeless-toolbar-button
        class="edgeless-pen-button"
        data-drawing-tool="${type}"
        .tooltip=${when(this.popper, () => nothing, () => html `<affine-tooltip-content-with-shortcut
              data-tip="${tip}"
