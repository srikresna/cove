...
import { adjustColorAlpha, keepColor, packColor, } from '@blocksuite/affine-components/color-picker';
import { DEFAULT_HIGHLIGHTER_LINE_WIDTH, DefaultTheme, HIGHLIGHTER_LINE_WIDTHS, HighlighterElementModel, resolveColor, } from '@blocksuite/affine-model';
    ...
    actions: [
        {
            ...
            content(ctx) {
                ...
                if (!models.length)
                    return null;
                const lineWidth = getMostCommonValue(models, 'lineWidth') ??
        ...
        },
        {
            ...
            content(ctx) {
                ...
                const color = keepColor(getMostCommonResolvedValue(models, field, color => resolveColor(color, theme)) ?? resolveColor(DefaultTheme.black, theme));
                const onPick = (e) => {
                    if (e.type === 'pick') {
