...
import { BrushElementModel, DefaultTheme, LineWidth, resolveColor, } from '@blocksuite/affine-model';
    ...
    actions: [
        {
            ...
            content(ctx) {
                ...
                if (!models.length)
                    return null;
                const lineWidth = getMostCommonValue(models, 'lineWidth') ?? LineWidth.Four;
                const onPick = (e) => {
                    e.stopPropagation();
        ...
        },
        {
            ...
            content(ctx) {
                ...
                const color = getMostCommonResolvedValue(models, field, color => resolveColor(color, theme)) ?? resolveColor(DefaultTheme.black, theme);
                const onPick = (e) => {
                    switch (e.type) {
                        case 'pick':
                            {
