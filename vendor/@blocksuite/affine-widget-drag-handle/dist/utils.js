import {} from '@blocksuite/affine-block-callout';
import { AFFINE_EDGELESS_NOTE, EdgelessNoteBackground, EdgelessNoteBlockComponent, } from '@blocksuite/affine-block-note';
import { ParagraphBlockComponent } from '@blocksuite/affine-block-paragraph';
import { DatabaseBlockModel, ListBlockModel, ParagraphBlockModel, } from '@blocksuite/affine-model';
import { DocModeProvider } from '@blocksuite/affine-shared/services';
import { calcDropTarget, findClosestBlockComponent, getBlockProps, getClosestBlockComponentByPoint, matchModels, } from '@blocksuite/affine-shared/utils';
...
export const getDragHandleContainerHeight = (model) => {
    ...
    if (key === 'paragraph' && model.props.type) {
        key = model.props.type;
    }
...
export const extractIdsFromSnapshot = (snapshot) => {
    const ids = [];
...
export const getParentNoteBlock = (blockComponent) => {
...
export const getClosestBlockByPoint = (editorHost, rootComponent, point) => {
    ...
    if (matchModels(closestBlock.model, [ParagraphBlockModel])) {
        const callout = closestBlock.closest('affine-callout');
        if (callout) {
            return callout;
        }
    }
...
export function getDragHandleLeftPadding(blocks) {
    const hasToggleList = blocks.some(block => (matchModels(block.model, [ListBlockModel]) &&
 ...
 */
function getHoveringNote(point) {
...
export function getSnapshotRect(snapshot) {
    let bound = null;
    const getBound = (block) => {
        if (block.flavour === 'affine:surface') {
            ...
            bound = bound
                ? bound.unite(Bound.deserialize(block.props.xywh))
                : Bound.deserialize(block.props.xywh);
        }
    };
