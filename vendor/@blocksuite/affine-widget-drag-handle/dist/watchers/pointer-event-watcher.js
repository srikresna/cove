...
import { ADD_BLOCK_WIDGET_WIDTH, DRAG_HANDLE_CONTAINER_WIDTH, DRAG_HANDLE_GRABBER_BORDER_RADIUS, DRAG_HANDLE_GRABBER_HEIGHT, DRAG_HANDLE_GRABBER_WIDTH, } from '../config.js';
import { AFFINE_DRAG_HANDLE_WIDGET } from '../consts.js';
import { getClosestBlockByPoint, getClosestNoteBlock, getDragHandleContainerHeight, includeTextSelection, insideDatabaseTable, isBlockIdEqual, isOutOfNoteBlock, updateDragHandleClassName, } from '../utils.js';
/**
...
export class PointerEventWatcher {
    ...
    constructor(widget) {
        ...
        this._canEditing = (noteBlock) => {
            if (noteBlock.store.id !== this.widget.store.id)
                return false;
        ...
        this._clickHandler = ctx => {
            if (!this.widget.isBlockDragHandleVisible)
                return;
        ...
        this._throttledPointerMoveHandler = throttle(ctx => {
            ...
            const point = new Point(state.raw.x, state.raw.y);
            const closestNoteBlock = getClosestNoteBlock(this.widget.host, this.widget.rootComponent, point);
        ...
        this.showDragHandleOnHoverBlock = () => {
            ...
            if (isBlockIdEqual(block.blockId, this._lastShowedBlock?.id)) {
                ...
                if (this.widget.selectionHelper.isBlockSelected(block))
                    applyStyle(this.widget.isDragHandleHovered &&
