...
import { DRAG_HANDLE_CONTAINER_OFFSET_LEFT_TOP_LEVEL, DRAG_HANDLE_CONTAINER_WIDTH_TOP_LEVEL, HOVER_AREA_RECT_PADDING_TOP_LEVEL, } from '../config.js';
/**
...
export class EdgelessWatcher {
    ...
    get hoveredElemArea() {
        ...
        const containerWidth = (DRAG_HANDLE_CONTAINER_WIDTH_TOP_LEVEL * scale) / viewScale;
    ...
    constructor(widget) {
        ...
        this._handleEdgelessViewPortUpdated = ({ zoom, center, }) => {
            ...
            if (this.widget.center[0] !== center[0] ||
        ...
        this._flushShowDragHandle = () => {
            this._showDragHandleRafId = null;
            if (!this.widget.anchorBlockId.peek())
                return;
        ...
        this._showDragHandle = (area) => {
            ...
            if (this.widget.isGfxDragHandleVisible &&
                this._showDragHandleRafId === null &&
                this._isAreaEqual(this._lastAppliedHoveredElemArea, this._pendingHoveredElemArea)) {
                return;
            }
            if (this._showDragHandleRafId !== null) {
                return;
            }
            this._showDragHandleRafId = requestAnimationFrame(this._flushShowDragHandle);
        };
        this._updateDragHoverRectTopLevelBlock = (area) => {
            if (!this.widget.dragHoverRect)
                return;
            const nextArea = area ?? this.hoveredElemArea;
            if (!nextArea) {
            ...
            const nextRect = new Rect(nextArea.left, nextArea.top, nextArea.right, nextArea.bottom);
    ...
    watch() {
        ...
        disposables.add(selection.slots.updated.subscribe(() => {
            this.updateAnchorElement();
        }));
        disposables.add(edgelessSlots.readonlyUpdated.subscribe(() => {
            this.updateAnchorElement();
        }));
        disposables.add(edgelessSlots.elementResizeEnd.subscribe(() => {
            this.updateAnchorElement();
        }));
        ...
        disposables.add(edgelessSlots.elementResizeStart.subscribe(() => {
            this.widget.hide();
        }));
        disposables.add(std.store.slots.blockUpdated.subscribe(payload => {
            if (this.widget.isGfxDragHandleVisible &&
        ...
        if (surface) {
            disposables.add(surface.elementUpdated.subscribe(({ id }) => {
                if (this.widget.isGfxDragHandleVisible) {
                    if (id !== this.widget.anchorBlockId.peek())
                        return;
                    this._scheduleShowDragHandleFromSurfaceUpdate();
                }
            }));
