import { EMBED_IFRAME_DEFAULT_HEIGHT_IN_SURFACE, EMBED_IFRAME_DEFAULT_WIDTH_IN_SURFACE, } from '@blocksuite/affine-block-embed';
import { ParagraphBlockComponent } from '@blocksuite/affine-block-paragraph';
import { DropIndicator } from '@blocksuite/affine-components/drop-indicator';
import { AttachmentBlockModel, BookmarkBlockModel, DatabaseBlockModel, DEFAULT_NOTE_HEIGHT, DEFAULT_NOTE_WIDTH, ListBlockModel, NoteBlockModel, RootBlockModel, } from '@blocksuite/affine-model';
import { surfaceRefToEmbed } from '@blocksuite/affine-shared/adapters';
import { BLOCK_CHILDREN_CONTAINER_PADDING_LEFT, EMBED_CARD_HEIGHT, EMBED_CARD_WIDTH, } from '@blocksuite/affine-shared/consts';
import { DndApiExtensionIdentifier, DocModeProvider, EmbedIframeService, TelemetryProvider, } from '@blocksuite/affine-shared/services';
import { captureEventTarget, findNoteBlockModel, getBlockComponentsExcludeSubtrees, getRectByBlockComponent, getScrollContainer, matchModels, } from '@blocksuite/affine-shared/utils';
import { Bound, Point, Rect, } from '@blocksuite/global/gfx';
import { assertType } from '@blocksuite/global/utils';
import { BlockComponent, } from '@blocksuite/std';
import { GfxBlockElementModel, GfxControllerIdentifier, GfxGroupLikeElementModel, GfxPrimitiveElementModel, isGfxGroupCompatibleModel, isPrimitiveModel, SURFACE_YMAP_UNIQ_IDENTIFIER, SurfaceBlockModel, } from '@blocksuite/std/gfx';
import { internalPrimitives, Slice, toDraftModel, } from '@blocksuite/store';
...
import { containBlock, extractIdsFromSnapshot, getParentNoteBlock, getSnapshotRect, includeTextSelection, isOutOfNoteBlock, } from '../utils.js';
export class DragEventWatcher {
    ...
    constructor(widget) {
        ...
        this.previewHelper = new PreviewHelper(this.widget);
        this.dropTargetCleanUps = new Map();
        this.resetOpacityCallbacks = [];
        this._createDropIndicator = () => {
            let topElement = this.widget;
        ...
        this._onDragMove = (point, payload, dropPayload, block) => {
            this._createDropIndicator();
            this._updateDropIndicator(point, payload, dropPayload, block);
        };
        this._getFallbackInsertPlace = (block) => {
         ...
         */
        this._getDropResult = (dropBlock, dragPayload, dropPayload) => {
            ...
            if (edge === 'right' && matchModels(dropModel, [ListBlockModel])) {
                ...
                const placement = 'in';
                if (snapshot.content.every(block => schema.safeValidate(block.flavour, 'affine:list'))) {
                    const rect = Rect.fromLWTH(domRect.left + BLOCK_CHILDREN_CONTAINER_PADDING_LEFT, domRect.width - BLOCK_CHILDREN_CONTAINER_PADDING_LEFT, domRect.top + domRect.height, 3 * scale);
                    result = {
                        placement,
                        rect,
                        modelState: {
                    ...
                    if (fallbackModel) {
                        ...
                        result = {
                            placement: 'after',
                            rect: Rect.fromLWTH(domRect.left, domRect.width, domRect.top + domRect.height, 3 * scale),
                            modelState: {
                                model: fallbackModel,
                                rect: domRect,
                ...
                const placement = isDropOnNoteBlock &&
        ...
        this._updateDropIndicator = (point, dragPayload, dropPayload, dropBlock) => {
        ...
        this._getSnapshotFromSelectedGfxElms = () => {
            ...
            const getElementsInContainer = (elem, selectedElements = []) => {
            ...
            const toSnapshotRequiredBlocks = (selectedModels) => {
                let surfaceAdded = false;
                const blocks = [];
                const blocksUnderSurface = [];
            ...
            const selectedElements = getElementsInContainer(selectedElm, []);
            const blocksOfSnapshot = toSnapshotRequiredBlocks(selectedElements);
            return {
                snapshot: this._toSnapshot(blocksOfSnapshot, [selectedElmId]),
            };
        };
        this._getDraggedSnapshot = () => {
            const { snapshot } = this.widget.activeDragHandle === 'block'
        ...
        this._getSnapshotFromHoveredBlocks = () => {
            ...
            const blocks = this.widget.selectionHelper.selectedBlockComponents.flatMap(block => {
                ...
                if (collapsedBlock.includes(block))
                    return [];
                // if block is toggled heading, should select all siblings
                if (block instanceof ParagraphBlockComponent &&
                    block.model.props.type.startsWith('h') &&
                    block.model.props.collapsed) {
                    const collapsedSiblings = block.collapsedSiblings.flatMap(sibling => this.widget.host.view.getBlock(sibling.id) ?? []);
        ...
        this._onPageDrop = (dropBlock, dragPayload, dropPayload, _) => {
            ...
            if (matchModels(parent, [NoteBlockModel]) &&
                ...
                snapshot.content = snapshot.content.filter(block => dragPayload.from?.docId !== this.widget.store.id ||
                    block.id !== parent.id);
            ...
            if (dragPayload.bsEntity?.fromMode === 'gfx' &&
                ...
                if (dragPayload.from?.docId === this.widget.store.id) {
                    ...
                    const walk = (block) => {
                        if (block.flavour === 'affine:surface') {
                            ...
                            if (block.props.xywh) {
                                const bound = Bound.deserialize(block.props.xywh);
                                const size = bound.w * bound.h;
                                if ((largestElem?.size ?? 0) < size) {
                    ...
                    if (largestElem) {
                        store.addBlock('affine:surface-ref', {
                ...
                // and create a surface-ref block or embed-linked-doc block
                    ...
                    this._mergeSnapshotToCurDoc(snapshot)
                        ...
                        let largestElem;
                        idRemap.forEach(val => {
                            const gfxElement = this.gfx.getElementById(val);
        ...
        this._onDrop = (dropBlock, dragPayload, dropPayload, point) => {
         ...
         * @param point
         */
        this._mergeSnapshotToCurDoc = async (snapshot, point) => {
            if (!point) {
            ...
            const schema = this.std.store.schema;
            const containerTree = { root: new Set() };
            const idRemap = new Map();
            let elemMap = {};
            const blockMap = {};
            const isGroupLikeElem = (elem) => {
                const constructor = surface.getConstructor(elem.type);
                const isGroup = Object.isPrototypeOf.call(GfxGroupLikeElementModel.prototype, constructor.prototype);
                return isGroup;
            };
            const isGroupLikeBlock = (flavour) => {
            ...
            const buildContainerTree = (block) => {
                if (block.flavour === 'affine:surface') {
                    elemMap = block.props.elements ?? {};
                    ...
                    const isSurfaceChild = schema.safeValidate(block.flavour, 'affine:surface');
                    blockMap[block.id] = {
            ...
            const addInDependencyOrder = async (id) => {
                if (containerTree[id]) {
                ...
                if (blockMap[id]) {
                    ...
                    if (isGroupLikeBlock(blockSnapshot.flavour)) {
                        Object.keys(blockSnapshot.props.childElementIds ?? {}).forEach(childId => {
                            assertType(blockSnapshot.props.childElementIds);
         ...
         * @returns
         */
        this._rewriteSnapshotXYWH = (snapshot, point, ignoreOriginalPos = false) => {
            ...
            const rewrite = (block) => {
                if (block.flavour === 'affine:surface') {
                    ...
                    const blockBound = Bound.deserialize(block.props.xywh) ??
                        new Bound(0, 0, 0, 0);
                    if (block.flavour === 'affine:embed-iframe') {
                        ...
                        const style = (block.props.style ?? 'vertical');
        ...
        this._dropAsGfxBlock = (snapshot, point) => {
            ...
            const groupByParent = groupBy(snapshot.content, block => schema.safeValidate(block.flavour, 'affine:surface')
                ? 'affine:surface'
            ...
            if (!groupByParent.empty?.length) {
                ...
                const content = snapshot.content.filter(block => schema.safeValidate(block.flavour, 'affine:note'));
                ...
                if (dndExtApi &&
                    ...
                    const linkedDocSnapshot = dndExtApi.fromEntity({
                        ...
                        props: {
                            ...
                            style: 'vertical',
                            xywh: new Bound(point.x, point.y, EMBED_CARD_WIDTH[style], EMBED_CARD_HEIGHT[style]).serialize(),
                        },
                    });
                ...
                // create note to wrap the snapshot
                    ...
                    let noteId;
                    if (originalNote) {
                        const placement = originalNote.children[0].id === snapshot.content[0].id
                            ? 'before'
                            : 'after';
                        noteId = store.addSiblingBlocks(originalNote, [
                            {
                                flavour: 'affine:note',
                                xywh: new Bound(point.x, point.y, DEFAULT_NOTE_WIDTH, DEFAULT_NOTE_HEIGHT).serialize(),
                            },
                        ], placement)[0];
                    }
                    else {
                        noteId = store.addBlock('affine:note', {
                            xywh: new Bound(point.x, point.y, DEFAULT_NOTE_WIDTH, DEFAULT_NOTE_HEIGHT).serialize(),
        ...
        this._toSnapshot = (blocks, selectedGfxElms) => {
            const slice = Slice.fromModels(this.std.store, blocks.map(block => toDraftModel(block instanceof BlockComponent ? block.model : block)));
        ...
        this._setOpacityOfDraggedBlocks = (snapshot) => {
            const OPACITY = 0.7;
            const gfx = this.gfx;
            ...
            const traverse = (block) => {
                if (block.flavour === 'affine:surface') {
                    ...
                    Object.keys(block.props.elements).forEach(elemId => {
                        const element = gfx.getElementById(elemId);
                        if (element) {
    ...
    async _dropToModel(snapshot, parent, index) {
        try {
            const std = this.std;
            const job = this._getJob();
    ...
    _isDropOnCurrentEditor(std) {
        return std === this.std;
    }
    _isUnderNoteBlock(model) {
    ...
    _makeDraggable(target) {
        const std = this.std;
        return std.dnd.draggable({
            element: target,
    ...
    _makeDropTarget(view) {
        ...
        const cleanups = [];
        cleanups.push(this.std.dnd.dropTarget({
            element: view,
            getIsSticky: () => {
            ...
            canDrop: ({ source }) => {
                ...
                if (source.data.bsEntity?.type === 'blocks') {
                    return (source.data.from?.docId !== widget.store.id ||
    ...
    _monitorBlockDrag() {
        return this.std.dnd.monitor({
            ...
            onDropTargetChange: ({ location }) => {
                this._clearDropIndicator();
                if (!this._isDropOnCurrentEditor(location.current.dropTargets[0]?.element?.std)) {
                    return;
                }
            },
            onDrop: ({ location, source }) => {
                ...
                this._onDrop(target.element, dragPayload, dropPayload, point);
            },
            onDrag: ({ location, source }) => {
                ...
                const dragPayload = source.data;
                const dropPayload = target.data;
                this._onDragMove(point, dragPayload, dropPayload, target.element);
            },
        });
    }
    watch() {
        ...
        if (scrollable && this.mode === 'page') {
            disposables.add(std.dnd.autoScroll({
                element: scrollable,
                canScroll: ({ source }) => {
        ...
        disposables.add(std.view.viewUpdated.subscribe(payload => {
