...
import { DefaultTheme, EDGELESS_TEXT_BLOCK_MIN_HEIGHT, EDGELESS_TEXT_BLOCK_MIN_WIDTH, EdgelessTextBlockSchema, ListBlockModel, ParagraphBlockModel, } from '@blocksuite/affine-model';
import { focusTextModel } from '@blocksuite/affine-rich-text';
import { ThemeProvider } from '@blocksuite/affine-shared/services';
import { handleNativeRangeAtPoint, matchModels, } from '@blocksuite/affine-shared/utils';
import { Bound, clamp } from '@blocksuite/global/gfx';
import { BlockSelection, GfxBlockComponent, TextSelection, } from '@blocksuite/std';
...
let EdgelessTextBlockComponent = (() => {
  ...
  `; }
        ...
        connectedCallback() {
            ...
            this.disposables.add(this.model.propsUpdated.subscribe(({ key }) => {
                this.updateComplete
                    ...
                    const blockSelections = this.model.children.map(child => this.std.selection.create(BlockSelection, {
                        blockId: child.id,
                    }));
                    if (key === 'fontStyle') {
        ...
        firstUpdated(props) {
            ...
            disposables.add(edgelessSelection.slots.updated.subscribe(() => {
            ...
            disposables.addFromEvent(this._textContainer, 'click', e => {
                ...
                if (isTop) {
                    ...
                    if (!firstChild ||
                        ...
                        newParagraphId = this.store.addBlock('affine:paragraph', {}, this.model.id, 0);
                    }
                }
                else {
                    ...
                    if (!lastChild ||
                        ...
                        newParagraphId = this.store.addBlock('affine:paragraph', {}, this.model.id);
                    }
                }
                if (newParagraphId) {
    ...
    `;
        ...
        renderPageContent() {
            const { color, fontFamily, fontStyle, fontWeight, textAlign } = this._style$.value;
            const themeProvider = this.std.get(ThemeProvider);
            const textColor = themeProvider.generateColorProperty(color, DefaultTheme.textColor, themeProvider.theme$.value);
    ...
    `;
        ...
        constructor() {
            ...
            this._style$ = computed(() => {
                const { color$: { value: color }, fontFamily$: { value: fontFamily }, fontStyle$: { value: fontStyle }, fontWeight$: { value: fontWeight }, textAlign$: { value: textAlign }, } = this.model.props;
                return {
                    color,
                    fontFamily,
                    fontStyle,
...
export const EdgelessTextInteraction = GfxViewInteractionExtension(EdgelessTextBlockSchema.model.flavour, {
    resizeConstraint: {
        ...
        return {
            ...
            onResizeMove(context) {
                ...
                if (lockRatio) {
                    ...
                    const newRealWidth = clamp(newBound.w / initialScale, constraint.minWidth, constraint.maxWidth);
