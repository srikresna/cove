...
import { addDays, differenceInCalendarDays, format as fmt, isToday, isTomorrow, isYesterday, startOfDay, startOfMonth, startOfWeek, startOfYear, } from 'date-fns';
...
export const createGroupByConfig = (config) => config;
export const ungroups = {
    key: 'Ungroups',
    value: null,
};
const WEEK_OPTS_MON = { weekStartsOn: 1 };
const WEEK_OPTS_SUN = { weekStartsOn: 0 };
const rangeLabel = (a, b) => `${fmt(a, 'MMM d yyyy')} – ${fmt(b, 'MMM d yyyy')}`;
function buildDateCfg(name, grouper, groupName) {
    return createGroupByConfig({
        name,
        matchType: t.date.instance(),
        groupName: (_t, v) => groupName(v),
        defaultKeys: _t => [ungroups],
        valuesGroup: (v, _t) => grouper(v),
        addToGroup: (grp, _old) => grp,
        view: createUniComponentFromWebComponent(DateGroupView),
    });
}
const dateRelativeCfg = buildDateCfg('date-relative', v => {
    if (v == null)
        return [ungroups];
    ...
    if (isToday(d))
        return [{ key: 'today', value: +d }];
    if (isTomorrow(d))
        return [{ key: 'tomorrow', value: +d }];
    if (isYesterday(d))
        return [{ key: 'yesterday', value: +d }];
    // Handle future dates
    if (daysDiff > 0) {
        if (daysDiff <= 7)
            return [{ key: 'next7', value: +d }];
        if (daysDiff <= 30)
            return [{ key: 'next30', value: +d }];
    ...
    if (daysAgo <= 7)
        return [{ key: 'last7', value: +d }];
    if (daysAgo <= 30)
        return [{ key: 'last30', value: +d }];
...
export const groupByMatchers = [
    createGroupByConfig({
        ...
        defaultKeys: type => t.tag.is(type) && type.data
            ? [ungroups, ...type.data.map(v => ({ key: v.id, value: v.id }))]
            : [ungroups],
        valuesGroup: (value, _t) => value == null ? [ungroups] : [{ key: `${value}`, value }],
        addToGroup: (v, _old) => v,
    ...
    createGroupByConfig({
        ...
        defaultKeys: type => t.array.is(type) && t.tag.is(type.element) && type.element.data
            ? [
                ungroups,
        ...
        removeFromGroup: (value, old) => Array.isArray(old) ? old.filter(v => v !== value) : old,
        view: createUniComponentFromWebComponent(SelectGroupView),
    }),
    createGroupByConfig({
        name: 'text',
        ...
        groupName: (_t, v) => `${v ?? ''}`,
        defaultKeys: _t => [ungroups],
        valuesGroup: (v, _t) => typeof v !== 'string' || !v ? [ungroups] : [{ key: hash(v), value: v }],
        addToGroup: (v, _old) => v,
    ...
    createGroupByConfig({
        ...
        groupName: (_t, v) => `${v ?? ''}`,
        defaultKeys: _t => [ungroups],
        valuesGroup: (v, _t) => typeof v !== 'number'
            ? [ungroups]
            : [{ key: `g:${Math.floor(v / 10)}`, value: Math.floor(v / 10) }],
        addToGroup: (v, _old) => typeof v === 'number' ? v * 10 : null,
    ...
    createGroupByConfig({
        ...
        valuesGroup: (v, _t) => typeof v !== 'boolean'
            ? [{ key: 'false', value: false }]
