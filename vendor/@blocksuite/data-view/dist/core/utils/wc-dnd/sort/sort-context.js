...
import { DndContext, draggableDataName, droppableDataName, } from '../dnd-context.js';
...
export class SortContext extends DndContext {
    ...
    constructor(config) {
        ...
        this.dragSourceList$ = computed(() => {
            if (!this.active$.value) {
                return;
            }
            return this.items.value.flatMap(id => {
                const ele = asHTMLElement(this.container.querySelector(`[${draggableDataName.attribute}='${id}']`));
                if (!ele) {
                    return [];
                }
                return {
