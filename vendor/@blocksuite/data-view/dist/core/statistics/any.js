...
export const anyTypeStatsFunctions = [
    ...
    createStatisticConfig({
        ...
        dataType: t.unknown.instance(),
        impl: data => {
            const values = data.reduce((acc, v) => {
                if (Array.isArray(v)) {
                    return acc + v.length;
                }
    ...
    createStatisticConfig({
        ...
        impl: (data, { meta, dataSource }) => {
            const emptyList = data.filter(value => meta.config.jsonValue.isEmpty({ value, dataSource }));
    ...
    createStatisticConfig({
        ...
        dataType: t.unknown.instance(),
        impl: (data, { meta, dataSource }) => {
            const notEmptyList = data.filter(value => !meta.config.jsonValue.isEmpty({ value, dataSource }));
