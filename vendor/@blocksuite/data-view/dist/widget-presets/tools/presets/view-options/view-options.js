...
import { ArrowRightSmallIcon, DeleteIcon, DuplicateIcon, FilterIcon, GroupingIcon, InfoIcon, LayoutIcon, MoreHorizontalIcon, PlusIcon, SortIcon, } from '@blocksuite/icons/lit';
import { autoPlacement, offset, shift } from '@floating-ui/dom';
import { signal } from '@preact/signals-core';
import { css, html } from 'lit';
import { styleMap } from 'lit/directives/style-map.js';
import { filterTraitKey } from '../../../../core/filter/trait.js';
import { buildGroupSelectItems, buildGroupSettingItems, } from '../../../../core/group-by/setting.js';
import { groupTraitKey } from '../../../../core/group-by/trait.js';
import { popCreateFilter, renderUniLit, } from '../../../../core/index.js';
...
export class DataViewHeaderToolsViewOptions extends WidgetBase {
    constructor() {
        ...
        this.clickMoreAction = (e) => {
            e.stopPropagation();
            this.openMoreAction(popupTargetFromElement(e.currentTarget));
        };
...
export const popViewOptions = (target, dataViewLogic, onClose) => {
    const view = dataViewLogic.view;
    const currentPage = signal('main');
    const pageStack = ['main'];
    let menuHandler;
    let mainPageHeight = null;
    let customPageTitle = '';
    let customPageItems = () => [];
    const isDesktopMenu = () => menuHandler.menu.menuElement.tagName.toLowerCase() === 'affine-menu';
    const navigate = (page) => {
        if (!isDesktopMenu()) {
            pageStack.push(page);
    ...
    const navigateToCustomPage = (title, getItems) => {
    ...
    const titleConfig = {
        ...
        get postfix() {
            if (currentPage.value !== 'properties')
                return undefined;
    ...
    const getGroupPageItems = () => {
        ...
        if (!gProp)
            return [];
        return buildGroupSettingItems(groupTrait, () => navigate('group-select'), () => navigate('main'));
    };
    const getGroupSelectPageItems = () => {
        const groupTrait = view.traitGet(groupTraitKey);
        if (!groupTrait)
            return getMainPageItems();
    ...
    const getMainPageItems = () => {
        const items = [];
        items.push(menu.input({
        ...
        items.push(menu.group({
            items: [
                menuObj => {
                    const viewTypeItems = menuObj.renderItems(view.manager.viewMetas.map(meta => {
                        return menuObj => {
                            if (!menuObj.search(meta.model.defaultName)) {
                                return;
                            }
                            const isSelected = meta.type === view.manager.currentView$.value?.type;
        ...
        const viewSpecificItems = dataViewLogic.getViewOptionsSettingItems?.(navigateToCustomPage, goBack) ?? [];
    ...
    if (isDesktopMenu()) {
        ...
        requestAnimationFrame(() => {
            const bodyEl = menuHandler.menu.menuElement.querySelector('.affine-menu-body');
            if (bodyEl) {
