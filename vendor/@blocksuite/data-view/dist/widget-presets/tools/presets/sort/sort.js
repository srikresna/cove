...
import { createDefaultShowQuickSettingBar, ShowQuickSettingBarKey, } from '../../../quick-setting-bar/context.js';
...
export class DataViewHeaderToolsSort extends WidgetBase {
    ...
    clickSort(event) {
        ...
        popCreateSort(popupTargetFromElement(event.currentTarget), {
            sortUtils: {
                ...sortUtils,
                add: sort => {
                    sortUtils.add(sort);
                    ...
                    requestAnimationFrame(() => {
                        const ele = this.closest('affine-data-view-renderer')?.querySelector('.data-view-sort-button');
                        if (ele) {
                            popSortRoot(popupTargetFromElement(ele), {
                                sortUtils: sortUtils,
                            });
                        }
                    });
    ...
    toggleShowQuickSettingBar(show) {
        const map = this.view.serviceGetOrCreate(ShowQuickSettingBarKey, createDefaultShowQuickSettingBar);
        map.value = {
            ...map.value,
