...
import { TableViewAreaSelection, TableViewRowSelection, } from '../view-presets/table/selection';
...
describe('TableHotkeysController', () => {
    it('deletes rows on Backspace', () => {
        ...
        const ctrl = new TableHotkeysController(logic);
        ctrl.hostConnected();
    ...
    it.each(TAG_COLUMN_TYPES)('stages draft for %s column instead of valueSetFromString', columnType => {
        const { logic, selectionController } = createLogic();
        const ctrl = new TableHotkeysController(logic);
        ctrl.hostConnected();
...
describe('Virtual TableHotkeysController', () => {
    it('writes character to cell', () => {
        const { logic, selectionController } = createLogic();
        const ctrl = new VirtualHotkeysController(logic);
        ctrl.hostConnected();
        const cell = {
            rowId: 'r1',
            dataset: { rowId: 'r1', columnId: 'c1' },
        ...
        logic.keyDown({ get: () => ({ raw: evt }) });
        expect(cell.column$.value.valueSetFromString).toHaveBeenCalledWith('r1', 'b');
        expect(selectionController.selection.isEditing).toBe(true);
        expect(evt.preventDefault).toHaveBeenCalled();
    });
    it.each(TAG_COLUMN_TYPES)('stages draft for %s column instead of valueSetFromString', columnType => {
