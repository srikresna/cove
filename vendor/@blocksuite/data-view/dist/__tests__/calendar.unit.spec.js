...
import { CalendarSingleView, calendarViewModel, } from '../view-presets/calendar/index.js';
import { formatEntryTime, openCalendarEntry, } from '../view-presets/calendar/pc/actions.js';
import { getCalendarDndEntity } from '../view-presets/calendar/pc/dnd.js';
import { viewConverts } from '../view-presets/convert.js';
const day = (value) => new Date(`${value}T00:00:00`).getTime();
const createCalendarView = (options) => {
    const rows = signal(options?.rows ?? ['row-1']);
    const columns = signal(['title', 'date', 'end-date', 'status']);
    const viewData = signal({
        id: 'view-1',
        name: 'Calendar',
        mode: 'calendar',
    ...
    const dataSource = {
        ...
        viewDataUpdate: (_id, updater) => {
            viewData.value = { ...viewData.value, ...updater(viewData.value) };
        },
        cellValueGet: (rowId, propertyId) => values.get(`${rowId}:${propertyId}`),
        ...
        propertyTypeGet: (propertyId) => types.get(propertyId),
        propertyNameGet: (propertyId) => propertyId,
        propertyDataGet: () => ({}),
        ...
        serviceGet: (key) => {
            ...
            return {
                title: (pageId, referenceInfo) => signal(referenceInfo?.title ?? options?.linkedDocTitles?.[pageId]),
            };
        },
        propertyMetaGet: (type) => ({
            ...
            config: {
                rawValue: {
                    toJson: ({ value }) => {
                        const deltas = typeof value === 'object' && value != null && 'deltas$' in value
                            ? value.deltas$?.value
                            : undefined;
                        ...
                        return deltas
                            ...
                            if (item.attributes?.reference?.type === 'LinkedPage' &&
                                ...
                                return (options?.linkedDocTitles?.[pageId] ?? item.insert ?? '');
                            }
                            return item.insert ?? '';
                        })
                            .join('');
                    ...
                    toString: ({ value }) => typeof value === 'string' ? value : '',
                },
                jsonValue: {
                    schema: {
    ...
    return {
        view: new CalendarSingleView(manager, 'view-1'),
        viewData,
        values,
        types,
        columns,
...
describe('CalendarSingleView', () => {
    it('creates default view data without selecting a start date', () => {
        const data = calendarViewModel.model.defaultData({
            dataSource: {
                ...
                propertyTypeGet: (id) => (id === 'title' ? 'title' : 'date'),
            },
        });
    ...
    it('creates row range entries and falls back when end date is invalid', () => {
        ...
        expect(view.rowEntries$.value.map(entry => [
            entry.rowId,
            entry.startAt,
            entry.endAt,
        ])).toEqual([
    ...
    it('creates a dated linked-doc row', () => {
        ...
        const title = values.get('row-2:title');
        expect(rowId).toBe('row-2');
        expect(values.get('row-2:date')).toBe(day('2026-05-25'));
        expect(values.get('row-2:status')).toBe('Done');
    ...
    it('updates date mapping through setup APIs', () => {
        ...
        expect('date' in viewData.value && viewData.value.date.startColumnId).toBe('date');
        expect(view.createDateColumn()).toBe('created-date');
        expect('date' in viewData.value && viewData.value.date.startColumnId).toBe('created-date');
    });
    it('aggregates external source entries without mutating view data', async () => {
        ...
        await expect(view.loadExternalEntries({
            from: day('2026-05-01'),
            to: day('2026-05-31'),
    ...
    it('does not let stale external entry loads overwrite newer entries', async () => {
        ...
        expect(view.entries$.value.filter(entry => entry.kind === 'external')).toEqual([newEntry]);
        resolveOld([oldEntry]);
...
describe('calendar entry actions', () => {
    ...
    it('opens row entries through the detail panel hook', () => {
        ...
        const target = {};
        openCalendarEntry({ openDetailPanel }, view, {
            kind: 'row',
            id: 'database:row-1',
            sourceId: 'database',
            rowId: 'row-1',
        ...
        expect(openDetailPanel).toHaveBeenCalledWith(expect.objectContaining({ view, rowId: 'row-1' }));
    });
});
describe('calendar view converts', () => {
    it('converts header/card semantics without date mapping', () => {
        const tableToCalendar = viewConverts.find(convert => convert.from === 'table' && convert.to === 'calendar');
        const calendarToKanban = viewConverts.find(convert => convert.from === 'calendar' && convert.to === 'kanban');
        const filter = { type: 'group', op: 'and', conditions: [] };
        const sort = { columns: [] };
        const header = { titleColumn: 'title' };
        expect(tableToCalendar?.convert({ filter, sort, header })).toEqual({
            filter,
            sort,
            card: { titleColumnId: 'title', visiblePropertyIds: [] },
        });
        expect(calendarToKanban?.convert({
            filter,
            sort,
...
describe('calendar dnd payload', () => {
    ...
    it('reads document payloads from blocksuite dnd data', () => {
        expect(getCalendarDndEntity({ bsEntity: { type: 'doc', docId: 'doc-1' } })).toEqual({ type: 'doc', docId: 'doc-1' });
    });
});
