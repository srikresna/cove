...
export const multiSelectPropertyModelConfig = multiSelectPropertyType.modelConfig({
    name: 'Multi-select',
    kanbanGroup: {
        enabled: true,
        mutable: true,
    ...
    rawValue: {
        schema: zod.array(zod.string()),
        default: () => [],
        toString: ({ value, data }) => value.map(id => data.options.find(v => v.id === id)?.value).join(','),
        ...
        fromJson: ({ value }) => Array.isArray(value) && value.every(v => typeof v === 'string')
            ? value
            : undefined,
    },
