...
export class KanbanSelectionController {
    ...
    getNextFocusCard(selection, index, nextPosition) {
        ...
        const group = host.querySelector(`affine-data-view-kanban-group[data-key="${selection.cards[0].groupKey}"]`);
    ...
    hostConnected() {
        this.host?.disposables.add(this.logic.selection$.subscribe(selection => {
    ...
    moveCard(rowId, key) {
        ...
        this.view.groupTrait.moveCardTo(rowId, selection.cards[0].groupKey, key, 'start');
        requestAnimationFrame(() => {
