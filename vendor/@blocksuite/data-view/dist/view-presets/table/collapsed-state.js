 ...
 */
export function getCollapsedState(viewId, groupKey) {
    try {
        const value = sessionStorage.getItem(`affine:table-group:${viewId}:${groupKey}:collapsed`);
        if (!value)
            return false;
 ...
 */
export function setCollapsedState(viewId, groupKey, collapsed) {
    try {
        sessionStorage.setItem(`affine:table-group:${viewId}:${groupKey}:collapsed`, JSON.stringify(collapsed));
    }
    catch {
        // ignore
    }
