...
export class TableHotkeysController {
    ...
    hostConnected() {
        this.disposables.add(this.logic.bindHotkey({
            Backspace: () => {
            ...
            Escape: () => {
                ...
                if (TableViewRowSelection.is(selection)) {
                    const result = this.selectionController.rowsToArea(selection.rows.map(v => v.id));
                    if (result) {
            ...
            'Mod-a': context => {
                ...
                if (selection) {
                    ...
                    this.selectionController.selection = TableViewRowSelection.create({
                        rows: this.logic.view.groupTrait.groupsDataList$.value?.flatMap(group => group?.rows.map(row => ({
                            groupKey: group.key,
                            id: row.rowId,
                        })) ?? []) ??
            ...
            '/': context => {
                ...
                if (cell) {
                    ...
                    popRowMenu(this.logic, popupTargetFromElement(cell), this.selectionController);
                }
            },
        }));
        this.disposables.add(this.logic.handleEvent('keyDown', ctx => {
            const event = ctx.get('keyboardState').raw;
            return handleCharStartEdit({
                event,
                selection: this.selectionController.selection,
                getCellContainer: this.selectionController.getCellContainer.bind(this.selectionController),
