...
export class TableClipboardController {
    ...
    constructor(logic) {
        ...
        this._onCopy = (tableSelection, isCut = false) => {
            ...
            this.clipboard
                ...
                .then(() => {
                if (area[0]?.row) {
                    this.notification.toast(`${area.length} row${area.length > 1 ? 's' : ''} copied to clipboard`);
                }
                else {
                    const count = area.flatMap(row => row.cells).length;
                    this.notification.toast(`${count} cell${count > 1 ? 's' : ''} copied to clipboard`);
                }
            })
                .catch(console.error);
            return true;
    ...
    hostConnected() {
        this.disposables.add(this.logic.handleEvent('copy', _ctx => {
            const tableSelection = this.selection;
            if (!tableSelection)
                return false;
            this._onCopy(tableSelection);
        ...
        this.disposables.add(this.logic.handleEvent('cut', _ctx => {
            const tableSelection = this.selection;
            if (!tableSelection)
                return false;
            this._onCut(tableSelection);
        ...
        this.disposables.add(this.logic.handleEvent('paste', ctx => {
            if (this.readonly)
                return false;
...
function getSelectedArea(selection, table) {
    ...
    if (TableViewRowSelection.is(selection)) {
        const rows = TableViewRowSelection.rows(selection)
            .map(row => {
            const y = table.selectionController.getRow(row.groupKey, row.id)?.top$?.value ??
                0;
            return {
                y,
                row,
...
function pasteToCells(table, rows, selection) {
    ...
    for (let i = 0; i < targetRange.row.length; i++) {
        for (let j = 0; j < targetRange.column.length; j++) {
            ...
            if (rowId && columnId) {
                targetContainer?.column$.value?.valueSetFromString(rowId, dataString ?? '');
            }
        }
    }
}
