...
import { addGroupIconStyle, addGroupStyle, groupsHiddenMessageStyle, tableGroupsContainerStyle, tableScrollContainerStyle, tableViewStyle, tableWrapperStyle, } from './table-view-style';
export class TableViewUILogic extends DataViewUILogicBase {
    constructor() {
        super(...arguments);
        this.ui$ = signal();
        this.scrollContainer$ = signal();
        this.tableContainer$ = signal();
...
export class TableViewUI extends DataViewUIBase {
    ...
    renderTable() {
        const groups = this.logic.view.groupTrait.groupsDataList$.value?.filter((g) => g !== undefined);
        if (groups && groups.length) {
            return html `
        <div class="${tableGroupsContainerStyle}">
          ${repeat(groups, group => group.key, group => html `<affine-data-view-table-group
