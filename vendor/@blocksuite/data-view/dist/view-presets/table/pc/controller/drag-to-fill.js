...
let DragToFillElement = (() => {
    ...
    return class DragToFillElement extends _classSuper {
        static {
...
export function fillSelectionWithFocusCellData(logic, selection) {
    ...
    if (rowsSelection && columnsSelection) {
        ...
        for (let i = start; i <= end; i++) {
            if (i === focus.rowIndex)
                continue;
            const cellContainer = logic.selectionController.getCellContainer(groupKey, i, draggingColIdx);
            if (!cellContainer)
                continue;
