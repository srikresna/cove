...
export class TableSelectionController {
    ...
    handleDragEvent() {
        this.host?.disposables.add(this.logic.handleEvent('dragStart', context => {
    ...
    handleSelectionChange() {
        this.host?.disposables.add(this.logic.selection$.subscribe(tableSelection => {
    ...
    insertTo(groupKey, rowId, before) {
        ...
        requestAnimationFrame(() => {
            const index = this.logic.view.properties$.value.findIndex(v => v.type$.value === 'title');
    ...
    focusToCell(position) {
        ...
        const cells = Array.from(row?.querySelectorAll('dv-table-view-cell-container') ?? []);
        if (!row || !rows || !cells) {
            return;
        }
    ...
    selectionAreaRight() {
        ...
        if (newSelection.columnsSelection.start === newSelection.focus.columnIndex) {
            const max = (this.rows(newSelection.groupKey)
                ?.item(0)
    ...
    startDrag(evt, cell, fillValues) {
        const groupKey = cell.closest('affine-data-view-table-group')
            ?.group?.key;
    ...
    `;
        ...
        startUpdate(selection) {
            ...
            if (selection?.selectionType === 'area' &&
                ...
                const showDragToFillHandle = !isEditing &&
