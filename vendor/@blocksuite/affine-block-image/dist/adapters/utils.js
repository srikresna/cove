...
import { FetchUtils, FULL_FILE_PATH_KEY, getImageFullPath, } from '@blocksuite/affine-shared/adapters';
import { getFilenameFromContentDisposition } from '@blocksuite/affine-shared/utils';
import { sha } from '@blocksuite/global/utils';
import { nanoid, } from '@blocksuite/store';
export async function processImageNodeToBlock(imageURL, walkerContext, assets, configs) {
    let blobId = '';
    if (!FetchUtils.fetchable(imageURL)) {
        ...
        try {
            const res = await FetchUtils.fetchImage(imageURL, undefined, configs.get('imageProxy'));
            if (!res) {
                return;
            }
