...
export function slashItemClassName({ name }) {
...
function itemCompareFn(a, b) {
    ...
    if (isNaN(aGroupIndex))
        return -1;
    if (isNaN(bGroupIndex))
        return 1;
...
export function buildSlashMenuItems(items, context, transform) {
    if (transform)
...
export function mergeSlashMenuConfigs(configs) {
    return {
        items: ctx => Array.from(configs.values()).flatMap(({ items }) => typeof items === 'function' ? items(ctx) : items),
        disableWhen: ctx => configs
            .values()
