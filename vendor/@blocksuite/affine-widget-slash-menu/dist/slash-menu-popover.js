...
import { cleanSpecifiedTail, getInlineEditorByModel, getTextContentFromInlineRange, } from '@blocksuite/affine-rich-text';
import { DocModeProvider, TelemetryProvider, } from '@blocksuite/affine-shared/services';
import { createKeydownObserver, getCurrentNativeRange, getPopperPosition, isControlledKeyboardEvent, isFuzzyMatch, substringMatchScore, } from '@blocksuite/affine-shared/utils';
...
import { AFFINE_SLASH_MENU_MAX_HEIGHT, AFFINE_SLASH_MENU_TOOLTIP_TIMEOUT, AFFINE_SLASH_MENU_TRIGGER_KEY, } from './consts.js';
    ...
    return class SlashMenu extends _classSuper {
        ...
        constructor(inlineEditor, abortController = new AbortController()) {
            ...
            this._handleClickItem = (item) => {
                ...
                cleanSpecifiedTail(this.context.std, this.context.model, AFFINE_SLASH_MENU_TRIGGER_KEY + (this._query || ''));
                this.inlineEditor
                    .waitForUpdate()
                    .then(() => {
                    ...
                    this._telemetry?.track('SelectSlashMenuItem', {
                        ...
                        segment: this.context.model.flavour === 'affine:edgeless-text'
                            ? 'edgeless-text'
                            : 'doc',
                        module: 'slash menu',
                        control: item.name,
            ...
            this._updateFilteredItems = () => {
                ...
                while (queue.length !== 0) {
                    ...
                    this._filteredItems = this._filteredItems.concat(queue.filter(({ name, searchAlias = [] }) => [name, ...searchAlias].some(str => isFuzzyMatch(str, searchStr))));
            ...
            this.#items_accessor_storage = (__runInitializers(this, __position_extraInitializers), __runInitializers(this, _items_initializers, void 0));
            this.#context_accessor_storage = (__runInitializers(this, _items_extraInitializers), __runInitializers(this, _context_initializers, void 0));
...
let InnerSlashMenu = (() => {
    ...
    return class InnerSlashMenu extends _classSuper {
        ...
        render() {
            ...
            const groups = groupBy(this.menu, ({ group }) => group && !this.context.searching ? parseGroup(group)[1] : '');
            return html `<div
      class="slash-menu"
      style=${style}
    ...
    >
      ...
      ${when(!this.context.searching, () => html `<div class="slash-menu-group-name">${groupName}</div>`)}
