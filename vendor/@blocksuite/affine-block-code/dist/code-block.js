...
import { BRACKET_PAIRS, EDGELESS_TOP_CONTENTEDITABLE_SELECTOR, } from '@blocksuite/affine-shared/consts';
import { BlockElementCommentManager, DocModeProvider, NotificationProvider, } from '@blocksuite/affine-shared/services';
import { getViewportElement } from '@blocksuite/affine-shared/utils';
import { IS_MAC, IS_MOBILE } from '@blocksuite/global/env';
import { noop } from '@blocksuite/global/utils';
import { BlockSelection, TextSelection } from '@blocksuite/std';
import { getInlineRangeProvider, INLINE_ROOT_ATTR, } from '@blocksuite/std/inline';
...
let CodeBlockComponent = (() => {
    ...
    return class CodeBlockComponent extends _classSuper {
        constructor() {
            ...
            this._localPreview$ = signal(null);
            ...
            this.collapsed$ = computed(() => !!this.model.props.collapsed$.value);
        ...
        get langs() {
            return (this.std.getOptional(CodeBlockConfigExtension.identifier)?.langs ??
        ...
        _updateHighlightTokens() {
            ...
            const matchedInfo = this.langs.find(info => info.id === modelLang ||
                info.name === modelLang ||
        ...
        connectedCallback() {
            ...
            this.bindHotKey({
                ...
                'Mod-Enter': () => {
                    ...
                    if (index === -1)
                        return;
                    const id = this.store.addBlock('affine:paragraph', {}, parent, index + 1);
        ...
        renderBlock() {
            const showLineNumbers = (this.std.getOptional(CodeBlockConfigExtension.identifier)
                ?.showLineNumbers ??
                true) &&
                (this.model.props.lineNumber ?? true);
            const preview = this.preview$.value;
            const previewContext = this.std.getOptional(CodeBlockPreviewIdentifier(this.model.props.language ?? ''));
