...
import { ConnectorElementModel, MindmapElementModel, } from '@blocksuite/affine-model';
...
export class SnapOverlay extends Overlay {
    constructor() {
        ...
        this._distributeCooldown = new AdaptiveCooldownController({
            cooldownFrames: DISTRIBUTE_ALIGN_COOLDOWN_FRAMES,
            maxCostMs: DISTRIBUTE_ALIGN_MAX_COST_MS,
        });
    }
    static { this.overlayName = 'snap-manager'; }
    clear() {
        this._referenceBounds = {
            vertical: [],
            horizontal: [],
    ...
    _alignDistributeHorizontally(rst, bound, threshold, viewport) {
        ...
        for (let i = 0; i < wBoxes.length; i++) {
            for (let j = i + 1; j < wBoxes.length; j++) {
                ...
                if (!lb.isHorizontalCross(rb) || lb.isIntersectWithBound(rb))
                    continue;
                ...
                const updateDif = () => {
                    ...
                    const curAveDis = (Math.abs(lb.center[0] - bound.center[0]) +
    ...
    _alignDistributeVertically(rst, bound, threshold, viewport) {
        ...
        for (let i = 0; i < hBoxes.length; i++) {
            for (let j = i + 1; j < hBoxes.length; j++) {
                ...
                if (!ub.isVerticalCross(db) || ub.isIntersectWithBound(db))
                    continue;
                let switchFlag = false;
                if (db.maxY < ub.minX) {
                    const temp = ub;
                ...
                const updateDiff = () => {
                    ...
                    const curAveDis = (Math.abs(ub.center[1] - bound.center[1]) +
    ...
    _calculateClosestDistances(bound, other) {
        ...
        return {
            horiz: closestX <= threshold
                ? {
                    distance: xDistances[xDistancesAbs.indexOf(closestX)],
                    get alignPositionIndices() {
                        const indices = [];
                        xDistancesAbs.forEach((val, idx) => almostEqual(val, closestX) && indices.push(idx));
                        return indices;
                    },
                }
                : undefined,
            vert: closestY <= threshold
                ? {
                    distance: yDistances[yDistancesAbs.indexOf(closestY)],
                    get alignPositionIndices() {
                        const indices = [];
                        yDistancesAbs.forEach((val, idx) => almostEqual(val, closestY) && indices.push(idx));
                        return indices;
                    },
                }
                : undefined,
    ...
    _updateXAlignPoint(rst, bound, other, distance) {
        if (!distance.horiz)
            return;
        const { distance: dx, alignPositionIndices: distanceIndices } = distance.horiz;
        ...
        this._intraGraphicAlignLines.horizontal = distanceIndices.map(idx => [
    ...
    _updateYAlignPoint(rst, bound, other, distance) {
        ...
        this._intraGraphicAlignLines.vertical = alignPositionIndices.map(idx => [
            new Point(left, alignXPosition[idx]),
            new Point(right, alignXPosition[idx]),
        ]);
    }
    alignResize(position, direction) {
        const rst = { dx: 0, dy: 0 };
        const { viewport } = this.gfx;
        const threshold = ALIGN_THRESHOLD / viewport.zoom;
        const searchBound = new Bound(position[0] - threshold / 2, position[1] - threshold / 2, threshold, threshold);
        ...
        for (const other of this._referenceBounds.all) {
            const closestDistances = this._calculateClosestDistances(alignBound, other);
    ...
    align(bound) {
        ...
        const shouldTryDistribute = this._referenceBounds.all.length <= DISTRIBUTE_ALIGN_MAX_CANDIDATES &&
    ...
    render(ctx) {
        if (this._intraGraphicAlignLines.vertical.length === 0 &&
    ...
    _updateAlignCandidates(movingBound) {
        ...
        const horizAreaBound = new Bound(Math.min(movingBound.x, viewportBound.x), movingBound.y, Math.max(movingBound.w, viewportBound.w), movingBound.h);
        const vertAreaBound = new Bound(movingBound.x, Math.min(movingBound.y, viewportBound.y), movingBound.w, Math.max(movingBound.h, viewportBound.h));
        ...
        const horizBounds = [];
        const allCandidateElements = new Set();
        vertCandidates.forEach(candidate => {
            if (skipped.has(candidate) || this._isSkippedElement(candidate))
                return;
    ...
    setMovingElements(movingElements, excludes = []) {
        ...
        return movingElements.reduce((prev, element) => prev.unite(element.elementBound), movingElements[0].elementBound);
    }
}
