  ...
  `; }
        ...
        addNote() {
            ...
            if (note) {
                ...
                if (note.root) {
                    note.root.props.title.insert(rowContent ?? '', 0);
                    note.root.children
                        .find(child => child.flavour === 'affine:note')
                        ?.children.find(block => matchModels(block, [
                        ParagraphBlockModel,
                        ListBlockModel,
                        CodeBlockModel,
                    ]));
        ...
        render() {
            return html `
      <div
        style="height: 1px;max-width: var(--affine-editor-width);background-color: ${unsafeCSS(cssVarV2.layer.insideBorder.border)};margin: auto;margin-bottom: 16px"
      ></div>
      ${this.renderNote()}
    `;
        }
