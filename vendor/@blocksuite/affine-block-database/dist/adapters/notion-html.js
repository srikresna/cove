...
import { AdapterTextUtils, BlockNotionHtmlAdapterExtension, HastUtils, } from '@blocksuite/affine-shared/adapters';
...
export const databaseBlockNotionHtmlAdapterMatcher = {
    flavour: DatabaseBlockSchema.model.flavour,
    toMatch: o => HastUtils.isElement(o.node) && DATABASE_NODE_TYPES.has(o.node.tagName),
    fromMatch: () => false,
    toBlockSnapshot: {
        enter: (o, context) => {
            ...
            switch (o.node.tagName) {
                case 'th': {
                    ...
                    walkerContext.pushGlobalContextStack('hast:table:column', {
                        type: columnType,
                        name: HastUtils.getTextContent(HastUtils.getTextChildrenOnlyAst(o.node)),
                ...
                case 'tr': {
                    if (o.parent?.node.type === 'element' &&
                        ...
                        const columns = walkerContext.getGlobalContextStack('hast:table:column');
                        ...
                        HastUtils.getElementChildren(o.node).forEach((child, index) => {
                            if (plainTable || columns[index] === undefined) {
                                ...
                                if (columns[index] === undefined) {
                                    ...
                                    walkerContext.pushGlobalContextStack('hast:table:children', {
                                        type: 'block',
                                        id: nanoid(),
                            ...
                            if (HastUtils.querySelector(child, 'time')) {
                                ...
                                let rawColumnData = HastUtils.getTextContent(timeElement).trim();
                                ...
                                child.type === 'element' &&
                                    child.children.forEach(span => {
                                        const filteredArray = column.data.options?.filter(option => option.value === HastUtils.getTextContent(span));
        ...
        leave: (o, context) => {
            ...
            switch (o.node.tagName) {
                case 'table': {
                    ...
                    const children = walkerContext.getGlobalContextStack('hast:table:children');
                    ...
                    if (o.parent?.node.type === 'element' &&
                        ...
                        databaseTitle = HastUtils.getTextContent(HastUtils.querySelector(o.parent.node, NotionDatabaseTitleToken));
                    }
...
export const DatabaseBlockNotionHtmlAdapterExtension = BlockNotionHtmlAdapterExtension(databaseBlockNotionHtmlAdapterMatcher);
